...
- **加载**：`PluginManager::load_plugins`（`plugin/mod.rs:816`）→ 拓扑排序依赖（:511）→ 权限审批（交互询问 :571、配置/缓存/询问裁决 :1198-1214）→ `on_load` 前**按权限配置 WASI 沙箱**（socket 策略/DNS/环境变量/预开目录/内存上限，`wasm_host/mod.rs:50-102,381-429`）
- **事件分发**：`fire()`（mod.rs:1640，零 handler 一行早退 :1646；blocking 段 `fire_blocking` :1682）按事件名取 handler 列表 → **先串行 blocking（可改事件）再串行非 blocking** → WASM handler 经 `call_guest(handle-event)` 进出，事件数据（含 cancelled）双向同步
- **热重载**：`notify` watcher 监听 `./plugins`，`.wasm` 变更（大小写不敏感）→ 500ms 去抖按路径合并 → 卸载旧 → 加载新（mod.rs:368-466）
