...
// 生成带有 `from_string` 和 `identifier_string` 方法的枚举的构建器。
pub struct EnumCreator {
    /// 要生成的枚举名称（转换为 PascalCase）。
    pub name: String,
    /// 变体名的集合（枚举变体会转换为 PascalCase）。
    pub values: BTreeSet<String>,
}

impl ToTokens for EnumCreator {
    /// 生成枚举定义及其 `from_string`/`identifier_string` impl 块。
...
/// 生成 `Tag` 类型、`RegistryKey` 枚举、标签
/// 模块，以及 `Taggable` trait 及其查找辅助函数。
