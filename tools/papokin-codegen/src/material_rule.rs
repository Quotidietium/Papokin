    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match self {
            Self::Absolute { absolute } => {
                tokens.extend(quote!(YOffset::Absolute(papokin_util::y_offset::Absolute { absolute: #absolute })));
            }
            Self::AboveBottom { above_bottom } => {
                tokens.extend(quote!(YOffset::AboveBottom(papokin_util::y_offset::AboveBottom { above_bottom: #above_bottom })));
            }
            Self::BelowTop { below_top } => {
                tokens.extend(quote!(YOffset::BelowTop(papokin_util::y_offset::BelowTop { below_top: #below_top })));
            }
        }
    }
}

/// 来自 `material_condition` 或内联于 `material_rule` 的原始反序列化表面材质条件。
#[derive(Deserialize, Clone)]
#[serde(untagged)]
...
/// 来自 `material_rule` 文件夹或内联定义的原始反序列化材质规则。
#[derive(Deserialize, Clone)]
#[serde(untagged)]
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match self {
            ...
            Self::VerticalGradient {
                ...
                let lo = u64::from_le_bytes(bytes[0..8].try_into().expect("长度不正确"));
                let hi = u64::from_le_bytes(bytes[8..16].try_into().expect("长度不正确"));
                tokens.extend(quote!(
            ...
            Self::StoneDepth {
                ...
                let surface_type_token = match surface_type.as_str() {
                    "ceiling" => quote!(
                        papokin_util::math::vertical_surface_type::VerticalSurfaceType::Ceiling
                    ),
                    "floor" => quote!(
                        papokin_util::math::vertical_surface_type::VerticalSurfaceType::Floor
                    ),
                    _ => quote!(panic!("未知的表面类型")),
                };
    ...
    match raw {
        RawMaterialCondition::Ref(r) => {
            ...
            let content = fs::read_to_string(&path)
                .unwrap_or_else(|_| panic!("读取条件文件 {:?} 失败", path));
            let parsed: RawMaterialCondition = serde_json::from_str(&content)
                .unwrap_or_else(|e| panic!("解析条件文件 {:?} 失败：{}", path, e));
...
pub fn resolve_rule(
    ...
    match raw {
        RawMaterialRule::Ref(r) => {
            ...
            let content = fs::read_to_string(&path)
                .unwrap_or_else(|_| panic!("读取规则文件 {:?} 失败", path));
            let parsed: RawMaterialRule = serde_json::from_str(&content)
                .unwrap_or_else(|e| panic!("解析规则文件 {:?} 失败：{}", path, e));
...
/// 读取 material_rule 文件，并从 material_condition 文件夹解析条件。
pub fn build() -> TokenStream {
    ...
    for rule_name in top_level_rules {
        ...
        let resolved = resolve_rule(&raw, rule_dir, cond_dir)
            .unwrap_or_else(|| panic!("解析材料规则 {} 失败", rule_name));
    ...
    quote!(
        ...
        use papokin_util::y_offset::YOffset;
        use papokin_util::y_offset::Absolute;
        ...
        pub struct StoneDepthMaterialCondition {
            ...
            pub surface_type: papokin_util::math::vertical_surface_type::VerticalSurfaceType,
        }
