...
struct TradeSetJson {
    trades: String, // 形如 "#minecraft:armorer/level_1" 的标签
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        ...
        for entry in entries {
            ...
            if path.is_dir() {
                ...
                let content = fs::read_to_string(&path).expect("读取 JSON 文件失败");
...
pub fn build() -> TokenStream {
    let data: VillagerDataJson =
        serde_json::from_str(&fs::read_to_string("../../assets/villager_data.json").unwrap())
            .expect("解析 villager_data.json 失败");
