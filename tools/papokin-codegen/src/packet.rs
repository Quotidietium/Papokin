...
/// 在 `PacketId::to_id` 中用作未知版本回退的最新协议版本。
const LATEST_VERSION: JavaMinecraftVersion = JavaMinecraftVersion::V_1_21_11;

/// 表示 JSON 中的 protocol_id 对象。
#[derive(Deserialize)]
pub struct PacketInfo {
    pub protocol_id: i32,
}

/// 表示从数据包方向（服务器接收 / 客户端接收）到数据包的映射。
...
/// 单个版本化网络数据包映射文件的原始反序列化结构。
#[derive(Deserialize)]
pub struct Packets(pub BTreeMap<String, PhaseData>);

/// 供 `assets/packets.json` / `26_3_packets.json` 使用的紧凑转储。
#[derive(Deserialize)]
...
fn parse_packets(path: &str, content: &str) -> Packets {
    ...
    let compact: CompactPackets =
        serde_json::from_str(content).unwrap_or_else(|e| panic!("解析 {path} 失败：{e}"));
...
/// 生成 `PacketId` 结构体、`CURRENT_MC_VERSION` 以及
/// 所有 `serverbound`/`clientbound` 数据包 ID 常量。
...
/// 根据可用版本动态生成 `PacketId` 结构体及其实现（包括 `to_id`）。
...
fn generate_phase_modules(
    ...
    // 为每个阶段定义别名，以保持向后兼容及与 PacketEvents 对齐
