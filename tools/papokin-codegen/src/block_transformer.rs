...
fn predicate_to_tokens(pred: &PredicateJson, valid_blocks: &HashSet<String>) -> TokenStream {
    ...
    match pred.predicate_type.as_str() {
        ...
        _ => panic!("不支持的谓词类型：{}", pred.predicate_type),
    }
}
...
fn state_provider_to_tokens(provider: &StateProviderJson) -> TokenStream {
    match provider.provider_type() {
        "minecraft:simple_state_provider" => {
            let state_name = provider.state_name().expect("simple_state 缺少 state");
        ...
        "minecraft:copy_properties_provider" | "minecraft:copy_properties" => {
            let src = provider
                .source
                .as_ref()
                .expect("copy_properties 缺少 source");
            let state_name = src.state_name().expect("copy_properties 缺少 state");
            let id = block_ident(state_name);
            quote! {
                BlockTransformerStateProvider::CopyProperties(BlockId::#id)
            }
        }
        other => panic!("不支持的状态提供器类型：{other}"),
    }
}
...
pub fn build() -> TokenStream {
    ...
    if dir.is_dir() {
        for entry in fs::read_dir(dir).expect("读取 block_transformer 目录失败") {
            let entry = entry.expect("DirEntry 错误");
            let path = entry.path();
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                let stem = path.file_stem().unwrap().to_str().unwrap().to_string();
                let content = fs::read_to_string(&path)
                    .unwrap_or_else(|_| panic!("读取 {} 失败", path.display()));
