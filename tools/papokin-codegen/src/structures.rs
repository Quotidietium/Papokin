...
pub struct WeightedEntryStruct {
    /// 结构的注册表键（例如 `"minecraft:village_plains"`）。
...
fn generation_step_to_token(step: &str) -> TokenStream {
    match step {
        ...
        _ => panic!("未知的生成阶段：{step}"),
    }
}

/// 从 26.3 数据包读取 structure 和 structure_set 文件，并生成完整的结构 `TokenStream`。
pub fn build() -> TokenStream {
    ...
    let mut s_entries: Vec<_> = fs::read_dir(structures_dir)
        .expect("缺少 worldgen/structure 目录")
        .flatten()
    ...
    for entry in s_entries {
        ...
        let content = fs::read_to_string(&path).expect("读取结构文件失败");
        let structure: StructureStruct =
            serde_json::from_str(&content).expect("解析结构 JSON 失败");
    ...
    let mut ss_entries: Vec<_> = fs::read_dir(structure_sets_dir)
        .expect("缺少 worldgen/structure_set 目录")
        .flatten()
        .filter(|e| e.path().extension().is_some_and(|ext| ext == "json"))
        .collect();
    ...
    for entry in ss_entries {
        let path = entry.path();
        let stem = path.file_stem().unwrap().to_string_lossy().into_owned();
        let content = fs::read_to_string(&path).expect("读取 structure_set 文件失败");
        let set: StructureSetStruct =
            serde_json::from_str(&content).expect("解析 structure_set JSON 失败");
    ...
    quote!(
        use papokin_util::math::floor_div;
