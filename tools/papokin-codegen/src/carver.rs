...
pub fn build() -> TokenStream {
    ...
    for entry in entries {
        ...
        let content = fs::read_to_string(&path).expect("读取雕刻器文件失败");
        let val: Value = serde_json::from_str(&content).expect("解析雕刻器 JSON 失败");
    ...
    quote! {
        use papokin_util::math::float_provider::{
