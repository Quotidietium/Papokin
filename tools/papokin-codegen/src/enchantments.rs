...
/// `enchantment/*.json` 中单个附魔条目的原始反序列化结构。
#[derive(Deserialize)]
...
impl AttributeModifierSlot {
    /// 将此槽位变体转换为 `TokenStream`，供生成的代码使用。
...
fn parse_float_provider(v: &serde_json::Value) -> TokenStream {
    if let Some(f) = v.as_f64() {
        let f = f as f32;
        quote! { papokin_util::math::float_provider::FloatProvider::Constant(#f) }
    } else if let Some(type_str) = v.get("type").and_then(|s| s.as_str()) {
        match type_str {
            "minecraft:constant" => {
                let val = v.get("value").and_then(|n| n.as_f64()).unwrap_or(0.0) as f32;
                quote! { papokin_util::math::float_provider::FloatProvider::Constant(#val) }
            }
            "minecraft:uniform" => {
                let min = v
                    .get("min_inclusive")
                ...
                let max = v
                    ...
                    .unwrap_or(0.0) as f32;
                quote! {
                    papokin_util::math::float_provider::FloatProvider::Object(
                        papokin_util::math::float_provider::NormalFloatProvider::Uniform(
                            papokin_util::math::float_provider::UniformFloatProvider::new(#min, #max)
                        )
                    )
                }
            }
            "minecraft:trapezoid" => {
                ...
                quote! {
                    papokin_util::math::float_provider::FloatProvider::Object(
                        papokin_util::math::float_provider::NormalFloatProvider::Trapezoid(
                            papokin_util::math::float_provider::TrapezoidFloatProvider::new(#min, #max, #plateau)
                        )
                    )
                }
            }
            _ => quote! { papokin_util::math::float_provider::FloatProvider::Constant(0.0) },
        }
    } else {
        quote! { papokin_util::math::float_provider::FloatProvider::Constant(0.0) }
    }
}
...
fn parse_velocity_source(v: Option<&serde_json::Value>) -> (f32, TokenStream) {
    ...
    let base_token = match base {
        ...
        None => quote! { papokin_util::math::float_provider::FloatProvider::Constant(0.0) },
    };
            ...
            "minecraft:apply_impulse" => {
                ...
                quote! {
                    EnchantmentEntityEffect::ApplyImpulse {
                        direction: papokin_util::math::vector3::Vector3::new(#dir_x, #dir_y, #dir_z),
                        coordinate_scale: papokin_util::math::vector3::Vector3::new(#scale_x, #scale_y, #scale_z),
                        magnitude: #mag,
                    }
                }
            }
        ...
        "minecraft:matching_block_tag" => {
            ...
            quote! {
                ReplaceDiskPredicate::MatchingBlockTag {
                    offset: papokin_util::math::vector3::Vector3::new(#offset_x, #offset_y, #offset_z),
...
/// 生成 `Enchantment` 结构体、`AttributeModifierSlot` 枚举的 `TokenStream`，
/// 数据驱动的效果类型，以及 `from_name`/`from_id` 查找方法。
pub fn build() -> TokenStream {
    ...
    for (i, entry) in entries.iter().enumerate() {
        ...
        let content = fs::read_to_string(entry.path()).expect("读取附魔文件失败");
        let mut enc: Enchantment = serde_json::from_str(&content).expect("解析附魔 JSON 失败");
        enc.id = i as u8;
    ...
    quote! {
        ...
        pub enum ReplaceDiskPredicate {
            MatchingBlockTag {
                offset: papokin_util::math::vector3::Vector3<i32>,
        ...
        pub enum EnchantmentEntityEffect {
            ...
            SpawnParticles {
                ...
                horizontal_velocity: VelocitySource,
                vertical_velocity: VelocitySource,
                speed: papokin_util::math::float_provider::FloatProvider,
            },
            RunFunction {
                function: &'static str,
            },
            ...
            ApplyImpulse {
                direction: papokin_util::math::vector3::Vector3<f64>,
                coordinate_scale: papokin_util::math::vector3::Vector3<f64>,
        ...
        impl PositionSourceType {
            ...
            pub fn get_coordinate(
                ...
                random: &mut impl papokin_util::random::RandomImpl,
            ) -> f64 {
                match self {
        ...
        pub struct VelocitySource {
            ...
            pub base: papokin_util::math::float_provider::FloatProvider,
        }
        ...
        impl VelocitySource {
            ...
            pub const fn new(
                movement_scale: f32,
                base: papokin_util::math::float_provider::FloatProvider,
            ) -> Self {
                Self {
                    movement_scale,
                    base,
            ...
            pub const fn movement_scaled(scale: f32) -> Self {
                Self {
                    ...
                    base: papokin_util::math::float_provider::FloatProvider::Constant(0.0),
                }
            }

            #[must_use]
            pub const fn fixed_velocity(
                provider: papokin_util::math::float_provider::FloatProvider,
            ) -> Self {
                Self {
                    movement_scale: 0.0,
                    base: provider,
        ...
        pub struct Enchantment {
            ...
            pub description: &'static str, // TODO 使用 TextComponent
            pub anvil_cost: u32,
