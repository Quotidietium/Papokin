...
/// 在 `TrackedId::get` 中用作未知版本回退的最新协议版本。
...
/// 生成 `TrackedId`、`TrackedData` 以及所有按实体跟踪模块的 `TokenStream`。
pub(crate) fn build() -> TokenStream {
    ...
    if raw_versions.is_empty() {
        panic!("在 assets/tracked_data 中未找到跟踪数据资源文件");
    }
...
/// 生成每个受支持版本含一个 `u8` 字段的 `TrackedId` 结构体定义。
...
/// 生成带有 `id` 和 `type` 字段的 `TrackedData` 结构体。
...
fn canonicalize_tracked_field_name(name: &str) -> String {
    match name.to_uppercase().as_str() {
        // 同一索引下 Yarn（1.21.x 资源）与 Mojang（26.x 资源）的名称对照。
...
mod tests {
    ...
    fn living_entity_flags_merge_yarn_and_mojang_names() {
        ...
        let living = generated
            ...
            .nth(1)
            .expect("living_entity 模块");
        let flags = living
            .split("DATA_LIVING_ENTITY_FLAGS")
            .nth(1)
            .expect("DATA_LIVING_ENTITY_FLAGS 常量")
            .split("pub const")
            .next()
            .expect("常量主体");
        assert!(
    ...
    fn field_body(generated: &str, module: &str, field: &str) -> String {
        let module_src = generated
            ...
            .nth(1)
            .unwrap_or_else(|| panic!("{module} 模块"));
        module_src
            .split(field)
            .nth(1)
