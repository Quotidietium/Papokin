...
/// 生成 `BREWING_RECIPES`、`POTION_RECIPES` 和 `ITEM_RECIPES` 的 `TokenStream`。
pub fn build() -> TokenStream {
    // 2. 从数据包加载 26.3 数据驱动的酿造配方
    ...
    for entry in entries {
        let content = fs::read_to_string(entry.path()).expect("读取酿造配方失败");
        let recipe: DataDrivenBrewingRecipe = serde_json::from_str(&content)
            .unwrap_or_else(|e| panic!("解析酿造配方 {} 失败：{e}", entry.path().display()));
