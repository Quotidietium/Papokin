...
fn collect_configured_features(dir: &Path, prefix: &str, map: &mut BTreeMap<String, Value>) {
    ...
    for entry in entries {
        ...
        if path.is_dir() {
            ...
            let content = fs::read_to_string(&path).expect("读取 configured_feature 文件失败");
            let val: Value =
                serde_json::from_str(&content).expect("解析 configured_feature JSON 失败");
            map.insert(key, val);
        }
    }
}

/// 从 26.2 数据包读取 configured_feature 文件，并生成完整的 `ConfiguredFeature` 枚举 `TokenStream`。
...
/// 从 26.2 数据包读取 configured_feature 文件，并生成 `build_configured_features()` 函数的 `TokenStream`。
pub fn build() -> TokenStream {
    ...
    let entries: Vec<TokenStream> = json
        ...
        .map(|(name, value)| {
            ...
            let variant_name = format_ident!("{}", name.to_pascal_case());
            quote! {
                map.insert(papokin_data::configured_feature::ConfiguredFeature::#variant_name, #cf);
            }
        })
        .collect();
    ...
    quote! {
        ...
        fn build_configured_features() -> std::collections::HashMap<papokin_data::configured_feature::ConfiguredFeature, ConfiguredFeature> {
...
fn value_to_fossil_processor(value: &Value) -> TokenStream {
    let variant = match value.as_str().expect("化石处理器的名称必须为字符串") {
        ...
        processor => panic!("不支持的化石处理器：{processor}"),
    };
...
/// 将单个已配置地物 JSON 值转换为其 `ConfiguredFeature` token 流。
///
/// # Arguments
/// – `v` – 该特征的 JSON 对象，应包含 `"type"` 和 `"config"` 字段。
    ...
    match type_str {
        ...
        "minecraft:vegetation_patch" | "minecraft:waterlogged_vegetation_patch" => {
            ...
            let surface = match config["surface"].as_str().unwrap_or("floor") {
                "ceiling" => {
                    quote! { papokin_util::math::vertical_surface_type::VerticalSurfaceType::Ceiling }
                }
                _ => {
                    quote! { papokin_util::math::vertical_surface_type::VerticalSurfaceType::Floor }
                }
            };
        ...
        "minecraft:fossil" => {
            let fossil_structures = config["fossil_structures"]
                .as_array()
                .expect("fossil_structures 必须为数组")
                .iter()
                .map(|value| {
                    let name = value.as_str().expect("化石结构的名称必须为字符串");
                    quote! { #name }
                });
            let overlay_structures = config["overlay_structures"]
                .as_array()
                .expect("overlay_structures 必须为数组")
                .iter()
                .map(|value| {
                    let name = value.as_str().expect("化石叠加结构的名称必须为字符串");
                    quote! { #name }
                });
            ...
            let max_empty_corners_allowed = config["max_empty_corners_allowed"]
                .as_u64()
                .expect("max_empty_corners_allowed 必须为无符号整数")
                as u8;
        ...
        "minecraft:multiface_growth" => {
                ...
                .map(|arr| {
                    arr.iter()
                        ...
                        .map(|name| {
                            ...
                            quote! { papokin_data::BlockId::#ident }
                        })
                        .collect()
                })
                .unwrap_or_default();
            quote! {
                ConfiguredFeature::MultifaceGrowth(crate::generation::feature::features::multiface_growth::MultifaceGrowthFeature {
                    place_block: papokin_data::BlockId::#block_ident,
        ...
        "minecraft:end_gateway" => {
            let exit = if let Some(exit_arr) = config["exit"].as_array() {
                ...
                quote! { Some(papokin_util::math::position::BlockPos::new(#x, #y, #z)) }
            } else {
                quote! { None }
            };
...
/// 将方块状态提供者 JSON 对象转换为其 `BlockStateProvider` token 流。
///
/// # Arguments
/// – `v` – 该提供器的 JSON 值，应包含 `"type"` 字段。
///
/// # Returns
...
/// 将噪声提供者 JSON 对象转换为 `NoiseBlockStateProviderBase` token 流。
///
/// # Arguments
/// – `v` – 包含 `"seed"`、`"noise"` 和 `"scale"` 字段的 JSON 对象。
...
/// 将双重柏林噪声参数 JSON 对象转换为 `DoublePerlinNoiseParametersCodec` token 流。
///
/// # Arguments
/// - `v` - 包含 `"firstOctave"` 和 `"amplitudes"` 字段的 JSON 对象。
...
/// 将规则测试 JSON 对象转换为其 `RuleTest` 枚举变体 token 流。
///
/// # Arguments
/// – `v` – 该规则测试的 JSON 对象，应包含 `"predicate_type"` 字段。
///
/// # Returns
/// 一个对应相应 `RuleTest` 变体的 `TokenStream`；类型无法识别时生成 `compile_error!`。
fn value_to_rule_test(v: &Value) -> TokenStream {
    ...
    match type_str {
        ...
        "minecraft:block_match" => {
            ...
            quote! { RuleTest::BlockMatch(BlockMatchRuleTest { block: papokin_data::BlockId::#block_ident }) }
        }
        ...
        "minecraft:tag_match" => {
            ...
            quote! { RuleTest::TagMatch(TagMatchRuleTest { tag: papokin_data::tag::Block::#tag_ident }) }
        }
        "minecraft:random_block_match" => {
            ...
            quote! { RuleTest::RandomBlockMatch(RandomBlockMatchRuleTest { block: papokin_data::BlockId::#block_ident, probability: #prob }) }
        }
...
/// 将方块列表 JSON 值（字符串或数组）转换为 `BlockWrapper` token 流。
///
/// # Arguments
/// – `v` – JSON 字符串（单个方块）或字符串数组（多个方块）。
///
/// # Returns
/// 字符串值用 `BlockWrapper::Single`，数组用 `BlockWrapper::Multi`；其他类型默认为 `BlockWrapper::Single("")`。
...
/// 将树木地物配置 JSON 对象转换为 `TreeFeature` token 流。
///
/// # Arguments
/// – `config` – `minecraft:tree` 配置化特征 JSON 条目中的 `"config"` 子对象。
...
fn value_to_block_list(v: &Value) -> TokenStream {
    if let Some(tag) = v.as_str().and_then(|s| s.strip_prefix('#')) {
        ...
        return quote! { &papokin_data::tag::Block::#ident.1 };
    }
    ...
    if let Some(arr) = v.as_array() {
        for b in arr {
            if let Some(s) = b.as_str() {
                ...
                let ident = syn::Ident::new(&name, proc_macro2::Span::call_site());
                blocks.push(quote! { papokin_data::BlockId::#ident.as_u16() });
            }
        }
    }
    quote! { &[#(#blocks),*] }
...
/// 将树干放置器 JSON 对象转换为 `TrunkPlacer` token 流。
///
/// # Arguments
/// – `v` – 树配置中的 `"trunk_placer"` 子对象，包含 `"type"`、`"base_height"`、`"height_rand_a"` 和 `"height_rand_b"` 字段。
...
/// 将树叶放置器 JSON 对象转换为 `FoliagePlacer` token 流。
///
/// # Arguments
/// – `v` – 树配置中的 `"foliage_placer"` 子对象，包含 `"type"`、`"radius"` 和 `"offset"` 字段。
...
/// 将地物尺寸 JSON 对象转换为 `FeatureSize` token 流。
///
/// # Arguments
/// – `v` – 树配置中的 `"minimum_size"` 子对象，包含 `"type"` 字段和尺寸参数。
...
/// 将树木装饰器 JSON 对象转换为其 `TreeDecorator` 枚举变体 token 流。
///
/// # Arguments
/// – `v` – 单个装饰器条目的 JSON 对象，应包含 `"type"` 字段。
///
/// # Returns
/// 一个对应相应 `TreeDecorator` 变体的 `TokenStream`；类型无法识别时生成 `compile_error!`。
...
/// 将内联的已放置地物 JSON 对象转换为 `PlacedFeature` token 流。
///
/// # Arguments
/// – `v` – 包含 `"feature"` 和 `"placement"` 字段的 JSON 对象。
...
/// 将已放置地物引用 JSON 值转换为 `PlacedFeatureWrapper` token 流。
///
/// # Arguments
/// – `v` – JSON 字符串（命名引用）或对象（内联放置特征）。
///
/// # Returns
/// 字符串值用 `PlacedFeatureWrapper::Named`，内联对象用 `PlacedFeatureWrapper::Direct`；其他类型默认为 `PlacedFeatureWrapper::Named(papokin_data::placed_feature::PlacedFeature::Acacia)`。
fn value_to_placed_feature_wrapper(v: &Value) -> TokenStream {
    match v {
        Value::String(s) => {
            let name = s.strip_prefix("minecraft:").unwrap_or(s);
            let variant_name = format_ident!("{}", name.to_pascal_case());
            quote! { PlacedFeatureWrapper::Named(papokin_data::placed_feature::PlacedFeature::#variant_name) }
        }
        Value::Object(_) => {
            // 它可能是 PlacedFeature 对象
            let pf = value_to_inline_placed_feature(v);
            quote! { PlacedFeatureWrapper::Direct(#pf) }
        }
        _ => {
            quote! { PlacedFeatureWrapper::Named(papokin_data::placed_feature::PlacedFeature::Acacia) }
        }
    }
}

/// 将地物引用 JSON 值转换为 `Feature` 枚举 token 流。
///
/// # Arguments
/// – `v` – JSON 字符串（命名引用）或对象（内联配置化特征）。
///
/// # Returns
/// 字符串值用 `Feature::Named`，对象用 `Feature::Inlined`；其他类型默认为 `Feature::Named(papokin_data::configured_feature::ConfiguredFeature::NoOp)`。
fn value_to_feature_ref(v: &Value) -> TokenStream {
    match v {
        Value::String(s) => {
            let name = s.strip_prefix("minecraft:").unwrap_or(s);
            let variant_name = format_ident!("{}", name.to_pascal_case());
            quote! { Feature::Named(papokin_data::configured_feature::ConfiguredFeature::#variant_name) }
        }
        Value::Object(_) => {
            let cf = value_to_configured_feature(v);
            quote! { Feature::Inlined(Box::new(#cf)) }
        }
        _ => quote! { Feature::Named(papokin_data::configured_feature::ConfiguredFeature::NoOp) },
    }
}

// configured_features 上下文中的放置修改器（与 placed_feature 相同，但重新导入）
/// 在已配置地物上下文中，将放置修饰器 JSON 对象转换为其 `PlacementModifier` token 流。
///
/// # Arguments
/// – `v` – 单个修改器条目的 JSON 对象，应包含 `"type"` 字段。
///
/// # Returns
/// 一个对应相应 `PlacementModifier` 变体的 `TokenStream`；类型无法识别时默认为 `PlacementModifier::Biome`。
