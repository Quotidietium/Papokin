...
fn load_placed_features() -> BTreeMap<String, Value> {
    ...
    let mut entries: Vec<_> = fs::read_dir(dir)
        .expect("缺少 worldgen/placed_feature 目录")
        .flatten()
    ...
    for entry in entries {
        ...
        let content = fs::read_to_string(&path).expect("读取 placed_feature 文件失败");
        let val: Value = serde_json::from_str(&content).expect("解析 placed_feature JSON 失败");
        map.insert(stem, val);
    }
    map
}

/// 从 26.3 数据包读取 placed_feature 文件，并生成完整的 `PlacedFeature` 枚举 `TokenStream`。
...
/// 从 26.3 数据包读取 placed_feature 文件，并生成完整的 `build_placed_features()` 函数 `TokenStream`。
pub fn build() -> TokenStream {
    ...
    let entries: Vec<TokenStream> = json
        ...
        .map(|(name, value)| {
            ...
            quote! {
                map.insert(papokin_data::placed_feature::PlacedFeature::#variant_name, #pf);
            }
        })
        .collect();
    ...
    quote! {
        ...
        fn build_placed_features() -> std::collections::HashMap<papokin_data::placed_feature::PlacedFeature, PlacedFeature> {
...
/// 将单个已放置地物 JSON 值转换为 `PlacedFeature` token 流。
///
/// # Arguments
/// – `v` – 该条目的 JSON 对象，应包含 `"feature"` 和 `"placement"` 字段。
...
/// 将 JSON `"feature"` 值转换为 `Feature` token 流。
///
/// # Arguments
/// - `v` – 要么为字符串（命名引用），要么为对象（内联配置的地物）。
///
/// # Returns
/// 字符串值用 `Feature::Named(…)`，对象值用 `Feature::Inlined(…)`。
...
/// 将单个放置修饰器 JSON 对象转换为其对应的 `PlacementModifier` token 流。
///
/// # Arguments
/// - `v` – 带 `"type"` 判别字段及修饰器专有字段的 JSON 对象。
///
/// # Returns
/// 一个 `PlacementModifier` 变体的 token 流，类型未知时生成 `compile_error!`。
...
/// 将方块谓词 JSON 对象转换为其对应的 `BlockPredicate` token 流。
///
/// # Arguments
/// - `v` – 带 `"type"` 判别字段及谓词专有字段的 JSON 对象。
///
/// # Returns
/// 一个 `BlockPredicate` 变体的 token 流，类型未知时生成 `compile_error!`。
pub fn value_to_block_predicate(v: &Value) -> TokenStream {
    // 处理裸字符串值："#minecraft:some_tag" 是方块标签谓词，
    // "true"（或任何非 # 字符串）即 AlwaysTrue。
        ...
        if let Some(tag) = s.strip_prefix('#') {
            ...
            return quote! {
                BlockPredicate::MatchingBlockTag(MatchingBlockTagPredicate {
                    ...
                    tag: papokin_data::tag::Block::#tag_ident,
                })
            };
        }
...
/// 将高度提供者 JSON 对象转换为其对应的 `HeightProvider` token 流。
///
/// # Arguments
/// - `v` – 带 `"type"` 判别字段及提供者专有字段的 JSON 对象。
///
/// # Returns
/// 一个 `HeightProvider` 变体的 token 流，类型未知时生成 `compile_error!`。
pub fn value_to_height_provider(v: &Value) -> TokenStream {
    ...
    match type_str {
        ...
        "minecraft:constant" => {
            // 恒定高度提供器包装一个 y_offset
            let y = value_to_y_offset(&v["value"]);
            // 映射到具有相同 min/max 的均匀分布
            quote! {
                HeightProvider::Uniform(UniformHeightProvider {
                    min_inclusive: #y,
                    max_inclusive: #y,
...
/// 将 Y 偏移 JSON 对象转换为其对应的 `YOffset` token 流。
///
/// # Arguments
/// - `v` – 包含 `"absolute"`、`"above_bottom"` 或 `"below_top"` 之一键名的 JSON 对象。
///
/// # Returns
/// 一个 `YOffset` 变体的 token 流；无法识别时默认为 `YOffset::Absolute(0)`。
...
/// 将整数提供者 JSON 值转换为其对应的 `IntProvider` token 流。
///
/// # Arguments
/// - `v` – 要么为裸 JSON 数字（常量），要么为带 `"type"` 判别字段的对象。
///
/// # Returns
/// `IntProvider` 变体的 token 流；无法识别时默认为 `IntProvider::Constant(0)`。
    ...
    match v {
        ...
        Value::Object(_) => {
            ...
            match type_str {
                ...
                _ => {
                    panic!("未知的 Int Provider，似乎 Mojang 添加了新类型：{:?}", v)
                }
            }
        }
        _ => panic!("未知的 Int Provider，似乎 Mojang 添加了新类型：{:?}", v),
    }
}

/// 将 SCREAMING_SNAKE_CASE 高度图名称转换为其 `HeightMap` token 流。
///
/// # Arguments
/// - `s` – 高度图名称字符串（例如 `"MOTION_BLOCKING"`）。
///
/// # Returns
/// 一个 `HeightMap` 变体的 token 流；无法识别时默认为 `HeightMap::MotionBlocking`。
...
/// 将小写方向字符串转换为其 `BlockDirection` token 流。
///
/// # Arguments
/// - `s` – 方向名称（例如 `"down"`、`"up"`、`"north"` 等）。
///
/// # Returns
/// 一个 `BlockDirection` 变体的 token 流；无法识别时默认为 `BlockDirection::Down`。
...
/// 将可选的偏移 JSON 值转换为 `OffsetBlocksBlockPredicate` token 流。
///
/// # Arguments
/// - `v` – 要么为 `null`/空对象（无偏移），要么为 3 元素整数数组 `[x, y, z]`。
///
/// # Returns
/// 一个 `OffsetBlocksBlockPredicate`，带 `offset: None` 或 `offset: Some(Vector3::new(x, y, z))`。
...
/// 将方块/流体 JSON 值转换为 `MatchingBlocksWrapper` token 流。
///
/// # Arguments
/// - `v` – 要么为单个字符串（一个方块/流体名称），要么为字符串数组。
///
/// # Returns
/// `MatchingBlocksWrapper::Single(…)` 或 `MatchingBlocksWrapper::Multiple(…)`。
...
/// 将方块状态 JSON 对象转换为 `BlockStateCodec` token 流。
///
/// # Arguments
/// - `v` – 带 `"Name"` 字段和可选 `"Properties"` 映射的 JSON 对象。
///
/// # Returns
/// 一个 `BlockStateCodec` token 流，引用对应的 `Block` 常量和可选的属性映射。
        ...
        quote! {
            {
                ...
                BlockStateCodec {
                    name: &papokin_data::Block::#block_ident,
...
pub fn value_to_block_state(v: &Value) -> TokenStream {
    ...
    if let Some(props) = props {
        ...
        quote! {
            papokin_data::Block::#block_ident.default_state
        }
    }
}

/// 将可为空的 JSON 整数转换为 `Option<i32>` token 流。
///
/// # Arguments
/// - `v` – JSON 数字或 `null`。
///
/// # Returns
/// `v` 为有效整数时返回 `Some(n)`，否则返回 `None`。
