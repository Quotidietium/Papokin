...
/// 包装 `f32`，提供按位精确的 `Hash` 实现，以便用作映射键。
#[derive(Clone, Copy)]
    ...
    fn emit_compiled_eval_fn(
        ...
        match self {
            Self::Constant { value } => {
                ...
                quote! {
                    #[inline(always)]
                    pub fn #fn_name<C: NoiseEvaluationContext>(pos: &papokin_util::math::vector3::Vector3<i32>, ctx: &mut C) -> f32 {
                        let _ = (pos, ctx);
                        #val
                    }
                }
            ...
            Self::Slice {
                ...
                let slice_pos = match axis {
                    Axis::X => {
                        quote! { papokin_util::math::vector3::Vector3::new(#coordinate, pos.y, pos.z) }
                    }
                    Axis::Y => {
                        quote! { papokin_util::math::vector3::Vector3::new(pos.x, #coordinate, pos.z) }
                    }
                    Axis::Z => {
                        quote! { papokin_util::math::vector3::Vector3::new(pos.x, pos.y, #coordinate) }
                    }
                };
                quote! {
                    #[inline(always)]
...
/// 从 JSON 反序列化得到的所有维度噪声路由器表示的顶层容器。
#[derive(Deserialize)]
...
fn load_df_json(base_df_dir: &std::path::Path, name: &str) -> serde_json::Value {
    ...
    let content = fs::read_to_string(&path)
        .unwrap_or_else(|err| panic!("读取 {} 处的密度函数失败：{err}", path.display()));
    serde_json::from_str(&content)
        .unwrap_or_else(|err| panic!("解析 {} 处的密度函数失败：{err}", path.display()))
}
    ...
    match val {
        ...
        serde_json::Value::Object(obj) => {
            let type_str = obj
                .get("type")
                .and_then(|t| t.as_str())
                .unwrap_or_else(|| panic!("密度函数对象缺少 type 字段：{val:?}"));
            ...
            match clean_type {
                ...
                "gradient" => {
                    ...
                    let from_coordinate =
                        obj.get("from_coordinate")
                            ...
                            .and_then(|v| v.as_i64())
                            .expect("缺少 from_coordinate") as i32;
                    let to_coordinate =
                        obj.get("to_coordinate")
                            .or_else(|| obj.get("to_y"))
                            .and_then(|v| v.as_i64())
                            .expect("缺少 to_coordinate") as i32;
                    let from_value = obj
                        .get("from_value")
                        .and_then(|v| v.as_f64())
                        .expect("缺少 from_value");
                    let to_value = obj
                        .get("to_value")
                        .and_then(|v| v.as_f64())
                        .expect("缺少 to_value");
                ...
                "distance_to_point" => {
                    let point_arr = obj.get("point").and_then(|v| v.as_array()).expect("缺少点");
                    let point = [
                ...
                "lerp" => {
                    let alpha =
                        parse_vanilla_df(base_df_dir, obj.get("alpha").expect("缺少 alpha"));
                    let first =
                        parse_vanilla_df(base_df_dir, obj.get("first").expect("缺少 first"));
                    let second =
                        parse_vanilla_df(base_df_dir, obj.get("second").expect("缺少 second"));
                ...
                "slice" => {
                    ...
                    let coordinate = obj
                        ...
                        .and_then(|v| v.as_i64())
                        .expect("缺少坐标") as i32;
                    let input = parse_vanilla_df(base_df_dir, obj.get("input").expect("缺少输入"));
                ...
                "add" | "mul" | "min" | "max" | "sub" | "div" | "pow" => {
                    let arg1_node = obj
                        ...
                        .expect("缺少 left/argument1/base");
                    let arg2_node = obj
                        .get("right")
                        .or_else(|| obj.get("argument2"))
                        .or_else(|| obj.get("exponent"))
                        .expect("缺少 right/argument2/exponent");
                ...
                "range_choice" => {
                    ...
                    let when_in = parse_vanilla_df(
                        base_df_dir,
                        obj.get("when_in_range").expect("缺少 when_in_range"),
                    );
                    let when_out = parse_vanilla_df(
                        base_df_dir,
                        obj.get("when_out_of_range")
                            .expect("缺少 when_out_of_range"),
                    );
                ...
                "spline" => {
                    let spline_node = obj.get("spline").expect("缺少样条");
                ...
                "shifted_noise" => {
                    let shift_x =
                        parse_vanilla_df(base_df_dir, obj.get("shift_x").expect("缺少 shift_x"));
                    let shift_y =
                        parse_vanilla_df(base_df_dir, obj.get("shift_y").expect("缺少 shift_y"));
                    let shift_z =
                        parse_vanilla_df(base_df_dir, obj.get("shift_z").expect("缺少 shift_z"));
                    let noise_name = obj
                        .get("noise")
                        .and_then(|v| v.as_str())
                        .expect("缺少噪声名称");
                    ...
                    let wrapper = if clean_type == "interpolated" {
                        let cell_size_xz =
                            obj.get("cell_size_xz")
                                .and_then(|v| v.as_i64())
                                .expect("缺少 cell_size_xz") as i32;
                        let cell_size_y =
                            obj.get("cell_size_y")
                                .and_then(|v| v.as_i64())
                                .expect("缺少 cell_size_y") as i32;
                        WrapperType::Interpolated {
                            cell_size_xz,
                            cell_size_y,
                        }
                ...
                "find_top_surface" | "weird_utility_density" => {
                    let density =
                        parse_vanilla_df(base_df_dir, obj.get("density").expect("缺少密度"));
                    let upper_bound = parse_vanilla_df(
                        base_df_dir,
                        obj.get("upper_bound").expect("缺少 upper_bound"),
                    );
                    let lower_bound = obj
                        .get("lower_bound")
                        .and_then(|v| v.as_i64())
                ...
                other => panic!("数据包中未知的密度函数类型：{other}"),
            }
        }
        _ => panic!("密度函数中不支持的 JSON 值：{val:?}"),
    }
}
...
fn parse_vanilla_spline(base_df_dir: &std::path::Path, val: &serde_json::Value) -> SplineRepr {
    ...
    if let Some(obj) = val.as_object() {
        ...
        let loc_fn = parse_vanilla_df(base_df_dir, obj.get("coordinate").expect("样条中缺少坐标"));
        let points = obj
            .get("points")
            .and_then(|v| v.as_array())
            .expect("样条中缺少 points 数组");
...
fn load_vanilla_noise_router(
    ...
    let content = fs::read_to_string(&path)
        .unwrap_or_else(|err| panic!("读取 {} 处的 noise_settings 失败：{err}", path.display()));
    let val: serde_json::Value = serde_json::from_str(&content)
        .unwrap_or_else(|err| panic!("解析 {} 处的 noise_settings 失败：{err}", path.display()));
    let nr = val
        .get("noise_router")
        .expect("noise_settings 中缺少 noise_router");
...
/// 读取原版数据包的 noise_settings 与 density_function 文件，并生成完整的噪声路由器常量 `TokenStream`。
pub fn build() -> TokenStream {
    ...
    quote! {
        ...
        pub trait NoiseEvaluationContext {
            ...
            fn sample_shift_a(&mut self, noise_id: DoublePerlinNoiseParameters, pos: &papokin_util::math::vector3::Vector3<i32>) -> f32;
            fn sample_shift_b(&mut self, noise_id: DoublePerlinNoiseParameters, pos: &papokin_util::math::vector3::Vector3<i32>) -> f32;
            ...
            fn sample_interpolated_noise(&mut self, pos: &papokin_util::math::vector3::Vector3<i32>) -> f32;
            fn sample_beardifier(&mut self, pos: &papokin_util::math::vector3::Vector3<i32>) -> f32;
            fn sample_blend_alpha(&mut self, pos: &papokin_util::math::vector3::Vector3<i32>) -> f32;
            fn sample_blend_offset(&mut self, pos: &papokin_util::math::vector3::Vector3<i32>) -> f32;
            fn sample_blend_density(&mut self, input_val: f32, pos: &papokin_util::math::vector3::Vector3<i32>) -> f32;
            fn sample_end_islands(&mut self, pos: &papokin_util::math::vector3::Vector3<i32>) -> f32;
            fn sample_wrapper(&mut self, wrapper_index: usize, wrapper_type: WrapperType, pos: &papokin_util::math::vector3::Vector3<i32>, eval_input: &dyn Fn(&papokin_util::math::vector3::Vector3<i32>, &mut Self) -> f32) -> f32;
            fn sample_spline(&mut self, spline_index: usize, location_value: f32, pos: &papokin_util::math::vector3::Vector3<i32>) -> f32;
            fn sample_find_top_surface(&mut self, density_fn: &dyn Fn(&papokin_util::math::vector3::Vector3<i32>, &mut Self) -> f32, upper_bound_fn: &dyn Fn(&papokin_util::math::vector3::Vector3<i32>, &mut Self) -> f32, lower_bound: i32, cell_height: i32, pos: &papokin_util::math::vector3::Vector3<i32>) -> f32;
        }
