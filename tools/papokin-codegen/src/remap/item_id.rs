...
/// 生成各版本物品 ID 重映射表及
/// `remap_item_id_for_version`/`remap_item_id_from_version` 函数。
pub fn build() -> TokenStream {
    // ViaBackwards 映射由新版本指向旧版本 (26.1 → 1.21.11 → ... → 1.20.5)
    // 对于新服务器向旧客户端发送的情况，这是正确的方向。
...
/// 构建旧 ID → 最新 ID 的映射。多个最新条目可能共用一个旧 ID（新的
/// 物品回退到类似的现有物品）。保留首个写入者：原始
/// 物品的 id 低于后续的回退项，因此 1.21.6 的钻石剑会映射
/// 变回钻石剑而不是钻石长矛。
...
mod tests {
    ...
    fn reverse_mapping_prefers_original_item_over_later_fallback() {
        // 最新的 id：2 = 原始剑，4 = 被 ViaBackwards 映射为相同旧 id 的长矛。
