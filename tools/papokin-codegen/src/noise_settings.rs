...
/// 来自 `worldgen/noise_settings` 的原始反序列化噪声设置文件。
#[derive(Deserialize)]
...
/// 从数据包读取 noise_settings 文件，并生成 NoiseSettings 常量。
pub fn build() -> TokenStream {
    ...
    let mut entries: Vec<_> = fs::read_dir(noise_settings_dir)
        .expect("缺少 worldgen/noise_settings 目录")
        .flatten()
    ...
    for entry in entries {
        ...
        let content = fs::read_to_string(&path).expect("读取 noise_settings 文件失败");
        let noise_settings: NoiseSettingsFileStruct =
            serde_json::from_str(&content).expect("解析 noise_settings JSON 失败");
