...
use papokin_util::loot_table::{LootBonusFormula, LootCondition};
use proc_macro2::{Span, TokenStream};
use quote::{format_ident, quote};
use serde::Deserialize;
use syn::LitStr;

/// `rolls` 可以是裸浮点数，也可以是带有 `type/min/max` 的对象。
...
/// 条件可以是内联定义，也可以是对 `predicate` 文件的引用。
#[derive(Deserialize, Clone, Debug)]
#[serde(untagged)]
enum ConditionValue {
    Reference(String),
...
where
    ...
    T: Deserialize<'de>,
{
    // 先尝试 `Many`，因为 `serde_json::Value` 也能匹配 `One`。
    #[derive(Deserialize)]
    #[serde(untagged)]
    enum OneOrMany<T> {
        Many(Vec<T>),
...
/// （递归地）读取 `../../assets/datapacks/26_3/data/minecraft/loot_table/` 下的每个战利品 JSON
/// 并生成包含静态常量的 `papokin-data/src/generated/chest_loot.rs`
/// 以及一个 `get_chest_loot_table(key) -> Option<&'static ChestLootTable>` 函数。
pub fn build() -> TokenStream {
    let base = Path::new("../../assets/datapacks/26_3/data/minecraft/loot_table");

    // 递归收集所有 JSON 文件并排序，保证输出确定性。
    ...
    // 生成 get_loot_table 和 get_chest_loot_table
    all_tokens.extend(quote! {
        #[must_use]
    ...
    quote! {
        pub use papokin_util::loot_table::*;
        #all_tokens
    }
}
