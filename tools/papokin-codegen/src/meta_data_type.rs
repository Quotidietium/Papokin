...
/// 生成带有各版本 ID 字段和常量的 `MetaDataType` 结构体的 `TokenStream`。
pub fn build() -> TokenStream {
    ...
    for &(ver, file) in &assets {
        ...
        let parsed: BTreeMap<String, i32> =
            serde_json::from_str(&fs::read_to_string(&path).unwrap())
                .unwrap_or_else(|_| panic!("解析 {file} 失败"));
    ...
    quote! {
        ...
        impl MetaDataType {
            ...
            pub const fn id(&self, version: papokin_util::version::JavaMinecraftVersion) -> i32 {
                match version {
                    #to_id_arms
                    _ => -1i32,
                }
