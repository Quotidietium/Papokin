...
fn make_handshake_packet(next_state: i32) -> Vec<u8> {
    ...
    encode_var_int(769, &mut payload); // 协议版本 769（1.21.4）
...
async fn check_server_alive(addr: SocketAddr) -> bool {
    ...
    let handshake = make_handshake_packet(1); // 状态状态
...
async fn main() {
    ...
    println!(
        "{}",
        "=== Pumpkin 服务器网络数据包 Fuzzer ===".cyan().bold()
    );
    ...
    if check_server_alive(target_addr).await {
        ...
        println!("{}", "无响应 / 离线（将继续测试）".yellow().bold());
    }
    ...
    while start.elapsed() < duration {
        ...
        println!(
            "[{:3}s] 连接： {:5} | 数据包： {:6} | 数据： {:7.1} KB ({:6.1} KB/s) | 关闭： {:4} | 重置/超时： {:4}",
            elapsed,
            conns,
            pkts,
            bytes as f64 / 1024.0,
    ...
    println!("  总连接数：   {}", conns);
    println!("  总发送数据包数：  {}", pkts);
    println!("  总发送数据量：     {:.2} KB", bytes as f64 / 1024.0);
    println!("  服务器正常断开： {}", closes);
    println!("  连接重置：   {}", errs);

    print!("\nVerifying server health post-fuzzing... ");
    if check_server_alive(target_addr).await {
        println!("{}", "成功：服务器存活且响应正常！".green().bold());
    } else {
