...
mod tests {
    ...
    fn rename_item() {
        ...
        assert_eq!(result.get_repair_cost(), 0); // 只有重命名不会增加修复费用
    }
    ...
    fn material_repair() {
        ...
        // 250 / 4 = 每个锭 62。125 点耐久损耗需要 3 个锭（62 + 62 + 1）。
    ...
    fn tool_combine_durability() {
        ...
        let result = anvil_inv.get_stack(2);
        // 耐久度合并：(max - 500) + (max - 500) + max * 12 / 100
    ...
    fn enchanted_book_combine() {
        ...
        // 锋利铁砧费用 = 1。使用书的费用 = max(1, 1/2) = 1。价格 = 1 * 3 = 3。
        assert_eq!(handler.get_cost(), 3);
        let result = anvil_inv.get_stack(2);
    ...
    fn too_expensive_limit() {
        ...
        // 罚费为 35 + 35 = 70。合并已损耗的工具再加 2。总计 = 72 >= 40。
