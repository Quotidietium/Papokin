...
/// 在交易完成前调用的交易门控。
///
/// 接收玩家、本次交易的报价以及商人实体 ID（如有
/// 已知）。返回 `Some(villager_experience)` 以允许该交易（该值
/// 替换该交易的村民经验）或返回 `None` 以否决。
pub type MerchantTradeCheck = Box<
    dyn Fn(
            &dyn InventoryPlayer,
            &papokin_protocol::java::client::play::MerchantOffer,
            Option<i32>,
        ) -> Option<i32>
        + Send
        + Sync,
...
pub struct MerchantScreenHandler {
    ...
    pub offers: Vec<papokin_protocol::java::client::play::MerchantOffer>,
    ...
    /// 在交易完成前触发的可选门控事件（插件事件钩子）。
    pub trade_check: Option<MerchantTradeCheck>,
    /// 此界面所属交易者的实体 ID（若该界面由实体支撑）。
...
impl MerchantScreenHandler {
    pub fn new(
        ...
        offers: Vec<papokin_protocol::java::client::play::MerchantOffer>,
    ) -> Self {
    ...
    fn adjusted_cost_a(offer: &papokin_protocol::java::client::play::MerchantOffer) -> ItemStack {
    ...
    fn satisfies(
        offer: &papokin_protocol::java::client::play::MerchantOffer,
        input_a: &ItemStack,
        input_b: &ItemStack,
    ) -> bool {
    ...
    fn complete_trade(&mut self, player: &dyn InventoryPlayer) -> bool {
        ...
        // 交易门控（插件钩子）：否决会在任何后续处理之前取消交易，
        // 报酬会被消耗。允许的检查可以替换村民经验值。
...
impl ScreenHandler for MerchantScreenHandler {
    ...
    fn on_slot_click(
        ...
        slot_index: i32,
        button: i32,
        action_type: papokin_protocol::java::server::play::SlotActionType,
...
mod tests {
    ...
    use papokin_data::{data_component_impl::EquipmentSlot, item::Item};
    ...
    fn bookshelf_offer() -> papokin_protocol::java::client::play::MerchantOffer {
    ...
    fn numeric_hotbar_key_swaps_occupied_stacks() {
        ...
        // 商人界面槽位 30-38 映射到快捷栏槽位 0-8。首先移动
        // 把剑从快捷栏槽位 1 换到空的快捷栏槽位 2。
