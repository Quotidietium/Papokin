...
/// 三维单纯形噪声采样器实现。
///
/// 单纯形噪声（Simplex noise）是 Ken Perlin 开发的一种改进型梯度噪声函数，作为
/// 经典 Perlin 噪声的替代方案。它有几个优点：
/// - 更低的计算复杂度（O(n²) 对比 O(2ⁿ)）。
/// - 无方向性伪影（各向同性更好）。
/// - 梯度定义良好且连续。
/// - 对于更高维度更容易实现。
///
/// 此实现同时提供 2D 和 3D 单纯形噪声采样。
#[derive(Clone)]
pub struct SimplexNoiseSampler {
    /// 用于哈希坐标的置换表（大小 256，复制一份以便于回绕）。
    permutation: [u8; 256],
    /// X 坐标原点偏移（经随机化以避免对称）。
    x_origin: f64,
    /// Y 坐标原点偏移（经随机化以避免对称）。
...
impl SimplexNoiseSampler {
    ...
    /// 2D 单纯形变换的偏斜因子（将方形网格映射到单纯形网格）。
    const SKEW_FACTOR_2D: f64 = 0.5f64 * (Self::SQRT_3 - 1f64);
    /// 二维单纯形变换的反偏斜因子（将单纯形网格映射回方形网格）。
    const UNSKEW_FACTOR_2D: f64 = (3f64 - Self::SQRT_3) / 6f64;

    /// 创建使用随机化原点和置换表的单纯形噪声采样器。
    ///
    /// # Arguments
    /// - `random` – 用于初始化的随机数生成器。
    ///
    /// # Returns
    /// 一个新的 `SimplexNoiseSampler` 实例。
    ...
    /// 计算单个单纯形角点的贡献。
    ///
    /// 此函数应用单纯形噪声衰减核：(r² - |dist|²)⁴ · gradient。
    ///
    /// # Arguments
    /// - `gradient_index` – 此角点在梯度数组中的索引。
    /// - `x` – 相对角落的 X 偏移量。
    /// - `y` – 相对角落的 Y 偏移量。
    /// - `z` – 相对角落的 Z 偏移量。
    /// - `distance` – 核半径（2D 通常为 0.5，3D 通常为 0.6）。
    ///
    /// # Returns
    /// 该角点对最终噪声值的贡献。
...
/// 一个多八度单纯形噪声采样器，组合了多个噪声层。
///
/// 八度噪声（又称分形噪声）组合了多个八度的 Simplex 噪声
/// 以不同的频率和振幅叠加，创造出更复杂、更自然的图案。
/// 每个八度都为整体噪声增添更精细的细节。
pub struct OctaveSimplexNoiseSampler {
    /// 每个八度的采样器列表（未使用的八度为 None）。
...
impl OctaveSimplexNoiseSampler {
    /// 创建一个新的八度 Simplex 噪声采样器。
    ///
    /// 此构造函数遵循 Minecraft 的旧式初始化模式，组合使用
    /// Xoroshiro 和 `LegacyRand` 的实现，以确保与现有世界生成兼容。
    ///
    /// # Arguments
    /// - `random` – 要使用的随机数生成器。
    /// - `octaves` – 要包含的八度索引列表（可为负）。
    ///
    /// # Returns
    /// 一个新的 `OctaveSimplexNoiseSampler` 实例。
    #[expect(clippy::many_single_char_names)]
    ...
    /// 在给定坐标处采样 2D 多倍频 Simplex 噪声。
    ///
    /// 此方法合并所有活动的倍频程，并将其贡献与……相加
    /// 基于持续性（persistence）和间隙度（lacunarity）的适当缩放。
    ///
    /// # Arguments
    /// - `x` – X 坐标。
    /// - `y` – Y 坐标。
    /// - `use_origin` – 是否包含各八度的原点偏移量。
    ///
    /// # Returns
    /// 所有八度叠加得到的噪声值。
    #[must_use]
