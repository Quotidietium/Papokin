...
/// 与旧版 Minecraft 兼容的旧版随机数生成器。
///
/// 此实现使用 48 位线性同余生成器（LCG），公式如下：
/// `seed = (seed * 0x5DEECE66D + 11) & 0xFFFFFFFFFFFF`
///
/// 它保持了与 Minecraft 在……中使用的原始随机数生成器的兼容性
/// 1.13 之前的版本。这对世界生成的兼容性至关重要
/// 旧世界，以及某些仍依赖旧算法的特性。
pub struct LegacyRand {
    /// 当前的 48 位种子值（存储在低 48 位中）。
    seed: u64,
    /// 用于下一次高斯生成所存储的高斯值。
...
impl LegacyRand {
    ...
    /// 根据给定的种子创建一个新的旧版随机数生成器。
    ///
    /// 种子与常量 `0x5DEECE66D` 进行异或，并掩码为 48 位。
    ///
    /// # Arguments
    /// - `seed` – 初始种子值。
    ///
    ...
    /// 生成下一个随机值并推进内部状态。
    ///
    /// 这实现了核心 LCG 算法：`seed = seed * 0x5DEECE66D + 11`
    /// 并返回新种子。
    ///
    /// # Returns
    /// 新的 48 位种子值。
...
impl RandomImpl for LegacyRand {
    fn skip(&mut self, count: i32) {
        // 使用 `next_i32()` 精确推进 `count` 个单独的 LCG 步。
        // `next_i64()` 会消耗两次 LCG 步骤，因此我们覆盖了默认值。
        for _ in 0..count {
            self.next_i32();
        }
    }
    ...
    fn split() {
        ...
        let mut original_rand = LegacyRand::from_seed(0);
        {
            if let RandomDeriver::Legacy(splitter) = original_rand.next_splitter() {
                ...
                panic!("预期为 RandomDeriver::Legacy");
            }
        }
