...
fn license_checker_offline_and_grace_period() {
    ...
    let lease = LicenseLease {
        ...
        last_verified_timestamp: now - 86400 * 2, // 2 天前
        expires_timestamp: now - 3600,            // 已于 1 小时前过期
    };
    ...
    match status {
        ...
        other => panic!("预期为 GracePeriod，实际为 {other:?}"),
    }
    ...
    match status {
        ...
        other => panic!("预期为 Valid，实际为 {other:?}"),
    }
}
...
fn global_init_and_metadata_access() {
    ...
    let original_meta = PapokinMetadata {
        marketplace_url: "http://127.0.0.1:0".to_string(),
        plugin_id: 777,
        plugin_name: "economy-core".to_string(),
        version: "3.2.1".to_string(),
        dev_id: 20,
        dev_name: "papokin-dev".to_string(),
        is_paid: true,
        user_id: 8888,
    ...
    // 1. 随元数据全局初始化
    let verified = init_with_metadata(original_meta, data_folder).expect("初始化应成功");
    ...
    assert_eq!(metadata().unwrap().dev_name, "papokin-dev");
