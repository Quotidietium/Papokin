...
use papokin_codecs::{DynamicOps, Number};
...
pub const INVALID_CODEPOINT: CommandErrorType<1> =
    CommandErrorType::new(translation::java::SNBT_PARSER_INVALID_CODEPOINT);

pub const NO_SUCH_OPERATION: CommandErrorType<1> =
    CommandErrorType::new(translation::java::SNBT_PARSER_NO_SUCH_OPERATION);

pub const EXPECTED_FLOAT_TYPE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::SNBT_PARSER_EXPECTED_FLOAT_TYPE);

pub const INVALID_CHARACTER_NAME: CommandErrorType<0> =
    CommandErrorType::new(translation::java::SNBT_PARSER_INVALID_CHARACTER_NAME);

pub const INVALID_UNQUOTED_START: CommandErrorType<0> =
    CommandErrorType::new(translation::java::SNBT_PARSER_INVALID_UNQUOTED_START);

pub const EXPECTED_UNQUOTED_STRING: CommandErrorType<0> =
    CommandErrorType::new(translation::java::SNBT_PARSER_EXPECTED_UNQUOTED_STRING);

pub const INVALID_STRING_CONTENTS: CommandErrorType<0> =
    CommandErrorType::new(translation::java::SNBT_PARSER_INVALID_STRING_CONTENTS);

pub const EXPECTED_BINARY_NUMERAL: CommandErrorType<0> =
    CommandErrorType::new(translation::java::SNBT_PARSER_EXPECTED_BINARY_NUMERAL);

pub const EXPECTED_DECIMAL_NUMERAL: CommandErrorType<0> =
    CommandErrorType::new(translation::java::SNBT_PARSER_EXPECTED_DECIMAL_NUMERAL);
...
pub const LEADING_ZERO_NOT_ALLOWED: CommandErrorType<0> =
    CommandErrorType::new(translation::java::SNBT_PARSER_LEADING_ZERO_NOT_ALLOWED);

pub const INFINITY_NOT_ALLOWED: CommandErrorType<0> =
    CommandErrorType::new(translation::java::SNBT_PARSER_INFINITY_NOT_ALLOWED);
...
impl SnbtParser<'_, '_> {
    ...
    fn float_literal(&mut self) -> Option<NbtTag> {
        ...
        //     如果遇到小数点，则必须选择路径 A。
        //     尝试解析 [eZZZ] 和 [suffix]：
        //         若 [eZZZ] 能解析，则无论 [suffix] 如何，都选择路径 D。
        //         若只有 [suffix] 能解析，则选择路径 C。
        //         若都无法解析，则失败。
        // 如果遇到小数点，必须选择路径 B。
        // 若既不是句点也不是数字则判定失败
