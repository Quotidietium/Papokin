use papokin_nbt::{compound::NbtCompound, nbt_compound_tag, tag::NbtTag};
...
macro_rules! assert_parse_ok {
    ($snbt:expr, $tag:expr) => {
        ...
        match SnbtParser::parse_for_commands(&mut reader) {
            Err(error) => {
                panic!("预期解析成功，但却得到错误：{error:?}")
            }
            Ok(tag_parsed) => {
                assert_eq!(tag_parsed, $tag, "解析出的标签与预期不符");
                assert!(
...
macro_rules! assert_parse_err {
    ($snbt:expr, $error_message:expr, $cursor:expr) => {
        ...
        match parsed {
            Ok(tag) => panic!("预期得到命令错误，但却得到结果：{tag:#?}"),
            Err(error) => {
                assert_eq!(error.message.get_text(), $error_message, "错误消息不匹配");
                // SNBT 解析中应当始终存在上下文。
                assert_eq!(
                    error.context.expect("SNBT 解析中应当始终有上下文").cursor,
                    $cursor,
                    "错误的光标位置不匹配"
                );
            }
