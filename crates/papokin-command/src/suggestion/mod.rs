...
/// 一种描述建议文本的结构。
/// 其实际值可以是 [`String`]，也可以是 [`i32`]。
///
/// 使用 [`new`] 方法创建新的 [`SuggestionText`]。
///
/// 如果想用 `i32` 作为建议文本，
/// 使用 [`SuggestionText::Integer`]。在所有其他情况下，
/// 使用 [`SuggestionText::Text`]。
///
/// # Invariant
/// 一个 [`SuggestionText::Text`] **不应存在**，如果它可以
/// 完整表示为 [`SuggestionText::Integer`]（没有前导零）。
/// 这对于建立正确的顺序很重要。
/// ```
/// use papokin_command::suggestion::SuggestionText;
///
...
pub enum SuggestionText {
    ...
    /// 用于整数建议的选项。存储一个 [`i32`]。
    /// 注意，此值内部存储了一个已缓存的 [`String`]
    /// 使比较该对象时不会发生 [`String`] 分配。
...
impl SuggestionText {
    ...
    /// 根据可用值创建新的 [`SuggestionText`]。
    /// 此值可以是 `&str`、[`String`] 或 `i32`。
...
impl Suggestion {
    ...
    /// 由范围、文本构造 [`Suggestion`]（文本可以是
    /// [`String`] 或 [`i32`]），以及一个提示文本组件。
    #[must_use]
    ...
    /// 由范围、文本构造 [`Suggestion`]（文本可以是
    /// [`String`] 或 [`i32`]），以及一个 [`TextComponent`] 的 [`Option`]。
    #[must_use]
    ...
    /// 使用 [`StringRange`] 将此 [`Suggestion`] 展开到命令上，
    /// 返回一个新的 [`Suggestion`]。
    #[must_use]
