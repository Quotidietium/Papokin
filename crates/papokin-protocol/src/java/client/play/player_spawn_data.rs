...
/// 写入当前与先前的游戏模式。自 26.3 起两者均为 var int，且先前的游戏
/// mode 部分可选，0 表示无，其他任意值为游戏模式 id 加一。
...
/// 读取由 [`write_game_modes`] 写入的当前与上一个游戏模式。
...
pub struct PlayerSpawnData {
    /// 当前维度的属性（光照、天空颜色）对应的维度信息。
    pub dimension: Dimension,
    /// 客户端用它来为本地生物群系噪声与装饰算法提供种子。
    pub hashed_seed: i64,
    pub game_mode: u8,
    /// 之前的游戏模式（用于 F3+F4 切换界面）。无则为 -1。
    pub previous_gamemode: i8,
    /// 如果为 true，该世界为调试世界（所有方块按网格显示）。
    pub debug: bool,
    /// 如果为 true，该世界为超平坦世界（影响地平线渲染）。
    pub is_flat: bool,
    /// 玩家上次死亡的位置（1.19 加入，用于恢复指南针）。
    pub death_dimension_name: Option<(ResourceLocation, BlockPos)>,
    /// 于 1.20 加入。
    pub portal_cooldown: VarInt,
    /// 海平面高度，通常为 63（1.21.2 新增）。
    pub sealevel: VarInt,
}
