use papokin_data::packet::clientbound::play::SECTION_BLOCKS_UPDATE;
use papokin_data::{BlockStateId, block_state_remap::remap_block_state_for_version};
use papokin_macros::java_packet;
use papokin_util::math::position::{BlockPos, chunk_section_from_pos, pack_local_chunk_section};
use papokin_util::math::vector3::{self, Vector3};
...
/// 更新单个区块段内的多个方块（旧版本中为区块）。
///
/// 此数据包比发送多个单独的包高效得多。
/// `CBlockUpdate` 数据包，当同一区域发生大量变化时
/// (例如爆炸、结构生成或大规模地形改造)。
...
pub struct CMultiBlockUpdate {
    ...
    /// 抑制光照更新（用于 1.16..=1.19.4）
    pub suppress_light_updates: bool,
    /// 方块更新数组：(`BlockPos`, `BlockStateId`)
