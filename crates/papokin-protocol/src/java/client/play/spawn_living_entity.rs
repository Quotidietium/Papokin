...
use papokin_data::packet::clientbound::play::SPAWN_LIVING_ENTITY;
...
pub fn remap_living_mob_type_for_version(entity_id: u16, version: JavaMinecraftVersion) -> u16 {
    ...
    // 1.7.10 - 1.10（以及 1.11 - 1.12 共享的遗留基础 ID）
    ...
    fn write_packet_data(
        ...
        if !v1_15 {
                ...
                && metadata.last().copied() == Some(127)
            {
                ...
                // 在 <= 1.8 中，为 DataWatcher 列表写入 127 (0x7F) 终止字节，
                // 与原版和 PacketEvents 保持一致。
                write.write_u8(127)?;
            }
        }
