...
use papokin_data::packet::clientbound::play::COMMANDS;
...
/// 将整个命令树发送给客户端，用于客户端解析和 Tab 补全。
///
/// Minecraft 使用“Brigadier”命令系统。此数据包（网络）告知客户端，
/// 存在哪些命令、它们的参数以及如何分支，从而让
/// 客户端在命令尚未发出时就能以红色标示语法错误。
#[java_packet(COMMANDS)]
pub struct CCommands<'a> {
    /// 命令图中所有节点的扁平列表。
    /// 节点通过它们在此数组中的索引相互引用。
    pub nodes: Box<[ProtoNode<'a>]>,
    /// `nodes` 数组中 "root" 节点的索引。
    /// 这是所有命令的入口点（'/' 符号）。
...
impl ArgumentType {
    ...
    pub fn to_id(&self, version: &JavaMinecraftVersion) -> i32 {
        // SAFETY: 由于 Self 是 repr(u32)，保证判别值位于前 4 字节中
        ...
        papokin_data::argument_type_id_remap::remap_argument_type_id_for_version(id, *version)
            as i32
    }
