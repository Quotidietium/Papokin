use papokin_data::packet::clientbound::play::LOGIN;
use papokin_util::{resource_location::ResourceLocation, version::JavaMinecraftVersion};
...
/// 将客户端从配置状态切换的"加入游戏"数据包
/// 到 Play 状态。
///
/// 这是协议中最大且最重要的数据包之一。它
/// 初始化玩家的世界视图、维度设置和本地游戏
/// 规则。客户端收到后便开始渲染世界。
#[java_packet(LOGIN)]
pub struct CLogin<'a> {
    /// 当前会话中分配给玩家的唯一 ID。
    ...
    /// 服务器上存在的所有维度的列表（1.16 新增）。
    pub dimension_names: &'a [ResourceLocation],
    pub max_players: VarInt,
    /// 客户端在每个方向上渲染的区块数量（1.14 新增）。
    pub view_distance: VarInt,
    /// 处理实体和世界刻的距离（1.18 新增）。
    pub simulated_distance: VarInt,
    /// 如果为 true，在 F3 屏幕上隐藏坐标及其他信息（1.8 加入）。
    ...
    /// 如果为 true，玩家发送未签名聊天消息时客户端会发出警告（1.20.5 加入）。
...
pub fn build_v1_20_registry_codec(
...
) -> papokin_nbt::compound::NbtCompound {
    ...
    let synced = papokin_data::registry::Registry::get_synced(version);
            ...
            if reg_name == "dimension_type" {
                ...
                for entry in &reg.registry_entries {
                    if let Some(ref data) = entry.data {
                        ...
                        if let Ok(element_nbt) = papokin_nbt::Nbt::read_unnamed(&mut reader) {
    ...
    fn write_packet_data(
        ...
        if v1_16 {
            ...
            if !v1_20_2 {
                ...
                let nbt_bytes = papokin_nbt::Nbt::new(String::new(), registry_codec).write();
                write.write_all(&nbt_bytes)?;
                if v1_16_2 && *version < JavaMinecraftVersion::V_1_19 {
                    // 在 1.16.2 - 1.18.2 中，此字段是维度类型 NBT 复合标签！
                    let dim_type_compound =
                        get_dimension_type_nbt(*version, self.spawn_data.dimension.minecraft_name);
                    let dim_bytes = papokin_nbt::Nbt::new(String::new(), dim_type_compound).write();
                    write.write_all(&dim_bytes)?;
                } else {
                    // 在 1.16 - 1.16.1 和 1.19 - 1.20.1 中，此字段为维度 Identifier 字符串
            ...
            if !v1_14 {
                // 难度（0：和平，1：简单，2：普通，3：困难）- 默认 2（普通）
                write.write_u8(2)?;
            }
        }

        // 哈希种子（1.15 加入，1.20.2 移动）
        if v1_15 && !v1_20_2 {
...
mod tests {
    ...
    fn v1_20_login_packet() {
        ...
        assert_eq!(packet_id, 0x28, "1.20 登录数据包 ID 必须是 0x28");
        ...
        let codec = papokin_nbt::Nbt::read(&mut reader).expect("NBT 编解码器");
