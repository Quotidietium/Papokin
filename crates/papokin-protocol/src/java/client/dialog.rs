...
impl<'a> DialogNBT<'a> {
    ...
    #[must_use]
    pub const fn from_nbt(compound: &'a papokin_nbt::compound::NbtCompound) -> Self {
    ...
    pub fn write_packet_data(
        ...
        match &self.0 {
            ...
            DialogNBTSource::Nbt(nbt) => {
                let tag = papokin_nbt::tag::NbtTag::Compound((*nbt).clone());
...
pub enum DialogNBTSource<'a> {
    Struct(&'a Dialog),
    Nbt(&'a papokin_nbt::compound::NbtCompound),
}
...
pub enum DialogBody {
    ...
    Item { item: i32 }, // TODO: ItemStack 序列化为 NBT
}
