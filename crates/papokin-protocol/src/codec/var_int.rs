...
/**
 * Minecraft 网络协议使用的可变长度整数类型。
 */
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
pub struct VarInt(pub VarIntType);

impl VarInt {
    pub const MAX_SIZE: NonZero<usize> = NonZero::new(5).expect("5 非零");
    ...
    /// 返回此 `VarInt` 在被写入时所需的确切字节数。
    /// [`Encode::encode`] 被调用（假定不发生错误）。
    #[must_use]
    #[inline]
