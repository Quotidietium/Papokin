...
use papokin_data::item_id_remap::{remap_item_id_for_version, remap_item_id_from_version};
...
/// 此组件在 `version` 中是否存在。重映射表用 `0` 表示
/// “不存在”；只有真正为 `0` 的分量才能合法地映射到该值。
...
use papokin_data::data_component_type_id_remap::{
...
fn serialize_item_stack_with_id(
    ...
    if version >= JavaMinecraftVersion::V_1_20_5 {
        ...
        if stack.is_empty() {
            ...
            write.write_u8(0)?; // TAG_End（无 NBT）
            Ok(())
        }
    } else if version >= JavaMinecraftVersion::V_1_13 {
        // 1.13 和 1.13.1：short id（空时为 -1）、字节计数、TAG_End
        if stack.is_empty() {
        ...
        // <= 1.12.2：short id（空时为 -1）、字节计数、short damage、TAG_End
        if stack.is_empty() {
            write.write_i16_be(-1)
        } else {
            write.write_i16_be(item_id as i16)?;
            write.write_i8(stack.item_count as i8)?;
            write.write_i16_be(0)?; // 伤害 / 元数据
            write.write_u8(0)?; // TAG_End（无 NBT）
            Ok(())
        }
    }
...
fn decode_custom_data(component_data: &[u8]) -> Result<Box<dyn DataComponentImpl>, ReadingError> {
    ...
    let data = match tag {
        NbtTag::Compound(compound) => compound,
        _ => papokin_nbt::compound::NbtCompound::new(),
    };
