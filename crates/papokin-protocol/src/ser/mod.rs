...
impl<'a, R: NetworkReadExt + ?Sized> papokin_nbt::deserializer::NbtDataSource<'a>
    for NetworkReadDataSource<'a, R>
{
    fn read_u8(&mut self) -> Result<u8, papokin_nbt::Error> {
        self.0.get_u8().map_err(|e| {
            papokin_nbt::Error::Incomplete(std::io::Error::new(
    ...
    fn read_bytes(&mut self, buf: &mut [u8]) -> Result<(), papokin_nbt::Error> {
    ...
    fn seek_relative(&mut self, offset: i64) -> Result<(), papokin_nbt::Error> {
        if offset < 0 {
            return Err(papokin_nbt::Error::Incomplete(std::io::Error::new(
    ...
    fn read_string(&mut self, len: usize) -> Result<Cow<'a, str>, papokin_nbt::Error> {
        ...
        let string =
            cesu8::from_java_cesu8(&buf).map_err(|_| papokin_nbt::Error::Cesu8DecodingError)?;
        Ok(Cow::Owned(string.into_owned()))
    }

    fn read_byte_array(&mut self, len: usize) -> Result<Cow<'a, [i8]>, papokin_nbt::Error> {
        let mut buf = vec![0u8; len];
        self.0.read_bytes_to_buf(&mut buf).map_err(|e| {
            papokin_nbt::Error::Incomplete(std::io::Error::new(
                std::io::ErrorKind::UnexpectedEof,
...
pub trait NetworkReadExt {
    ...
    fn get_nbt_with_version(
        ...
        if *version >= JavaMinecraftVersion::V_1_8 {
            ...
            if tag_id == papokin_nbt::END_ID {
                return Ok(None);
            }
            let mut helper =
                papokin_nbt::deserializer::NbtReadHelperJava::new(NetworkReadDataSource(self));
            if *version < JavaMinecraftVersion::V_1_20_2 {
                let _name = helper
                    .get_string()
                    .map_err(|e| ReadingError::Message(e.to_string()))?;
            }
            let tag = if tag_id == papokin_nbt::COMPOUND_ID {
                NbtTag::Compound(
...
impl<'a> NetworkReadSliceExt<'a> for &'a [u8] {
    ...
    fn get_component_borrowed(
        ...
        if *version < JavaMinecraftVersion::V_1_20_3 {
            ...
            let mut nbt_reader = papokin_nbt::deserializer::NbtReadHelperJava::new(&mut cursor);
...
impl<R: Read> NetworkReadExt for R {
    ...
    fn get_str_bounded(&mut self, bound: usize) -> Result<Box<str>, ReadingError> {
        ...
        // 我们把 `bound` 视为允许的 Java `char` 最大数量。

        // 首先，检查字节数是否多到超出 UTF-16 上限
        // 1 个 Java `char` 在 UTF-8 中最多占 3 字节：
        let maximum_utf8_bytes = bound.saturating_mul(3).min(crate::MAX_PACKET_DATA_SIZE);
        if bytes_len > maximum_utf8_bytes {
            return Err(ReadingError::TooLarge(format!(
                "string has too many bytes ({bytes_len} > {maximum_utf8_bytes})"
...
pub trait NetworkWriteExt {
    ...
    fn write_nbt(&mut self, data: NbtTag) -> Result<(), WritingError> {
        self.write_nbt_with_version(Some(&data), &JavaMinecraftVersion::V_1_21_11)
    }
}
    ...
    fn write_nbt_with_version(
        ...
        if *version >= JavaMinecraftVersion::V_1_8 {
                ...
                && !matches!(tag, NbtTag::End)
            {
                ...
                self.write_u8(papokin_nbt::END_ID)?;
            }
        } else {
            // <= 1.7.10
            if let Some(tag) = data
                && !matches!(tag, NbtTag::End)
            {
                ...
                buf.extend_from_slice(&[0x00, 0x00]); // 空根名称
