...
/// 内置数据集（方块、物品、实体等）对应的 Minecraft 版本。
///
/// 早于此版本的连接需要在网络传输时重映射其注册表 ID。
/// 这是刻意独立于 `packet::CURRENT_MC_VERSION`（即
/// 协议目标）：协议可切换到另一个版本，而
/// 生成的数据集——以及每个重映射表——仍遵循 26.3。
pub const NATIVE_DATA_VERSION: papokin_util::version::JavaMinecraftVersion =
...
/// 针对插件注册的自定义伤害类型的运行时支持：已解析的
/// 伤害执行路径所使用的原版或自定义表示，
/// 客户端同步 NBT 数据块，以及按版本进行的网络 id 转换。
...
mod sync_id_remap_tests {
    ...
    /// 该条目在 1.21.11 数据包文件夹中的排序索引——即 id
    /// 1.21.11 客户端在服务器同步注册表时为它分配的。
    #[test]
    fn biome_sync_remap_matches_1_21_11_registry() {
        ...
        // 仅 26.x 存在的生物群系回退到最接近的 1.21.11 生物群系。
        assert_eq!(
        ...
        // 任何条目都不得超过 1.21.11 注册表的大小（65 种生物群系），否则
        // 客户端的调色板查找会失败。
        assert!(
    ...
    fn damage_type_sync_remap_matches_1_21_11_registry() {
        ...
        // 位于仅限 26.x 的 `sulfur_cube_hot` 之前的条目保持其索引
        assert_eq!(
