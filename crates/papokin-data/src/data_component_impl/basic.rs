...
impl InstrumentImpl {
    /// 网络传输中为山羊角写入的乐器。该组件保持
    /// 单元结构体，因为生成的物品表构造的是 `&InstrumentImpl`
    /// (仅单元结构体支持的值表达式)；每件物品的乐器
    /// 名称需要先重新生成这些表。
...
/// 锁的物品谓词，以原始 NBT 复合标签形式保留，因为 Pumpkin
/// 尚未对物品谓词建模。
// TODO: 待物品谓词建模后，将 `predicate` 替换为带类型的物品谓词。
#[derive(Clone, Debug, PartialEq)]
pub struct LockImpl {
