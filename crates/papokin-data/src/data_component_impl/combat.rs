...
/// 插件注册的自定义附魔的定义，用于驻留一个
/// 进程生命周期的 [`&'static Enchantment`](Enchantment)，使自定义
/// 条目可以共享原版的组件存储
/// (`Cow<'static, [(&'static Enchantment, i32)]>`)，且不改变其类型。
pub struct CustomEnchantmentDefinition {
    /// 完整的带命名空间的 ID，例如 `"myplugin:frost"`。
    pub name: String,
    /// 工具提示中使用的翻译键（原版惯例：
...
/// 运行时注册表，存放插件注册的自定义附魔，按注册
/// 顺序。
///
/// 驻留的附魔会获得延续原版序列的 ID
/// (`Enchantment::ALL.len()`)，而这正是客户端
/// 会在注册表同步将自定义条目追加到
/// 原版条目。保持存储元素类型为 `&'static Enchantment`
/// 保持不变，可以让所有现有的读取方（铁砧、砂轮、附魔台、
/// 实体装备、生成的物品表等）处理自定义条目
/// 保持不变，并使 NBT 持久化继续基于名称（`Enchantment::name`）。
static CUSTOM_ENCHANTMENTS: std::sync::RwLock<Vec<&'static Enchantment>> =
    std::sync::RwLock::new(Vec::new());

/// 驻留一个自定义附魔并返回其进程生命周期的句柄。
///
/// 当名称与原版条目或已注册的自定义条目冲突时报错
/// 附魔，或当 id 空间（与原版共享的 `u8`）已满时。
...
/// 将网络 id 解析为驻留的自定义附魔。低于
/// `Enchantment::ALL.len()` 的均属原版，并在此返回 `None`。
#[must_use]
...
/// 每个已驻留的自定义附魔的完整命名空间名称，
/// 注册顺序（即原版条目之后的网络 id 顺序）。
#[must_use]
...
/// 此附魔是否为驻留的自定义（插件注册）条目
/// 而不是原版条目。自定义 id 沿用原版的编号序列。
#[must_use]
pub fn is_custom_enchantment(enchantment: &Enchantment) -> bool {
    usize::from(enchantment.id) >= Enchantment::ALL.len()
}

/// [`EnchantmentsImpl`] 与 [`StoredEnchantmentsImpl`] 共用的 NBT 读取器。
///
/// 同时接受扁平的 `{name: level}` 形式和 `{levels: {name:
/// level}}` 包装器。原版条目按名称解析；自定义条目则
/// 通过驻留表以其完整的命名空间 ID 进行查找。未知名称或
/// 格式错误的等级会被跳过而不是导致整个组件失败，因此
/// 插件已被移除的自定义附魔不会损坏物品。
...
/// 冒险模式的方块谓词，以其原始 NBT（复合标签或列表）形式保留
/// 因为 Pumpkin 尚未对面方块谓词建模。
// TODO: 待方块谓词建模后，将 `predicate` 替换为带类型的方块谓词。
#[derive(Clone, Debug, PartialEq)]
...
/// 盔甲纹饰的材料和图案，以其原始 NBT 保留（各自为一个注册表
/// id 或内联定义），因为 Pumpkin 尚未建模纹饰注册表。
// TODO: 待这些注册表建模后，将 `material`/`pattern` 替换为带类型的纹饰材料/图案。
#[derive(Clone, Debug, PartialEq)]
pub struct TrimImpl {
    pub material: NbtTag,
    pub pattern: NbtTag,
