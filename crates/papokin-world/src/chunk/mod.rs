...
use papokin_data::block_properties::{blocks_movement, has_random_ticks, is_air};
use papokin_data::chunk::ChunkStatus;
use papokin_data::fluid::Fluid;
use papokin_data::tag::Block::MINECRAFT_LEAVES;
...
// 在此处克隆，因为我们希望克隆区块的快照，以免阻塞写入过长时间
pub struct ChunkData {
    pub section: ChunkSections,
    /// 更多信息参见 `https://minecraft.wiki/w/Heightmap`
...
/// 从磁盘读取的区块的往返保留数据。
///
/// 原版和 Paper/Papo 的世界包含 Pumpkin 没有对应模型的区块字段
/// (`structures`, `blending_data`, `LastUpdate`, 未建模的高度图等)。
/// 它们在此处原样存储并原样写回，因此保存一个
/// 外部世界也绝不会销毁数据。
#[derive(Clone)]
pub struct PreservedChunkData {
    ...
    /// 当与规范名称不同时的原始 `Status` 字符串
    /// 解析后的状态（如 `minecraft:noise`），期间按原样写回，
    /// 状态枚举尚未推进。
...
pub struct ChunkEntityData {
    ...
    /// 在序列化的实体被消费并生成后置位。从那时起，
    /// 存活实体列表是唯一可信来源，`data` 会在每次保存时从它重建。
    pub live: AtomicBool,
...
impl ChunkSections {
    /// 逐区块段复制该区块的所有方块状态 ID，其来源为
    /// 自底向上。分区内条目先按 Y 排序，再按 Z，再按 X。
    /// 用于读取时复制的快照。
    #[must_use]
    ...
    /// 复制区块的所有生物群系 ID（每个区块段 4x4x4 四分格分辨率），
    /// 自底向上逐个区段进行。区段内的条目
...
impl ChunkHeightmaps {
    pub fn set(&mut self, heightmap: ChunkHeightmapType, x: i32, z: i32, height: i32, min_y: i32) {
        ...
        // 在 Minecraft 1.16+ 中，高度存储为 (y - min_y + 1)。0 表示低于 min_y。
        // 每个值使用 9 位，打包方式保证不跨越 u64 边界。
        // 64 / 9 = 每个 u64 存 7 个值。
...
impl ChunkSections {
    ...
    /// 在区块中设置给定方块，并返回旧方块
    /// 与 `set_block` 不同，此方法不会更新高度图。
    ///
    /// 仅在确定不需要更新高度图时才使用
    /// 或者当你在 `empty_with_heightmap` 中手动设置高度图时
...
impl ChunkData {
    ...
    /// 在单个批次内按绝对 Y 坐标设置区块中的多个方块。
    ///
    /// 此方法在全部更新中只获取一次区块段写锁和高度图锁，
    /// 显著加快批量方块修改。
    ///
    ///返回 `(relative_x, y, relative_z, replaced_block_state_id)` 的列表。
...
pub enum ChunkParsingError {
    ...
    FailedReadStatus(papokin_nbt::Error),
...
pub enum ChunkSerializingError {
    ...
    ErrorSerializingChunk(papokin_nbt::Error),
}
...
mod tests {
    ...
    use papokin_data::{Block, block_properties::has_random_ticks};
    ...
    fn heightmap_is_opaque() {
        ...
        // MOTION_BLOCKING：阻挡移动或为液体
        assert!(!ChunkHeightmapType::MotionBlocking.is_opaque(air));
        assert!(ChunkHeightmapType::MotionBlocking.is_opaque(stone));
        assert!(ChunkHeightmapType::MotionBlocking.is_opaque(leaves)); // 树叶阻挡移动
        assert!(ChunkHeightmapType::MotionBlocking.is_opaque(water)); // 水是液体

        // MOTION_BLOCKING_NO_LEAVES：阻挡移动或为液体，但不包括树叶
        ...
        assert!(!ChunkHeightmapType::MotionBlockingNoLeaves.is_opaque(leaves)); // 不含树叶
        assert!(ChunkHeightmapType::MotionBlockingNoLeaves.is_opaque(water)); // 水是液体
    }
