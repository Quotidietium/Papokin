...
impl Cylindrical {
    ...
    ///针对给定视距，返回预计算的相对区块偏移对 `(dx, dy)` 列表。
    #[must_use]
    #[inline]
    pub const fn get_offsets(view_distance: u8) -> &'static [(i8, i8)] {
        let vd = view_distance as usize;
        let max = papokin_data::chunk_view_lut::MAX_VIEW_DISTANCE as usize;
        let idx = if vd < max { vd } else { max };
        papokin_data::chunk_view_lut::CHUNK_VIEW_LUT[idx]
    }

    ///使用预计算的查找表（LUT），返回此圆柱体内所有区块的迭代器
    #[must_use]
    #[inline]
