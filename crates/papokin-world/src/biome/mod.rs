...
// clippy 1.98 误报：初始化器已经是 `const` 块。
thread_local! {
    /// 一个捷径；检查上次使用的生物群系是否就是应当使用的那个。
    ///
    /// 以叶片所属的树为键。`BiomeTree::get` 以此为搜索距离设定初始值
    /// 此叶子节点，且只接受严格更近的节点，因此从某个维度缓存的叶子
    /// 树本会胜过另一棵树的每个节点并被原样返回。原版
    /// 将 `lastResult` 存储在 `RTree` 自身上，而不是在所有树之间共享一个。
...
impl ActiveBiomeSupplier {
    ...
    pub fn from_biome_source(
        ...
        dimension: &papokin_data::dimension::Dimension,
    ) -> Self {
        match source {
            Some(crate::world_info::BiomeSource::Fixed { biome, .. }) => {
                let clean = biome.strip_prefix("minecraft:").unwrap_or(biome);
                let resolved = papokin_data::biome::Biome::from_name(clean)
                    .unwrap_or(&papokin_data::biome::Biome::PLAINS);
    ...
    #[must_use]
    pub fn from_dimension(dimension: &papokin_data::dimension::Dimension) -> Self {
        if dimension == &papokin_data::dimension::Dimension::THE_END {
            Self::End(end::TheEndBiomeSupplier)
        } else if dimension == &papokin_data::dimension::Dimension::THE_NETHER {
            Self::MultiNoise(MultiNoiseBiomeSupplier::NETHER)
        } else {
            Self::MultiNoise(MultiNoiseBiomeSupplier::OVERWORLD)
        }
...
#[cfg(test)]
mod test {
    use papokin_data::{chunk::Biome, dimension::Dimension};
    use papokin_util::read_data_from_file;
    ...
    /// 从某个维度的生物群系树缓存的叶子绝不能用于播种另一个维度的
    /// 搜索，否则下界会充满主世界的生物群系（及其生物与地物）。
    #[test]
