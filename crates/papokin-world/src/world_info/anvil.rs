...
use papokin_util::{Difficulty, world_seed::Seed};
...
fn check_data_version(data: &NbtCompound) -> Result<(), WorldInfoError> {
    let Some(data_version) = data.get_int("DataVersion") else {
        error!("level.dat 文件没有数据版本！这意味着它已损坏或版本过旧（不支持读取）");
...
fn check_level_version(data: &NbtCompound) -> Result<(), WorldInfoError> {
    let Some(level_version) = data.get_int("version") else {
        error!("level.dat 文件没有存档版本！这意味着它已损坏或版本过旧（不支持读取）");
        return Err(WorldInfoError::DeserializationError("缺少 version".into()));
    };
...
impl WorldInfoReader for AnvilLevelInfo {
    fn read_world_info(&self, level_folder: &Path) -> Result<LevelData, WorldInfoError> {
        ...
        let Some(data) = root.get_compound(LEVEL_DATA_TAG) else {
            error!("level.dat 文件没有 {LEVEL_DATA_TAG} 复合标签，文件已损坏");
            return Err(WorldInfoError::DeserializationError("缺少 Data".into()));
        };
        ...
        // game_rules.dat – 优先使用新文件；否则回退到 level.dat 中的值
...
impl WorldInfoWriter for AnvilLevelInfo {
    fn write_world_info(
        ...
        // 首次运行：世界文件夹可能尚不存在（`read_world_info`
        // 刚刚返回了 `InfoNotFound`）；在此处创建它正是
        // 使初始的 `level.dat` 写入成功。
        std::fs::create_dir_all(level_folder)?;

        // 写入 `level.dat_new`，执行 fsync，然后原子换入；一旦崩溃
        // 写入中途也绝不会留下残缺的 `level.dat`。
        ...
        {
            error!("写入 world_gen_settings.dat 失败：{e}");
        }

        // world_clocks.dat – 持久化主世界的 day_time；保留其他
        ...
        if let Err(e) = write_world_clocks(level_folder, &clocks) {
            error!("写入 world_clocks.dat 失败：{e}");
        }
        ...
        if let Err(e) = write_weather(level_folder, &weather) {
            error!("写入 weather.dat 失败：{e}");
        }

        // wandering_trader.dat（存根 / 加载-保存）
...
mod test {
    ...
    fn serialize_level_dat() {
        let mut data_comp = papokin_nbt::compound::NbtCompound::new();
        ...
        let bytes = papokin_nbt::Nbt::from(root).write();
