...
use papokin_data::{BlockDirection, BlockState, BlockStateId};
...
impl LightEngine {
    ...
    /// 检查两个方块状态的光照属性是否不同，与原版 `LightEngine.hasDifferentLightProperties` 一致。
    #[inline]
    #[must_use]
    ...
    /// 返回此方块状态的最小光照衰减值，与原版 `LightEngine.getOpacity` 一致。
    #[inline]
    #[must_use]
    ...
    /// 计算两个方块之间的光照衰减，对应原版 `LightEngine.getLightDampeningInto`。
    #[inline]
    #[must_use]
