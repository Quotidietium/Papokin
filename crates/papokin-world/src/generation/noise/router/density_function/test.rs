...
use papokin_data::noise_router::OVERWORLD_BASE_NOISE_ROUTER;
...
// 这只是一个占位值，因为我们实际上并没有在构建针对特定区块的函数
...
#[test]
// 此测试验证在种子初始化之后、区块生成之前所生成的函数
// 初始化与公认的 Java 值一致。
//
// 这等价于 Java 的 `NoiseRouter` 被传入 `NoiseConfig` 之后、尚未被
// 传入 `ChunkNoiseGenerator` 的
#[expect(clippy::too_many_lines)]
fn normal_surface_noisified() {
    let pos = Vector3 { x: 0, y: 0, z: 0 };
    // TODO: 将这些值移到文件中，并为它们创建提取器
    assert_close(
...
// // 以下测试所用密度函数的密度函数组件的有效性
// // 地形生成。基本上，这有助于缩小正确性错误的出现位置。
...
fn round_function_parity() {
    ...
    use papokin_data::noise_router::RoundingOperation;
    ...
    // 类型 1：四舍五入（Java 的 (float)Math.round(x) 即 floor(x + 0.5)）
...
fn gradient_function_parity() {
    ...
    use papokin_data::noise_router::{Axis, GradientData, Tiling};

    // 钳制采样器：from_y = 0，to_y = 10，from_val = 0.0，to_val = 100.0
