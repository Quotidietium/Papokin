...
mod test {
    ...
    fn terrain_biome_lookup_crosses_chunk_boundary() {
        ...
        // 生物群系缩放会在 z=9600 两侧选取绝对四分位 (338, 17, 2399)。
        // 原版通过所属区块来解析该 quart。它不得对该 quart 进行回绕
        // 将坐标归入当前正在生成地表的区块。
        ...
        for chunk_x in south.x - 1..=south.x + 1 {
            for chunk_z in south.z - 1..=south.z + 1 {
                ...
                if (chunk_x, chunk_z) == (north.x, north.z) {
                    // 使存储的 authority 与新的生物群系源采样可观测地不同。
    ...
    fn nether_complex_references_use_selected_structure() {
        ...
        panic!("在采样的下界复合区域中未找到堡垒遗迹起点");
    }

    /// 验证结构引用能在部分生成的往返过程中保持完好。
    #[test]
    ...
    // 针对 Noise 阶段区块恢复时高度图被转置的回归测试。
    // 平坦地形无法暴露此 bug，因此使用带坡度的区块
    #[test]
    fn heightmap_roundtrip_through_chunk_data_resume() {
        ...
        assert_eq!(chunk_data.status, papokin_data::chunk::ChunkStatus::Terrain);
            ...
            for local_y in 0..dumped_height {
                for z in 0..16usize {
                    ...
                    if mismatches < 10 {
                        ...
                        let act_block =
                            papokin_data::BlockState::from_id(actual).id.to_block().name;
                        let exp_block = papokin_data::BlockState::from_id(
                            papokin_data::BlockStateId::new(expected).unwrap(),
                        )
                        .id
                        .to_block()
                        .name;
                        println!(
                            "[{test_name}] 局部坐标 ({x}, {y}, {z}) 处不匹配：实际为 {act_block} ({}), 预期为 {exp_block} ({expected})",
                            actual.as_u16()
                        );
                    }
                    mismatches += 1;
        ...
        for x in 0..16usize {
            for local_y in dumped_window_height(expected_data)..chunk.height() as usize {
                for z in 0..16usize {
                    let actual = chunk.get_block_state_raw(x as i32, local_y as i32, z as i32);
                    assert!(
                        papokin_data::BlockState::from_id(actual).is_air(),
    ...
    fn no_blend_no_beard_0_0() {
        let expected: Vec<u16> = papokin_util::read_data_from_file!(
