...
pub fn try_generate_structure(
    ...
    if let Some(pos) = structure_pos {
        // 获取结构起始位置处的生物群系。
        // 将生物群系 Y 钳制到区块的有效范围——结构的 start_pos.y 可能超出
        // 区块的逻辑高度（例如下界化石使用完整高度 256，但
        // ProtoChunk 仅覆盖 logical_height 128）。
...
pub fn lazily_generate_structure(
    key: &StructureKeys,
    structure: &Structure,
    mut context: StructureGeneratorContext, // 替代了 5 个独立参数！
