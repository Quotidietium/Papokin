...
impl StructurePieceBase for LibraryPiece {
    ...
    fn place(
        ...
        let max_y = if self.tall { 10 } else { 5 }; // Java：i-1，其中 i 为 11 或 6
        ...
        if self.tall {
            inner.add_block(chunk, Block::AIR.default_state, 12, 9, 1, &box_limit); // 清空顶部箱子上方的空间
            inner.add_chest(
                chunk,
                &box_limit,
                random,
