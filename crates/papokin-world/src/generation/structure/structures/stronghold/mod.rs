...
/// 极佳参考：<https://minecraft.wiki/w/Stronghold>
#[derive(Deserialize)]
...
impl StructureGenerator for StrongholdGenerator {
    fn get_structure_position(
        ...
        loop {
            ...
            // 在原版中，pieces_to_process 是 'start.pieces'
...
pub struct PieceWeight {
    ...
    pub weight: i32,
    pub limit: u32,
    pub generated_count: u32, // 添加用于跟踪每代数量的计数器
}
...
impl StrongholdPiece {
    ...
    pub fn fill_nw_opening(
        ...
        if let Some(facing) = &self.piece.facing {
            ...
            // 翻译为世界坐标的 Java switch(direction) 逻辑
