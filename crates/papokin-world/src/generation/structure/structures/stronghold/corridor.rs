...
impl StructurePieceBase for SmallCorridorPiece {
    ...
    fn place(
        ...
        for i in 0..self.length {
            ...
            for j in 1..=3 {
                p.add_block(chunk, stone, 0, j, i, &box_limit); // 左墙
                p.add_block(chunk, cave_air, 1, j, i, &box_limit); // 空气间隙
                p.add_block(chunk, cave_air, 2, j, i, &box_limit); // 空气间隙
                p.add_block(chunk, cave_air, 3, j, i, &box_limit); // 空气间隙
                p.add_block(chunk, stone, 4, j, i, &box_limit); // 右墙
            }
