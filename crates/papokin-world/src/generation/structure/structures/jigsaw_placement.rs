...
impl MaxDistance {
    ...
    pub const fn new(horizontal: i32) -> Self {
        Self {
            horizontal,
            vertical: 384, // 默认 Y_SIZE（min_y 到 max_y）
        }
    }
}
...
/// 针对 1.20+ 引入的池别名的动态查找（例如试炼密室的刷怪笼/内容物别名）。
...
impl PoolAliasLookup {
    ...
    pub fn from_bindings(
        bindings: &[papokin_data::structures::PoolAliasBinding],
    ...
    fn add_binding(
        &mut self,
        binding: &papokin_data::structures::PoolAliasBinding,
        random: &mut RandomGenerator,
    ) {
        use papokin_data::structures::PoolAliasBinding;
        match binding {
    ...
    fn try_placing_children(
        ...
        'source_jigsaws: for source_jigsaw in source_jigsaws {
            ...
            let Some(target_pool) = TemplatePool::discover(pool_name) else {
                tracing::warn!("模板池为空或不存在: {}", pool_name);
                continue;
            };
...
pub const fn rotate_direction(
    dir: papokin_util::BlockDirection,
    rotation: Rotation,
) -> papokin_util::BlockDirection {
    use papokin_util::BlockDirection;
    match rotation {
        Rotation::None => dir,
