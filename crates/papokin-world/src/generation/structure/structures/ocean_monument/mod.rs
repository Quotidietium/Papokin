...
use papokin_data::{Block, BlockState};
...
pub(crate) fn has_valid_biomes(
    ...
    let start_biomes = papokin_data::tag::WorldgenBiome::MINECRAFT_HAS_STRUCTURE_OCEAN_MONUMENT.1;
    ...
    let allowed = papokin_data::tag::WorldgenBiome::MINECRAFT_REQUIRED_OCEAN_MONUMENT_SURROUNDING.1;
...
#[cfg(test)]
mod tests {
    use papokin_data::{Block, dimension::Dimension};
    use papokin_util::{
        BlockDirection,
    ...
    impl HeightSampler for OceanFloorSampler {
        fn estimate_height(&mut self, _block_x: i32, _block_z: i32) -> i32 {
            panic!("海底神殿不得使用 world-surface 高度图");
        }
    ...
    fn elder_guardian_nbt_is_accepted_by_the_entity_loader() {
        let nbt = elder_guardian_nbt(papokin_util::math::vector3::Vector3::new(1, 2, 3));
