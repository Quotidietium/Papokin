...
use papokin_util::math::block_box::BlockBox;
use papokin_util::math::position::BlockPos;
use papokin_util::math::vector3::Vector3;
use papokin_util::random::{RandomDeriverImpl, RandomImpl};
...
pub enum PoolElementKind {
    ...
    List(Vec<Self>),
    Feature(papokin_data::placed_feature::PlacedFeature),
}
    ...
    fn into_element(self) -> Option<(PoolElementKind, JigsawProjection)> {
        match self {
            ...
            Self::Feature {
                ...
                papokin_data::placed_feature::PlacedFeature::from_name(feature)
...
impl PoolElementKind {
    ...
    #[must_use]
    pub fn get_bounding_box(&self, offset: BlockPos, rotation: papokin_data::Rotation) -> BlockBox {
        match self {
            Self::Single {
                resolved_template, ..
    ...
    pub fn get_shuffled_jigsaw_blocks(
        ...
        random: &mut papokin_util::random::RandomGenerator,
    ) -> Vec<JigsawBlock> {
        match self {
            Self::Single {
                resolved_template, ..
            ...
            Self::Feature(_) => vec![JigsawBlock {
                ...
                facing: papokin_util::BlockDirection::Down,
                up: papokin_util::BlockDirection::South,
...
impl PoolElement {
    #[must_use]
    pub const fn feature(&self) -> Option<papokin_data::placed_feature::PlacedFeature> {
        match self.kind {
    ...
    pub fn discover(id: &str) -> Option<Arc<Self>> {
            ...
            crate::generation::structure::template::get_template_pool_json(id)
        {
            let raw: RawTemplatePool = match serde_json::from_str(json) {
                Ok(pool) => pool,
                Err(error) => {
                    tracing::error!("解析模板池 {id} 失败：{error}");
                    return None;
                }
            };
            let elements = raw
...
pub struct JigsawBlock {
    ...
    pub facing: papokin_util::BlockDirection,
    pub up: papokin_util::BlockDirection,
...
impl JigsawBlock {
    ...
    pub fn from_template_block(
        ...
        let facing = match facing_part {
            "south" => papokin_util::BlockDirection::South,
            "east" => papokin_util::BlockDirection::East,
            "west" => papokin_util::BlockDirection::West,
            "up" => papokin_util::BlockDirection::Up,
            "down" => papokin_util::BlockDirection::Down,
            _ => papokin_util::BlockDirection::North,
        };

        let up = match up_part {
            "north" => papokin_util::BlockDirection::North,
            "south" => papokin_util::BlockDirection::South,
            "east" => papokin_util::BlockDirection::East,
            "west" => papokin_util::BlockDirection::West,
            "down" => papokin_util::BlockDirection::Down,
            _ => papokin_util::BlockDirection::Up,
        };
    ...
    pub fn can_attach(
        source: &Self,
        target_facing: papokin_util::BlockDirection,
        target_name: &str,
    ) -> bool {
...
impl StructurePieceBase for PoolElementStructurePiece {
    ...
    fn place(
        ...
        let origin =
            papokin_util::math::vector3::Vector3::new(self.pos.0.x, self.pos.0.y, self.pos.0.z);
        ...
        self.element
            .for_each_template(|_name, processor_list, legacy, template| {
                ...
                let placement_origin = papokin_util::math::vector3::Vector3::new(
...
pub struct JigsawGenerator {
    ...
    pub pool_aliases: &'static [papokin_data::structures::PoolAliasBinding],
}
...
impl JigsawGenerator {
    ...
    pub const fn with_pool_aliases(
        mut self,
        aliases: &'static [papokin_data::structures::PoolAliasBinding],
    ) -> Self {
...
impl StructureGenerator for JigsawGenerator {
    fn get_structure_position(
        ...
        let structure = context
            .structure_key
            .map(|key| papokin_data::structures::Structure::get(&key));
        ...
        let mut alias_random =
            papokin_util::random::legacy_rand::LegacyRand::from_seed(context.seed as u64)
                .next_splitter()
...
mod tests {
    ...
    fn ancient_city_pools_match_vanilla_weights() {
        ...
        for (id, element_count, total_weight) in expected {
            let pool = TemplatePool::discover(id).unwrap_or_else(|| panic!("缺少池 {id}"));
    ...
    fn ancient_city_start_templates_and_anchor_exist() {
        ...
        for element in &pool.elements {
            let template = element.first_template().expect("缺少起始模板");
            assert!(
    ...
    fn ancient_city_builds_a_multi_piece_graph() {
        ...
        let context = StructureGeneratorContext {
            ...
            height: 384,
            height_sampler: None,
            structure_key: Some(papokin_data::structures::StructureKeys::AncientCity),
        };
    ...
    fn pillager_outpost_builds_the_vanilla_jigsaw_graph() {
        ...
        let world_gen = crate::generation::get_world_gen(
            papokin_util::world_seed::Seed(SEED as u64),
        ...
        let context = StructureGeneratorContext {
            ...
            structure_key: Some(papokin_data::structures::StructureKeys::PillagerOutpost),
        };
