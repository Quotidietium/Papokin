...
impl StructurePiece {
    ...
    /// 根据实心的渲染邻居调整箱子方块状态的朝向，与原版 `StructurePiece.reorient` 一致。
    #[must_use]
    pub fn reorient(
        pos: &Vector3<i32>,
        block_state: &'static BlockState,
        mut get_block_state: impl FnMut(&Vector3<i32>) -> papokin_data::BlockStateId,
    ) -> &'static BlockState {
        use papokin_data::block_properties::{ChestProperties, HorizontalFacing};
    ...
    /// 在给定局部坐标处放置一个带有延迟战利品表的箱子。
    ///
    ///若箱子已被放置（即该位置位于包围盒内），则返回 `true`，
    /// 否则返回 `false`。
    #[allow(clippy::too_many_arguments)]
    pub fn add_chest(
        &self,
    ...
    /// 在给定的局部坐标处创建并放置一个箱子，对应原版 `StructurePiece.createChest`。
    #[allow(clippy::too_many_arguments)]
    pub fn create_chest(
        &self,
        chunk: &mut ProtoChunk,
    ...
    pub fn add_dispenser(
        ...
        y: i32,
        z: i32,
        facing: papokin_data::block_properties::Facing,
        loot_table: &str,
    ) -> bool {
        use papokin_data::block_properties::DispenserLikeProperties;
        use papokin_nbt::compound::NbtCompound;
...
impl StructurePiecesCollector {
    ...
    /// 计算一个随机垂直位置，并平移结构以适配。
    /// 对应 'shiftInto(int topY, int bottomY, Random random, int topPenalty)'
    pub fn shift_into(
        &mut self,
        top_y: i32,
        bottom_y: i32,
...
pub trait HeightSampler {
    ...
    fn sample_column_block(
        ...
        _block_z: i32,
        _y: i32,
    ) -> Option<&'static papokin_data::BlockState> {
        None
    }
}
...
pub struct StructureGeneratorContext<'a> {
    ...
    pub structure_key: Option<papokin_data::structures::StructureKeys>,
}
