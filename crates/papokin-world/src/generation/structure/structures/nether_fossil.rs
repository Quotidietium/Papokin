...
use papokin_data::{Block, BlockDirection, BlockState, Mirror, Rotation};
use papokin_util::{
...
/// 下界化石的原版高度提供者范围。
/// 来自 `nether_fossil.json`：uniform(absolute=32, `below_top=2`)。
/// 在原版下界中，生成器生成深度为 128：
...
impl StructureGenerator for NetherFossilGenerator {
    fn get_structure_position(
        ...
        // 原版随机调用顺序（NetherFossilStructure.java）：
        // 1. nextInt(16) 取区块内 X 偏移
        // 2. nextInt(16) 取区块内 Z 偏移
        // 3. height.sample(random, generationContext) 取初始 Y（均匀分布 32..125）
        // 4. 向下按列扫描至海平面（无随机调用）
        //    若 y <= seaLevel：返回 empty
...
mod tests {
    ...
    fn nether_fossil_never_places_on_roof() {
        for seed in 0..100 {
            ...
            let context = StructureGeneratorContext {
                ...
                structure_key: Some(papokin_data::structures::StructureKeys::NetherFossil),
            };
    ...
    fn nether_fossil_rejects_when_no_ground_above_sea_level() {
        let mut sampler = MockColumnSampler { ground_y: 20 }; // 低于海平面（32）
        let context = StructureGeneratorContext {
            seed: 42,
            chunk_x: 0,
            chunk_z: 0,
