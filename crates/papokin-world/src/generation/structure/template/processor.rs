use papokin_data::{Block, BlockId, BlockState, tag::Taggable};
...
/// 在世界生成或放置期间。
///
/// 规范参见 <https://minecraft.wiki/w/Processor_list>。
#[derive(Clone, Debug)]
pub enum StructureProcessor {
    /// 将通过规则测试的方块替换为自定义状态，并应用方块实体修改器。
    Rule(Vec<ProcessorRule>),
    /// 根据完整性随机移除方块，保留世界中原本存在的方块。
    BlockRot {
        integrity: f32,
        rottable_blocks: Option<RottableBlocks>,
    },
    /// 使石头、圆石、楼梯、台阶、墙和黑曜石方块老化，产生苔藓/裂纹变体。
    BlockAge { mossiness: f32 },
    /// 移除指定的方块（如结构空位 / 结构方块 / 空气），并保留世界中原本存在的方块。
    ...
    /// 将石头变种替换为黑石变种，将铁栏杆替换为锁链。
    BlackstoneReplace,
    /// 将拼图方块替换为其目标 `final_state`。
    JigsawReplacement,
    /// 轮廓形状不完整的方块无法在世界中覆盖熔岩。
    LavaSubmergedBlock,
    /// 限制委托处理器可修改方块的最大数量。
    Capped {
        limit: IntProvider,
...
/// 26.3 将方块状态键从 Name 和 Properties 重命名为 id 和 properties，并写入一个
/// 无属性的状态仅表示为其名称。
#[derive(Deserialize, Debug)]
#[serde(untagged)]
...
pub fn load_processor_list(name: &str) -> Arc<[StructureProcessor]> {
    ...
    let raw_list: Vec<RawProcessor> = match serde_json::from_str::<RawProcessorListWrapper>(json) {
        ...
        Err(error) => {
            tracing::error!("解析结构处理器列表 {name} 失败：{error}");
            return Arc::from([]);
        }
    };
