//! 结构模板的加载、变换与保存（与 Minecraft 原版 `StructureTemplate` 1:1 对应）。
//!
//! 本模块负责解析 Minecraft 原版结构模板文件（`.nbt`）
//! 转换为包含调色板、实体、方块信息与变换的运行时表示。

use std::io::Cursor;
use std::sync::OnceLock;

use papokin_data::{Mirror, Rotation};
use papokin_nbt::{compound::NbtCompound, nbt_compress::read_gzip_compound_tag, tag::NbtTag};
use papokin_util::math::{block_box::BlockBox, vector3::Vector3};
use thiserror::Error;
...
pub enum TemplateError {
    ...
    NbtError(#[from] papokin_nbt::Error),
...
/// 模板中惰性解析的拼图方块。
///
/// 加一层包装，使模板的 `Debug` 派生不要求 `JigsawBlock: Debug`。
...
/// 一个从 NBT 文件加载的结构模板，与原版 `StructureTemplate` 相对应。
...
pub struct StructureTemplate {
    ...
    // 向后兼容字段
    // TODO: 将这些字段改为私有并提供访问器。它们目前是公开的，因此
    // 调用方可在拼图缓存填充后修改它们。缓存随后
    // 过期。`load()` 会重置缓存，但直接修改不会。
    pub palette: Vec<PaletteEntry>,
    pub blocks: Vec<TemplateBlock>,
    pub entities: Vec<TemplateEntity>,

    /// 从扁平的 `blocks`/`palette` 视图解析出的拼图方块，惰性计算
    /// 首次使用时完成，因此放置绝不会重新扫描模板的方块列表。
...
pub struct PaletteEntry {
    /// 方块名称（例如 "`minecraft:stone_bricks`"）。
    pub name: String,

    /// 方块状态属性（如 [("facing", "north"), ("lit", "false")]）。
...
impl PaletteEntry {
    ...
    /// 将方块状态字符串（例如 "`minecraft:oak_log`[axis=x]"）解析为调色板条目。
    #[must_use]
    ...
    pub fn transform(&self, rotation: Rotation, mirror: Mirror) -> Self {
        ...
        let properties = papokin_data::transform_block_properties(
            &self.name,
            &self.properties,
            rotation,
            mirror,
...
impl StructureTemplate {
    ...
    /// 返回模板的拼图方块（以模板本地坐标表示）。
    ///
    /// 从扁平的 `blocks`/`palette` 视图解析一次并缓存，而不是
    /// 在每次放置尝试时都被重新扫描和重新解析。
    #[must_use]
...
mod tests {
    ...
    /// 从随附的 26.3 数据包（资源）中加载一个真实模板，该模板将调色板键命名为 id 和
    /// 属性，而不是 Name 和 Properties。
    #[test]
    fn load_26_3_template() {
        let bytes = include_bytes!(
            "../../../../../../assets/datapacks/26_3/data/minecraft/structure/igloo/top.nbt"
        );
        let template = StructureTemplate::from_nbt_bytes(bytes).expect("加载模板失败");
