...
pub struct VegetationPatchFeature {
    ...
    pub depth: papokin_util::math::int_provider::IntProvider,
    pub extra_bottom_block_chance: f32,
    pub vertical_range: i32,
    pub vegetation_chance: f32,
    pub xz_radius: papokin_util::math::int_provider::IntProvider,
    pub extra_edge_column_chance: f32,
}

impl VegetationPatchFeature {
    ///返回指向表面“内部”的方块方向（地面为向下，天花板为向上）。
    pub(crate) const fn surface_direction(&self) -> papokin_data::BlockDirection {
        match self.surface {
            VerticalSurfaceType::Floor => papokin_data::BlockDirection::Down,
            VerticalSurfaceType::Ceiling => papokin_data::BlockDirection::Up,
        }
    }

    /// `self.surface_direction().to_offset()` 的快捷方式。
    ...
    fn place_ground<T: GenerationCache>(
        ...
        for i in 0..depth {
            ...
            let state_block_id = papokin_data::Block::from_state_id(state_to_place.id).id;
