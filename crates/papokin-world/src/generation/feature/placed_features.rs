...
use papokin_data::{Block, BlockDirection, BlockStateId};
use papokin_util::HeightMap;
use std::collections::HashMap;
use std::iter;
use std::sync::LazyLock;

use papokin_util::biome::FOLIAGE_NOISE;
...
pub static PLACED_FEATURES: LazyLock<
    HashMap<papokin_data::placed_feature::PlacedFeature, PlacedFeature>,
> = LazyLock::new(build_placed_features);

pub enum PlacedFeatureWrapper {
    Direct(PlacedFeature),
    Named(papokin_data::placed_feature::PlacedFeature),
}
...
pub enum Feature {
    Named(papokin_data::configured_feature::ConfiguredFeature),
...
impl PlacedFeature {
    pub fn generate_in_proto_chunk(
        ...
        feature_name: papokin_data::placed_feature::PlacedFeature,
...
impl CountOnEveryLayerPlacementModifier {
    pub fn get_positions<T: GenerationCache>(
        ...
        let mut positions = Vec::new(); // 使用 Vec 收集结果，类似于 Stream.builder()
        let mut i = 0; // 表示 findPos 中的 'targetY'
        let mut bl;
...
impl ConditionalPlacementModifier for BiomePlacementModifier {
    fn should_place<T: GenerationCache>(
        &self,
        _block_registry: &dyn WorldPortalExt,
        this_feature: papokin_data::placed_feature::PlacedFeature,
        chunk: &T,
        _random: &mut RandomGenerator,
        pos: BlockPos,
    ) -> bool {
...
pub trait CountPlacementModifierBase {
    fn get_positions(
        ...
        let count = self.get_count(random, pos).max(0); // 原版把负数数量视为 0，但 Rust 中的 `i32 as usize` 会发生回绕。
...
// 放在 `src/generated` 中，便于升级 MC 版本时查找。
// 该路径相对于本文件（向上两级才能到达 `src`）。

///返回所有已放置地物的名称，用于 `/place feature` 的 Tab 补全。
#[must_use]
pub const fn all_placed_feature_names() -> &'static [&'static str] {
    papokin_data::placed_feature::PlacedFeature::all_names()
}
include!("../../../../papokin-data/src/generated/placed_features_generated.rs");
