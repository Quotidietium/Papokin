...
use papokin_data::chunk::DoublePerlinNoiseParameters;
use papokin_data::fluid::{Fluid, FluidState};
...
use papokin_data::{Block, BlockState, block_properties::blocks_movement, chunk::Biome};
use papokin_data::{BlockId, BlockStateId, tag};
use papokin_util::random::xoroshiro128::XoroshiroSplitter;
use papokin_util::random::{RandomImpl, get_large_feature_seed, legacy_rand::LegacyRand};
use papokin_util::{
    HeightMap,
...
use papokin_data::tag::get_tag_ids;
...
impl ProtoChunk {
    ...
    pub fn from_chunk_data(
        ...
        // 从磁盘加载的区块，其方块实体随区块数据一同携带。
        // 保留它们，以便区块就绪时世界可以创建方块实体
        // 变为活动状态，因此再次保存区块不会丢弃它们。
        ...
        {
            // 结构起点与引用目前是临时性的原区块数据。
            // 在恢复部分生成的区块时重建它们，以便结构
            // 跨越区块边界的数据不会在卸载边界处被截断。
    ...
    pub fn step_to_noise(&mut self, generator: &super::generator::VanillaGenerator) {
        ...
        for (key, instance) in &self.structure_starts {
            let structure = papokin_data::structures::Structure::get(key);
            let terrain_adaptation = structure.terrain_adaptation;

            // 原版在 adaptation 为 None 时会尽早严格跳过 Beardifier 部件的过滤
            if terrain_adaptation == papokin_data::structures::TerrainAdaptation::None {
                continue;
            }
            ...
            for piece in &collector.pieces {
                ...
                // 匹配 `piece.isCloseToChunk(chunkPos, 12)`
                // 校验向外扩展 12 个方块后是否覆盖区块边界
        ...
        for local_x in 0..16 {
            for local_z in 0..16 {
                ...
                let Some(this_biome) =
                    ...
                    panic!("Surface 生物群系邻域必须覆盖模糊生物群系查找");
                };
                // 石柱在地表以上用默认方块填充，而原版会保留
                // 从立柱之前的高度开始扫描，因此立柱本身永远不会被扫描到
                // 材质规则，保持为石头，而不会像地形那样分层。
    ...
    pub fn generate_features_and_structure<T: GenerationCache>(
        ...
        // 原版会先收集 3x3 区块邻域内存储的所有生物群系，然后再选择
        // 全局有序的地物集合。
    ...
    pub fn set_structure_references(&mut self, generator: &super::generator::VanillaGenerator) {
        ...
        // 在该维度每个区块中都为常量，因此将其提升到循环外
        // 进出下方（已缓存的）结构起点计算。
        // 与原版 WorldGenerationContext 一致：
        ...
        for (set_index, set) in StructureSet::ALL.iter().enumerate() {
            ...
            match &set.placement.placement_type {
                StructurePlacementType::RandomSpread(spread) => {
                    let region_x = papokin_util::math::floor_div(self.x, spread.spacing);
                    let region_z = papokin_util::math::floor_div(self.z, spread.spacing);
            ...
            for (candidate_chunk_x, candidate_chunk_z) in candidate_chunks {
                    ...
                    && (candidate_chunk_z - self.z).abs() <= 8
                {
                    ...
                    for entry in entries {
                        ...
                        // 结构的放置仅取决于其起始区块和
                        // 世界种子，所以要缓存它：否则周围每个依赖它的区块
                        // 引用重叠，它就会重新运行（开销高昂的）拼图
                        // 展开。`context` 仅在缓存未命中时才构建。
