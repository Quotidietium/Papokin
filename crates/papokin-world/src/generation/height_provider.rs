...
impl VeryBiasedToBottomHeightProvider {
    pub fn get(&self, random: &mut RandomGenerator, min_y: i8, height: u16) -> i32 {
        ...
        if max - min - inner < 0 {
            warn!("高度范围为空: min={min}, max={max}, inner={inner}");
            return min;
        }
...
impl UniformHeightProvider {
    pub fn get(&self, random: &mut RandomGenerator, min_y: i8, height: u16) -> i32 {
        ...
        if min > max {
            // TODO: 调查为何会出现 min > max（例如 min=136, max=127）
            return min;
        }
...
mod tests {
    ...
    use papokin_util::y_offset::{AboveBottom, BelowTop};
    ...
    fn very_biased_to_bottom_parity() {
        ...
        // 下界高度 128，min_y 0 → min = 10，max = 118
        for _ in 0..1000 {
