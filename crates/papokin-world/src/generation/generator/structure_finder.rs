...
use papokin_util::math::{floor_div, position::BlockPos};
...
/// 查找其放置信息被列出的最近结构的方块位置
/// 在 `placements` 中，位于 `max_search_radius` 个区块区域环内。
///
/// 对应原版实现中的两遍处理逻辑
/// `ChunkGenerator.findNearestMapStructure`：
///
/// 1. **同心环**式放置（要塞）在一次遍历中解析
///    从预先计算好的 [`GlobalStructureCache`] 中获取。
/// 2. **随机扩散**类型的放置位置由内向外逐环搜索，直到
///    在第一个能产生结果的半径处。
///
/// 返回两轮扫描中的最佳候选。
...
/// 查找实际能产出 `target_structures` 之一的最近候选。
/// 探险家地图使用此方式，而不是指向仅用于放置的候选结构，因为后者的
/// 生物群系可能会拒绝所请求的结构。
#[must_use]
...
pub fn find_nearest_structure_start(
    ...
    target_structures: &[papokin_data::structures::StructureKeys],
    max_search_radius: i32,
    ...
    use papokin_data::structures::Structure;
