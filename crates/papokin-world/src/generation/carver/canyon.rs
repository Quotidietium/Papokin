    ...
    fn do_carve(
        ...
        for current_step in step..distance {
            ...
            let mut horizontal_radius =
                1.5 + f64::from(papokin_util::math::sin(progress) * thickness);
            ...
            let xc = papokin_util::math::cos(vertical_rotation);
            let xs = papokin_util::math::sin(vertical_rotation);
            x += f64::from(papokin_util::math::cos(horizontal_rotation) * xc);
            y += xs as f64;
            z += f64::from(papokin_util::math::sin(horizontal_rotation) * xc);
...
mod tests {
    ...
    use papokin_data::carver::CANYON;
