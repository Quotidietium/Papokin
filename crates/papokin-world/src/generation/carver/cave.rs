...
use papokin_data::carver::{CarverAdditionalConfig, CarverConfig, HeightProvider};
...
impl CaveCarver {
    ...
    fn create_room(
        ...
        let horizontal_radius =
            1.5 + f64::from(papokin_util::math::sin(std::f32::consts::FRAC_PI_2) * thickness);
    ...
    fn create_tunnel(
        ...
        for current_step in step..dist {
            ...
            let horizontal_radius =
                1.5 + f64::from(papokin_util::math::sin(progress_arg) * thickness);
            let vertical_radius = horizontal_radius * y_scale;
            let cos_x = papokin_util::math::cos(vertical_rotation);
            x += f64::from(papokin_util::math::cos(horizontal_rotation) * cos_x);
            y += f64::from(papokin_util::math::sin(vertical_rotation));
            z += f64::from(papokin_util::math::sin(horizontal_rotation) * cos_x);
    ...
    pub fn carve_block(
        ...
        if current_state == papokin_data::Block::BEDROCK.default_state.id {
            return false;
        }
        let block = papokin_data::Block::from_state_id(current_state);

        if block.id == papokin_data::Block::GRASS_BLOCK.id
            || block.id == papokin_data::Block::MYCELIUM.id
        {
            *has_grass = true;
        }
...
mod tests {
    use super::*;
    use papokin_data::carver::CAVE;
    use papokin_data::{Block, BlockStateId, dimension::Dimension};
    ...
    fn cave_carve() {
        ...
        let mut random = RandomGenerator::Legacy(
            papokin_util::random::legacy_rand::LegacyRand::from_seed(12345),
        );
    ...
    fn carve_at(
        ...
        y: i32,
        z: i32,
        initial_state: &'static papokin_data::BlockState,
    ...
    fn find_aquifer_carve_state(
        run: &mut Run,
        predicate: impl Fn(&'static papokin_data::BlockState, bool) -> bool,
    ) -> Option<(i32, i32, i32, &'static papokin_data::BlockState)> {
        for y in -64..=63 {
            for x in 0..16 {
                for z in 0..16 {
