...
use papokin_data::block_state::BlockState;
use papokin_data::carver::{CarverAdditionalConfig, CarverConfig};
#[cfg(test)]
use papokin_data::dimension::Dimension;
use papokin_data::fluid::Fluid;
use papokin_data::material_rule::MaterialRule;
use papokin_util::math::vector2::Vector2;
use papokin_util::math::vector3::Vector3;
use papokin_util::random::{RandomGenerator, RandomImpl};
...
impl CarverBlockIds {
    ...
    pub fn new() -> Self {
        Self {
            air: papokin_data::Block::AIR.default_state,
            cave_air: papokin_data::Block::CAVE_AIR.default_state,
            lava: papokin_data::Block::LAVA.default_state,
            dirt: papokin_data::Block::DIRT.default_state,
            grass_block: papokin_data::Block::GRASS_BLOCK.default_state,
            mycelium: papokin_data::Block::MYCELIUM.default_state,
        }
    }
}
...
pub fn carve(chunk: &mut ProtoChunk, generator: &VanillaGenerator) {
    // 原版 applyCarvers 使用的范围为 8 个区块（17x17 区域）
    let radius = 8;
    let chunk_x = chunk.x;
    let chunk_z = chunk.z;
...
fn apply_carving_mask(run: &mut CarveRun, mask: &mask::CarvingMask) {
    ...
    let bedrock_id = papokin_data::Block::BEDROCK.default_state.id;
    let grass_block_id = papokin_data::Block::GRASS_BLOCK.default_state.id;
    let mycelium_id = papokin_data::Block::MYCELIUM.default_state.id;
...
fn get_large_feature_seed(seed: u64, chunk_x: i32, chunk_z: i32) -> u64 {
    let mut random = papokin_util::random::legacy_rand::LegacyRand::from_seed(seed);
...
const fn new_carver_random(seed: u64, non_vanilla_random: bool) -> RandomGenerator {
    if non_vanilla_random {
        RandomGenerator::Xoroshiro(papokin_util::random::xoroshiro128::Xoroshiro::from_seed(
            seed,
        ))
    } else {
        RandomGenerator::Legacy(papokin_util::random::legacy_rand::LegacyRand::from_seed(
            seed,
        ))
    }
}
...
fn schedule_fluid_tick_for_state(
    ...
    if state.id == papokin_data::Block::WATER.default_state.id {
        chunk.schedule_fluid_tick(x, y, z, &Fluid::WATER);
    } else if state.id == papokin_data::Block::LAVA.default_state.id {
...
mod tests {
    ...
    use papokin_data::material_rule::{
    ...
    fn biome_carvers_driven_by_data() {
        ...
        use papokin_data::carver::{CANYON, CAVE, CAVE_EXTRA_UNDERGROUND, NETHER_CAVE};
