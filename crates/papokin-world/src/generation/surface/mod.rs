...
pub struct MaterialRuleContext<'a> {
    ...
    /// 由 [`test_noise_threshold`] 构建的噪声采样器，以噪声参数 id 为键。
    ///
    /// 对于生成的常量，`DoublePerlinNoiseParameters::id` 唯一标识
    /// 完整参数集；生成的表面规则只引用这些常量。
    ///
    /// 构建开销很大且只依赖 `random_deriver`，因此它们会
    /// 在上下文的整个生命周期内复用。表面规则仅引用极少量
    /// 种不同噪声，因此线性扫描在这里优于哈希。
...
impl<'a> MaterialRuleContext<'a> {
    pub const fn new(
        ...
        const HORIZONTAL_POS: i64 = -i64::MAX; // 原版
        Self {
            min_y,
            height,
            ...
            unique_horizontal_pos_value: HORIZONTAL_POS - 1, // 因为是前置递增
...
pub fn test_vertical_gradient(
    ...
    let splitter = context
        ...
        .next_splitter();
    let mapped = papokin_util::math::map(block_y as f32, true_at as f32, false_at as f32, 1.0, 0.0);
