...
pub enum StagedChunkEnum {
    None,
    /// 初始的空区块，已准备好进行生物群系填充
    Empty = 1, // 空的 STRUCTURE_STARTS 与 STRUCTURE_REFERENCES
    /// 已填充生物群系、可进行噪声生成的区块
    Biomes,
    StructureStart,
    StructureReferences,
...
impl StagedChunkEnum {
    ...
    pub const fn get_direct_dependencies(self) -> &'static [Self] {
        match self {
            // 在原版中 StructureStart 最先执行，但由于它需要 Vanilla 中的生物群系，因此会在 StructureStart 中计算，并且
            // 生物群系步骤（Biome Step），这样应该更高效
...
impl Chunk {
    ...
    pub fn get_proto_chunk_mut(&mut self) -> &mut ProtoChunk {
        match self {
            Self::Level(_) => panic!("该区块不是 ProtoChunk"),
    ...
    pub fn upgrade_to_level_chunk(
        ...
        let proto_chunk_box = match std::mem::replace(
            ...
            Self::Level(Arc::new(ChunkData {
                ...
                preserved_data: Mutex::new(None),
            })),
        ) {
            Self::Proto(proto) => proto,
            Self::Level(_) => panic!("无法升级 Level 区块"),
        };
        ...
        for nbt in proto_chunk.pending_block_entities {
                ...
                && let Some(z) = nbt.get_int("z")
            {
                pending_block_entities
                    .insert(papokin_util::math::position::BlockPos::new(x, y, z), nbt);
            }
        }
