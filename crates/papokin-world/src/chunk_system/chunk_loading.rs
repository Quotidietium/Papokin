...
use crate::chunk_system::chunk_state::StagedChunkEnum; // 固定路径
...
pub struct ChunkLoading {
    ...
    pub ticket: HashMapType<ChunkPos, Vec<i8>>, // TODO 生命周期与 id
...
impl ChunkLoading {
    ...
    pub const MAX_LEVEL: i8 = 49; // 等级 49 会被卸载。
    fn debug_check_error(&self) -> bool {
        ...
        for (ticket_pos, levels) in &self.ticket {
            ...
            for r in 0..=range {
                ...
                for &(dx, dy) in papokin_data::chunk_view_lut::get_chebyshev_ring(r) {
        ...
        for val in &temp {
            if val != self.pos_level.get_key_value(val.0).expect("键值应存在") {
            ...
            assert_eq!(
                val,
                self.pos_level.get_key_value(val.0).expect("键值应存在")
            );
        }
        true
    }
