...
/// 在单个同步旧式 guest 调用进行期间可用的能力。
///
/// Store 数据只能被同步闭包借用。其底层
/// Wasmtime Store 上下文绝不会暴露到此 crate 之外。
    ...
    P: StorePolicy,
{
    ...
    /// 将一个持有所有权的 Store 操作加入队列。一旦被接受，该操作即使
    /// 如果调用方停止等待其结果。
...
where
    T: Send + 'static,
{
    /// 将同步的 guest 调用加入队列，或经由活跃的宿主进行路由
    /// 当此 Store 已出现在当前因果链中时的栈帧。
    ...
    P: StorePolicy,
{
    ...
    /// 停止接收、排空已接受的工作，但不执行最终的 Store
    /// 操作。
    ///
    /// 这适用于其所有者已准备但从未被接纳的 Store
    /// 其更高层的生命周期。若两者皆无便丢弃执行器，
    /// 调用此方法或完成 [`Self::shutdown`] 仍然是错误。
    pub fn discard(&self) {
...
where
    T: Send + 'static,
{
    /// 停止接收、排空已接受的工作、执行一次最终的 Store 操作，
    /// 排空 Wasmtime 任务，并丢弃 Store。
