...
impl ItemBehaviour for TridentItem {
    fn normal_use(&self, _item: &Item, player: &Player) {
        ...
        player
            .living_entity
            .set_active_hand(papokin_util::Hand::Right, stack, 72000);
    }
    ...
    fn on_stopped_using(&self, _stack: &ItemStack, player: &Player) {
        ...
        if let Some(enchantments) =
            stack_guard.get_data_component::<papokin_data::data_component_impl::EnchantmentsImpl>()
        {
            for (enchantment, level) in enchantments.enchantment.iter() {
                if **enchantment == papokin_data::Enchantment::RIPTIDE {
        ...
        if riptide_level > 0 {
            let in_water = world.get_block_state(&player.position().to_block_pos()).id
                == papokin_data::Block::WATER.default_state.id;
            if !in_water {
        ...
        if player.gamemode.load() != GameMode::Creative {
            ...
            if main_hand_item.item.id == Item::TRIDENT.id {
                ...
                let off_hand_slot =
                    papokin_inventory::player::player_inventory::PlayerInventory::OFF_HAND_SLOT;
