...
use papokin_data::data_component_impl::ChargedProjectilesImpl;
...
impl ItemBehaviour for CrossbowItem {
    fn normal_use(&self, _item: &Item, player: &Player) {
        ...
        // 每把弩默认都携带 ChargedProjectiles 组件，因此仅
        // 存在并不意味着弩已装填。原版还会检查列表是否
        // 非空（CrossbowItem.java:68）。
        if stack
        ...
        if use_ticks >= charge_time {
            ...
            if arrow_slot.is_some() || is_creative {
                ...
                if !drawn.is_empty() {
                    ...
                    for item in drawn {
                        let mut arrow_nbt = papokin_nbt::compound::NbtCompound::new();
