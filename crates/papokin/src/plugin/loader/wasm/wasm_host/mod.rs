...
use papokin_plugin_runtime::RuntimeSpawner;
...
impl SocketPolicy {
    const fn allows(self, addr: SocketAddr, reason: SocketAddrUse) -> bool {
        // Wasmtime 在出站 connect/send 之前执行通配符绑定。
        // 此处只放行该前兆；稍后的目的地检查仍会
        // 强制执行实际的协议与回环策略。
...
impl PluginRuntime {
    ...
    pub async fn init_plugin<P: AsRef<Path>>(
            ...
            |bytes, path_str| {
                ...
                if verification.is_signed && verification.is_valid {
                    verification.metadata.map(|m| {
                        wit::v0_1::papokin::plugin::context::MarketplaceMetadata {
        ...
        let wasm_bytes = signature::strip_papokin_sections(&wasm_bytes).unwrap_or(wasm_bytes);

        // JIT 编译（及缓存反序列化）受 CPU 限制，可能耗时
        // 分钟（对大型调试构建而言）；不要把它放到异步工作线程上
        // 这样热重载就无法拖住服务器运行时。
...
fn load_component(
    ...
    if cache_path.exists() {
        // SAFETY: 缓存文件由 Wasmtime 针对此确切的 WASM 二进制哈希和包版本生成。
    ...
    // 编译后的组件已经拿到；缓存写入失败（例如
    // 并发加载方持有该文件）不得导致插件加载失败。
    match component.serialize() {
        Ok(serialized) => {
            if let Err(error) = fs::write(&cache_path, serialized) {
                tracing::warn!(%error, "写入插件 JIT 缓存失败；将在没有缓存的情况下继续");
            }
        }
        Err(error) => {
            tracing::warn!(%error, "序列化插件以写入 JIT 缓存失败；将在没有缓存的情况下继续");
        }
    }
    Ok(component)
}
...
impl WasmPlugin {
    ...
    /// 使用全新的上下文资源调用 guest 的 `on-enable` 导出。
    ...
    /// 使用全新的上下文资源调用 guest 的 `on-disable` 导出。
    pub async fn on_disable(
        &self,
        context: Arc<Context>,
    ) -> Result<Result<(), String>, wasmtime::Error> {
        // Bukkit 语义：禁用插件会取消其已调度的任务。
        // （Unload 在 `on_unload` 中做同样处理；禁用但不卸载必须
        // 不会让插件的任务继续运行。）
    ...
    async fn cancel_scheduled_tasks(&self, server: &Arc<crate::server::Server>) {
        ...
        match loaded_plugin {
            ...
            Ok(None) => tracing::debug!("插件实例已不存在；没有需要取消的任务"),
            Err(error) => {
                tracing::warn!(%error, "无法解析插件以取消任务");
            }
        }
    }
    ...
    /// 调用 guest 的 `handle-plugin-message` 导出。
