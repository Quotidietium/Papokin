...
use papokin_data::entity::EntityPose as InternalEntityPose;
...
impl HostEntity for PluginHostState {
    ...
    async fn set_velocity(
        ...
        entity
            .get_entity()
            .velocity
            .store(papokin_util::math::vector3::Vector3::new(
    ...
    async fn get_custom_name(
        ...
        if let Some(name) = name.as_ref() {
            Ok(Some(self.add_text_component(name.clone()).map_err(
                |_| wasmtime::Error::msg("添加文本组件资源失败"),
            )?))
        } else {
            Ok(None)
        }
    ...
    async fn get_nearby_entities(
        ...
        let box_range = papokin_util::math::boundingbox::BoundingBox::new(
    ...
    async fn create_snapshot(&mut self, this: Resource<Entity>) -> wasmtime::Result<Vec<u8>> {
        ...
        Ok(papokin_nbt::Nbt::from(nbt).write_unnamed().to_vec())
    }
...
impl
    crate::plugin::loader::wasm::wasm_host::wit::v0_1::papokin::plugin::world::HostEntityWithStore<
        PluginHostState,
...
pub(crate) fn to_wit_entity_type(name: &str) -> wasmtime::Result<entity_types::EntityType> {
    ...
    let index = papokin_data::entity::EntityType::ALL
...
) -> wasmtime::Result<&'static papokin_data::entity::EntityType> {
    let index = entity_type as usize;
    papokin_data::entity::EntityType::ALL
        .get(index)
        .copied()
        .ok_or_else(|| wasmtime::Error::msg(format!("无效的实体类型索引：{index}")))
}
