...
const fn from_wasm_hand(hand: WitHand) -> papokin_util::Hand {
    match hand {
        WitHand::Right => papokin_util::Hand::Right,
        WitHand::Left => papokin_util::Hand::Left,
    }
}
        ...
        match provider {
            ...
            InventoryProvider::PlayerMain(player) => {
                for slot in 0..36 {
                    player
                        .inventory()
                        .set_stack(slot, papokin_data::item_stack::ItemStack::EMPTY.clone());
                    let empty_serializer = ItemStackSerializer::from(
                        papokin_data::item_stack::ItemStack::EMPTY.clone(),
                    );
                    let packet = CSetContainerSlot::new(0, 0, slot as i16, &empty_serializer);
