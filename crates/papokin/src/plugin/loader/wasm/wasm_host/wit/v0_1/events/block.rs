    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockIgniteEvent(data) => Self {
                block_pos: from_wasm_block_position(data.block_pos),
                igniting_block: &papokin_data::Block::FIRE,
                player: None,
                cancelled: data.cancelled,
            },
            _ => panic!("意外的事件类型"),
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockFromToEvent(data) => Self {
                ...
                block: &papokin_data::Block::WATER,
                cancelled: data.cancelled,
            },
            _ => panic!("意外的事件类型"),
        }
...
impl ToFromWasmEvent for BellRingEvent {
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BellRingEvent(_) => panic!("无法从 WASM 构造 BellRingEvent"),
            _ => panic!("意外的事件类型"),
        }
    }
}
...
impl ToFromWasmEvent for BlockBrushEvent {
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockBrushEvent(_) => panic!("无法从 WASM 构造 BlockBrushEvent"),
            _ => panic!("意外的事件类型"),
        }
    }
}
...
impl ToFromWasmEvent for BlockCookEvent {
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockCookEvent(_) => panic!("无法从 WASM 构造 BlockCookEvent"),
            _ => panic!("意外的事件类型"),
        }
    }
}
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockDamageAbortEvent(_) => {
                panic!("无法从 WASM 构造 BlockDamageAbortEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockDispenseArmorEvent(_) => {
                panic!("无法从 WASM 构造 BlockDispenseArmorEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockDispenseLootEvent(_) => {
                panic!("无法从 WASM 构造 BlockDispenseLootEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
...
impl ToFromWasmEvent for BlockDropItemEvent {
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockDropItemEvent(_) => panic!("无法从 WASM 构造 BlockDropItemEvent"),
            _ => panic!("意外的事件类型"),
        }
    }
}
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockFertilizeEvent(_) => {
                panic!("无法从 WASM 构造 BlockFertilizeEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockMultiPlaceEvent(_) => {
                panic!("无法从 WASM 构造 BlockMultiPlaceEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockReceiveGameEvent(_) => {
                panic!("无法从 WASM 构造 BlockReceiveGameEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockShearEntityEvent(_) => {
                panic!("无法从 WASM 构造 BlockShearEntityEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
...
impl ToFromWasmEvent for CampfireStartEvent {
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::CampfireStartEvent(_) => panic!("无法从 WASM 构造 CampfireStartEvent"),
            _ => panic!("意外的事件类型"),
        }
    }
}
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::CauldronLevelChangeEvent(_) => {
                panic!("无法从 WASM 构造 CauldronLevelChangeEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
...
impl ToFromWasmEvent for CrafterCraftEvent {
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::CrafterCraftEvent(_) => panic!("无法从 WASM 构造 CrafterCraftEvent"),
            _ => panic!("意外的事件类型"),
        }
    }
}
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::EntityBlockFormEvent(_) => {
                panic!("无法从 WASM 构造 EntityBlockFormEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::VaultDisplayItemEvent(_) => {
                panic!("无法从 WASM 构造 VaultDisplayItemEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
