    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::EntityDamageEvent(data) => Self {
                ...
                // WIT 枚举只列出了原版伤害类型的名称；插件处理
                // 通过 `damage-by-name` 造成的自定义伤害不会
                // 经由此转换完成往返。
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::EntityTeleportEvent(data) => Self {
                entity_id: data.entity_id,
                from_position: papokin_util::math::vector3::Vector3::new(
                    data.from_position.0,
                    data.from_position.1,
                    data.from_position.2,
                ),
                to_position: papokin_util::math::vector3::Vector3::new(
                    data.to_position.0,
                    data.to_position.1,
                    data.to_position.2,
                ),
...
impl ToFromWasmEvent for PiglinBarterEvent {
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::PiglinBarterEvent(_) => panic!("无法从 WASM 构造 PiglinBarterEvent"),
            _ => panic!("意外的事件类型"),
        }
    }
}
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::WitchConsumePotionEvent(_) => {
                panic!("无法从 WASM 构造 WitchConsumePotionEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::WitchReadyPotionEvent(_) => {
                panic!("无法从 WASM 构造 WitchReadyPotionEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::WitchThrowPotionEvent(_) => {
                panic!("无法从 WASM 构造 WitchThrowPotionEvent")
            }
            _ => panic!("意外的事件类型"),
        }
    }
