...
fn lookup_table(key: &str) -> Result<&'static papokin_util::loot_table::LootTable, String> {
    papokin_data::loot_table::get_loot_table(key).ok_or_else(|| format!("未知的战利品表：{key}"))
}

impl PluginHostState {
    fn stacks_to_wit(
        &mut self,
        items: Vec<papokin_data::item_stack::ItemStack>,
...
impl Host for PluginHostState {
    async fn has_loot_table(&mut self, key: String) -> wasmtime::Result<bool> {
        Ok(papokin_data::loot_table::get_loot_table(&key).is_some())
    }
    ...
    async fn fill_inventory(
        ...
        let inventory: Arc<dyn papokin_inventory::Inventory> = match provider {
            InventoryProvider::Generic(inventory) => inventory,
            // 玩家物品栏需要逐槽位的客户端同步数据包，而
            // 原版箱子填充路径不会产生的；拒绝它们，而不是
            // 导致客户端静默失去同步。
            InventoryProvider::PlayerMain(_) | InventoryProvider::PlayerEnderChest(_) => {
                return Ok(Err("fill-inventory 仅支持通用容器物品栏".to_string()));
            }
        };
