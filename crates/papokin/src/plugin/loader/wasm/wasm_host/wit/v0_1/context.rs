...
fn register_player_event(
    ...
    match event_type {
        ...
        _ => {
            tracing::error!("非玩家事件不应路由到 register_player_event");
        }
    }
}
...
impl PluginHostState {
    ...
    fn take_command(&mut self, res: &Resource<Command>) -> wasmtime::Result<CommandResource> {
        self.resource_table
            ...
            // 将 ResourceTableError 转换为 wasmtime::Error
            .map_err(wasmtime::Error::from)
    }
}
...
fn register_entity_event(
    ...
    match event_type {
        ...
        _ => {
            tracing::error!("非实体事件不应路由到 register_entity_event");
        }
    }
}
...
fn register_inventory_event(
    ...
    match event_type {
        ...
        _ => {
            tracing::error!("非物品栏事件不应路由到 register_inventory_event");
        }
    }
}
...
fn register_vehicle_event(
    ...
    match event_type {
        ...
        _ => {
            tracing::error!("非载具事件不应路由到 register_vehicle_event");
        }
    }
}
...
fn register_enchantment_event(
    ...
    match event_type {
        ...
        _ => {
            tracing::error!("非附魔事件不应路由到 register_enchantment_event");
        }
    }
}
...
fn register_world_event(
    ...
    match event_type {
        ...
        _ => {
            tracing::error!("非世界事件不应路由到 register_world_event");
        }
    }
}
...
fn register_block_event(
    ...
    match event_type {
        ...
        _ => {
            tracing::error!("非方块事件不应路由到 register_block_event");
        }
    }
}
...
fn register_raid_event(
    ...
    match event_type {
        ...
        _ => {
            tracing::error!("非袭击事件不应路由到 register_raid_event");
        }
    }
}
...
fn register_dialog_event(
    ...
    match event_type {
        ...
        _ => {
            tracing::error!("非对话框事件不应路由到 register_dialog_event");
        }
    }
}
...
fn register_server_event(
    ...
    match event_type {
        ...
        _ => {
            tracing::error!("非服务器事件不应路由到 register_server_event");
        }
    }
}
...
fn register_hanging_event(
    ...
    match event_type {
        ...
        _ => {
            tracing::error!("非悬挂实体事件不应路由到 register_hanging_event");
        }
    }
}
...
impl papokin::plugin::context::Host for PluginHostState {}

impl papokin::plugin::context::HostContext for PluginHostState {
    #[allow(clippy::too_many_lines)]
    async fn register_event(
        ...
        // 若插件已被丢弃或上下文句柄已失效，则使用 ? 捕获
        let plugin = self
            .plugin
            .as_ref()
            .ok_or_else(|| wasmtime::Error::msg("插件状态未初始化"))?
            .upgrade()
    ...
    async fn register_permission(
        ...
        let util_permission = papokin_util::permission::Permission {
            node: permission.node,
            description: permission.description,
            default: match permission.default {
                PermissionDefault::Deny => papokin_util::permission::PermissionDefault::Deny,
                PermissionDefault::Allow => papokin_util::permission::PermissionDefault::Allow,
                PermissionDefault::Op(lvl) => {
                    papokin_util::permission::PermissionDefault::Op(match lvl {
                        PermissionLevel::Zero => papokin_util::permission::PermissionLvl::Zero,
                        PermissionLevel::One => papokin_util::permission::PermissionLvl::One,
                        PermissionLevel::Two => papokin_util::permission::PermissionLvl::Two,
                        PermissionLevel::Three => papokin_util::permission::PermissionLvl::Three,
                        PermissionLevel::Four => papokin_util::permission::PermissionLvl::Four,
                    })
                }
            },
            children,
    ...
    async fn get_server(
        ...
        self.add_server(server_provider)
            .map_err(|_| wasmtime::Error::msg("添加服务器资源失败"))
    }
