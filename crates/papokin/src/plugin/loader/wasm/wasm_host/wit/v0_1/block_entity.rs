...
impl papokin::plugin::block_entity::Host for PluginHostState {}
...
fn block_entity_from_resource(
    ...
    state
        ...
        .map_err(|_| wasmtime::Error::msg("无效的方块实体资源句柄"))
...
fn from_wasm_sign_text(text: SignText) -> InternalText {
    ...
    InternalText::from(papokin_nbt::tag::NbtTag::Compound({
        ...
        nbt.put_list(
            ...
            messages
                .iter()
                .map(|s| papokin_nbt::tag::NbtTag::String(s.clone().into()))
                .collect(),
        );
        nbt
    }))
...
fn get_container_from_be<T>(
    ...
    entity.get_inventory().map_or_else(
        || Err(wasmtime::Error::msg("方块实体物品栏不可用")),
...
impl HostContainerBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let container = self
            ...
            .map_err(|_| wasmtime::Error::msg("无效的 container 方块实体资源句柄"))?;
    ...
    async fn set_stack(
        ...
        let stack = match stack_res {
            ...
            None => papokin_data::item_stack::ItemStack::EMPTY.clone(),
        };
...
impl HostCommandBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            ...
            .map_err(|_| wasmtime::Error::msg("无效的 command 方块实体资源句柄"))?;
    ...
    async fn last_output(&mut self, res: Resource<CommandBlockEntity>) -> wasmtime::Result<String> {
        ...
        entity
            ...
            .map_or_else(
                || Err(wasmtime::Error::msg("不是 command 方块实体")),
                |cmd| {
                    Ok(cmd
                        .last_output
                        .lock()
...
impl HostSignBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 sign 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
    ...
    async fn get_front_text(
        ...
        entity
            ...
            .map_or_else(
                || Err(wasmtime::Error::msg("不是 sign 方块实体")),
...
impl HostJukeboxBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 jukebox 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
    ...
    async fn is_playing(&mut self, res: Resource<JukeboxBlockEntity>) -> wasmtime::Result<bool> {
        ...
        entity
            ...
            .map_or_else(
                || Err(wasmtime::Error::msg("不是 jukebox 方块实体")),
...
impl HostChestBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 chest 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
    ...
    async fn viewer_count(&mut self, res: Resource<ChestBlockEntity>) -> wasmtime::Result<u32> {
        ...
        entity
            ...
            .map_or_else(
                || Err(wasmtime::Error::msg("不是 chest 方块实体")),
...
impl HostMobSpawnerBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 mob spawner 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
    ...
    async fn get_spawn_count(
        ...
        entity
            ...
            .map_or_else(
                || Err(wasmtime::Error::msg("不是 mob spawner 方块实体")),
...
impl HostMapBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 map 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
    ...
    async fn get_map_id(&mut self, res: Resource<MapBlockEntity>) -> wasmtime::Result<i32> {
        ...
        entity
            ...
            .map_or_else(
                || Err(wasmtime::Error::msg("不是 map 方块实体")),
    ...
    async fn set_map_id(
        ...
        let map_be = entity
            .as_any()
            .downcast_ref::<InternalMapBlockEntity>()
            .ok_or_else(|| wasmtime::Error::msg("不是 map 方块实体"))?;
    ...
    async fn update(&mut self, res: Resource<MapBlockEntity>) -> wasmtime::Result<()> {
        ...
        let server = self
            .server
            .as_ref()
            .ok_or_else(|| wasmtime::Error::msg("服务器上下文未设置"))?;
...
impl HostHangingSignBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 hanging sign 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
    ...
    async fn get_front_text(
        ...
        entity
            ...
            .map_or_else(
                || Err(wasmtime::Error::msg("不是 hanging sign 方块实体")),
                |sign| Ok(to_wasm_sign_text(&sign.front_text)),
            )
    }
...
impl HostTrappedChestBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 trapped chest 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
    ...
    async fn viewer_count(
        ...
        entity
            ...
            .map_or_else(
                || Err(wasmtime::Error::msg("不是 trapped chest 方块实体")),
                |chest| Ok(chest.get_viewer_count() as u32),
            )
    }
        ...
        impl $trait_name for PluginHostState {
            async fn get_block_entity(
                ...
                let entity = self
                    ...
                    .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
                    .map_err(|_| {
                        wasmtime::Error::msg(concat!("无效的 ", $name_str, " 资源句柄"))
                    })?;
                let provider = entity.provider.clone();
                self.add_block_entity(provider)
            }
...
impl HostBannerBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 banner 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostBarrelBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 barrel 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostBeaconBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 beacon 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostBeehiveBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 beehive 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostBellBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 bell 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostBrewingStandBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 brewing stand 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostChiseledBookshelfBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 chiseled bookshelf 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostComparatorBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 comparator 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostCrafterBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 crafter 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostCreakingHeartBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 creaking heart 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostEndGatewayBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 end gateway 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostEnderChestBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 ender chest 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostShulkerBoxBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 shulker box 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostHopperBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 hopper 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostJigsawBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 jigsaw 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostLecternBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 lectern 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostPistonBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 piston 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostSculkShriekerBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 sculk shrieker 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostSkullBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 skull 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
...
impl HostStructureBlockBlockEntity for PluginHostState {
    async fn get_block_entity(
        ...
        let entity = self
            .resource_table
            .get::<BlockEntityResource>(&Resource::new_own(res.rep()))
            .map_err(|_| wasmtime::Error::msg("无效的 structure block 方块实体资源句柄"))?;
        let provider = entity.provider.clone();
        self.add_block_entity(provider)
    }
