...
fn merge_toml(defaults: &str, existing: &str) -> Result<String, String> {
    ...
    let defaults: Value = toml::from_str(defaults).map_err(|e| format!("无效的默认配置：{e}"))?;
    let existing: Value = toml::from_str(existing).map_err(|e| format!("无效的配置文件：{e}"))?;
    let merged = merge_values(defaults, existing);
    match merged {
        table @ Value::Table(_) => {
            toml::to_string(&table).map_err(|e| format!("序列化配置失败：{e}"))
        }
        scalar => Ok(scalar.to_string()),
    }
}

/// 通过同目录临时文件加重命名将 `content` 写入 `path`，因此崩溃时
/// 写入中途绝不会留下被截断的配置。
fn write_config_atomic(path: &std::path::Path, content: &str) -> Result<(), String> {
    ...
    std::fs::write(&tmp_path, content).map_err(|e| format!("写入配置失败：{e}"))?;
    std::fs::rename(&tmp_path, path).map_err(|e| format!("替换配置失败：{e}"))
}

#[allow(clippy::unused_async_trait_impl)]
impl papokin::plugin::config::Host for PluginHostState {
    ...
    async fn save_config(&mut self, content: String) -> wasmtime::Result<Result<(), String>> {
        ...
        if !content.trim().is_empty() && toml::from_str::<Value>(&content).is_err() {
            return Ok(Err("内容不是有效的 TOML".to_string()));
        }
