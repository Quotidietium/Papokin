...
use crate::{
    ...
    plugin::loader::wasm::wasm_host::{
        ...
        wit::v0_1::papokin::plugin::command::{CommandError as CommandErrorWit, SuggestionRequest},
    },
    server::Server,
};
    ...
    match result {
        ...
        Err(CommandErrorWit::InvalidConsumption(value)) => Err(DISPATCHER_PARSE_EXCEPTION
            .create_without_context(TextComponent::text(format!(
                "无效的 consumption：{value:?}"
            )))),
        Err(CommandErrorWit::InvalidRequirement) => Err(DISPATCHER_PARSE_EXCEPTION
            .create_without_context(TextComponent::text("无效的要求条件"))),
                ...
                self.plugin
                    ...
                    .map_err(|error| {
                        DISPATCHER_PARSE_EXCEPTION.create_without_context(
                            TextComponent::text(format!("Wasm 命令失败，错误如下：{error}"))
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                    ...
                    .await
                {
                    ...
                    Err(error) => {
                        tracing::error!("Wasm 命令补全失败：{error}");
                        Suggestions::empty()
                    }
                }
            })
