...
use papokin_data::packet::{CURRENT_MC_VERSION, LOWEST_SUPPORTED_MC_VERSION};
...
impl CachedBranding {
    /// 向客户端通告的服务器品牌字符串（例如在 `minecraft:brand` 插件消息中）。
    pub const BRAND: &'static str = "Papokin";
        ...
        let favicon = if config.use_favicon {
            config.favicon_path.as_ref().map_or_else(
                ...
                |icon_path| {
                    ...
                    {
                        warn!("图标不是 PNG 图片，将使用默认图标。");
                        return Some(load_icon_from_bytes(DEFAULT_ICON));
                    }
                    debug!("正在尝试从 '{icon_path}' 加载服务器图标");
                    ...
                    match load_icon_from_file(icon_path) {
                        ...
                        Err(e) => {
                            ...
                            warn!("从 '{icon_path}' 加载图标失败：{error_message}");
