...
pub use papokin_gametest::{GameTestBatchReport, GameTestRetryOptions};
use papokin_nbt::NbtCompound;
use papokin_util::math::{position::BlockPos, vector2::Vector2};
use papokin_util::text::TextComponent;
use papokin_world::{chunk::ChunkHeightmapType, world::BlockFlags};
...
/// `GameTest` 结构必须让其区块保持加载并持续运行刻，就像原版的
/// `TestInstanceBlockEntity::forceLoadChunks`。保留一个小的引用计数，以便
/// 重叠的测试共享同一强制加载租约和既有的 `/forceload`
/// 区块永远不会被 `GameTest` 运行时释放。
/// 唯一标识 `GameTest` 请求的强制加载区块的键。
type ForcedGameTestChunkKey = (uuid::Uuid, i32, i32);

/// 由活跃 `GameTest` 强制加载的区块的引用计数映射。
type ForcedGameTestChunkMap = StdMutex<HashMap<ForcedGameTestChunkKey, ForcedGameTestChunk>>;

/// 由 `GameTest` 运行时强制加载的区块的全局注册表。
...
pub async fn stop_game_tests() {
    // 发布停止请求期间保持队列互斥锁。drain_game_test_queue
    // 在消费 STOP_GAME_TESTS 之前会先获取同一把互斥锁，因此"停止+新运行"命令
    // 在 runner 清理与队列排空两个阶段之间不存在竞态。
...
pub(super) async fn drain_game_test_queue(server: &Arc<Server>, runner: &mut GameTestRunner) {
    // 在消费停止信号时持有 stop_game_tests 使用的同一队列互斥锁
    // 标志并排空请求。这消除了新的 /test 运行会碰上的异步竞态
    // 否则可能在旧 runner 被清理前就被排空。
    let queued = {
        let mut queue = GAME_TEST_QUEUE.lock().await;
    ...
    for request in queued {
        ...
        match prepare_test_run(server, request).await {
            Ok(run) => {
                info!(target: "papokin::gametest", test = %test_id, "Starting queued GameTest");
                runner.enqueue(run);
            }
            Err(error) => {
                warn!(
...
struct ServerGameTestWorld {
    world: Arc<World>,
    /// 此逻辑测试在其中持有一个引用的区块
    /// `FORCED_GAME_TEST_CHUNKS`。同一个适配器会在复制重置过程中保留
    /// 重试，因此这些区块在整个重试生命周期内保持活跃。
...
impl ServerGameTestWorld {
    ...
    async fn ensure_chunk_loaded(&self, position: &BlockPos) {
        ...
        // World::set_block_state 特意设计为同步，且只修改
        // 已加载区块。原版会强制加载完整的 GameTest 结构
        // 在放置之前；让异步 GameTest 适配器提供该保证。
...
impl GameTestWorld for ServerGameTestWorld {
    ...
    async fn set_block_entity_nbt(
        ...
        // 这通常跟在对同一位置调用的 set_block_state 之后，但保留
        // 这里的保证也让方块实体的放置独立地保持正确。
        self.ensure_chunk_loaded(position).await;
    ...
    async fn clear_non_player_entities(
        ...
        // World::entities 有意排除玩家，以匹配原版的
        // `removeEntities` 过滤器，同时完全避开任何移除玩家的路径。
