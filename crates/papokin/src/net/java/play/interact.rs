...
impl JavaClient {
    ...
    pub fn handle_interact(
        ...
        let Ok(action) = ActionType::try_from(interact.r#type.0) else {
            self.try_kick(&TextComponent::text("无效的动作类型"));
            return;
        };
                    ...
                    match event.action {
                        ActionType::Attack => {
                            ...
                            if entity_id.0 == player.entity_id() {
                                self.try_kick(&TextComponent::translate(translation::java::MULTIPLAYER_DISCONNECT_INVALID_ENTITY_ATTACKED, []));
                                return;
                            }
