...
/// 将 NBT 复合标签写入写入器，格式会适配目标 Minecraft 版本。
pub fn write_compound_nbt(
    mut write: impl Write,
    comp: &papokin_nbt::compound::NbtCompound,
    version: JavaMinecraftVersion,
) -> Result<(), WritingError> {
    write.write_compound_nbt_with_version(Some(comp), &version)
}

/// 从光照容器中检索 2048 字节的半字节数组，若为空则以 `default_val` 填充。
#[must_use]
...
/// 将条目按位打包且不跨越 64 位边界（Minecraft 1.16+ 格式）。
#[must_use]
...
/// 将条目按位打包并允许跨越 64 位边界（Minecraft 1.9 至 1.15.2 的旧版格式）。
#[must_use]
