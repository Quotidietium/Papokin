...
use papokin_config::networking::proxy::VineConfig;
...
/// 通过发送 `CLoginPluginRequest` 发起 Vine 现代转发握手
/// 使用唯一的 16 字节挑战随机数来防范重放攻击。
...
/// 从配置派生 Ed25519 验证（公）钥。
/// 若 `public_key` 以十六进制提供，则直接解析（32 字节）。
/// 若提供了 `secret`，则计算 SHA-256 以派生 32 字节的 Ed25519 种子
/// 并获取相应的公钥。
...
pub fn receive_vine_plugin_response(
    ...
    if let Some(data) = response.data {
        ...
        // 5. GameProfile（UUID、名称、Properties）
...
mod tests {
    ...
    fn vine_plugin_response_verification() {
        ...
        VarInt(0).encode(&mut payload).unwrap(); // 0 个属性
