...
use papokin_data::block_properties::{BedPart, WhiteBedLikeProperties as BedProperties};
use papokin_data::effect::StatusEffect;
use papokin_data::entity::{EntityPose, EntityType};
use papokin_data::item::Item;
use papokin_data::item_stack::ItemStack;
use papokin_data::potion::Effect;
use papokin_data::tag::{Enchantment as EnchantmentTag, Taggable};
use papokin_data::tracked_data;
use papokin_inventory::SimpleInventory;
...
use papokin_protocol::java::client::play::{CMerchantOffers, Metadata};
...
fn enchanted_book_offer_items(
    ...
    use papokin_data::data_component_impl::StoredEnchantmentsImpl;
...
fn enchant_trade_item(
    ...
    use papokin_data::data_component_impl::EnchantableImpl;
    use rand::RngExt;
...
pub(crate) fn apply_random_dye(rng: &mut impl rand::Rng, stack: &mut ItemStack) {
    use papokin_data::data_component::DataComponent;
    use papokin_data::data_component_impl::{DataComponentImpl, DyedColorImpl};
    use rand::RngExt;
...
pub(crate) fn apply_potion(stack: &mut ItemStack, potion_name: &str) {
    use papokin_data::data_component::DataComponent;
    use papokin_data::data_component_impl::{DataComponentImpl, PotionContentsImpl};

    let Some(potion) = papokin_data::potion::Potion::from_name(
        potion_name
...
/// 将交易目的地解析为其结构集、结构、后备地图
/// 名称和装饰。26.3 为其加上 `#` 前缀并重命名了若干项，因此
/// 两种拼写均可接受。
fn explorer_map_target(
    destination: &str,
) -> Option<(
    &'static str,
    papokin_data::structures::StructureKeys,
    &'static str,
    i32,
)> {
...
pub struct VillagerEntity {
    ...
    pub offers: std::sync::Mutex<Vec<papokin_protocol::java::client::play::MerchantOffer>>,
...
impl VillagerEntity {
    ...
    fn create_explorer_map(&self, destination: &str, given: &ItemStack) -> Option<ItemStack> {
        use papokin_data::data_component::DataComponent;
        use papokin_data::data_component_impl::{DataComponentImpl, ItemNameImpl, MapIdImpl};
        use papokin_data::structures::StructureSet;
        use papokin_world::generation::generator::structure_finder::find_nearest_structure_start;
        ...
        // 26.3 交易中发放一个专用的地图物品，其已自带相关数据
        // 名称；旧数据交易的是普通已填充地图，必须在此处命名。
    ...
    pub fn add_trades(&self, profession: VillagerProfession, level: i32) {
        use papokin_data::villager::VillagerTradeModifier;
            ...
            while added < trade_set.amount && !remaining_trades.is_empty() {
                ...
                match trade.modifier {
                    ...
                    VillagerTradeModifier::RandomPotion => {
                        let Some(potion_name) = papokin_data::tag::Potion::MINECRAFT_TRADEABLE
                            .0
                            .choose(&mut rng)
                        else {
                            continue;
                ...
                new_offers.push(papokin_protocol::java::client::play::MerchantOffer {
    ...
    fn complete_trade(&self, offer_index: usize, world: &Arc<World>, player_uuid: Uuid) {
        ...
        self.xp.fetch_add(xp_gain, Ordering::Relaxed);
    ...
    /// 将当前交易报价重新发送给当前正在交易的玩家
    /// 此村民的交易列表（如有）（Java `CMerchantOffers`）。
    ...
    ///返回该村民当前交易报价的快照。
    pub fn trade_offers(&self) -> Vec<papokin_protocol::java::client::play::MerchantOffer> {
        self.offers
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .clone()
    ...
    /// 替换该村民的交易提议，并将其重新发送给进行交易的
    /// 玩家（如有）。
    pub fn set_trade_offers(
        &self,
        new_offers: Vec<papokin_protocol::java::client::play::MerchantOffer>,
    ) {
        *self
            .offers
            .lock()
    ...
    /// 追加一个交易报价，并将报价列表重新发送给正在交易的玩家
    /// 玩家（如有）。
    pub fn add_trade_offer(&self, offer: papokin_protocol::java::client::play::MerchantOffer) {
        self.offers
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .push(offer);
    ...
    /// 移除 `index` 处的交易提议，并将提议列表重新发送给
    /// 与其交易的玩家。当 `index` 越界时返回 `false`。
    ...
    fn work_at_job_site(&self, game_time: i64, day_time: i64, day: i64) {
...

        {
            ...
            if !offers
                .iter()
                .any(papokin_protocol::java::client::play::MerchantOffer::needs_restock)
            {
                return;
            }
            for offer in offers.iter_mut() {
                ...
                < 2.0f64.powi(2)
        {
            ...
            if let Some(claimed_profession) = profession_for_block(block) {
                ...
                world.send_entity_status(
                    self.get_entity(),
                    papokin_data::entity::EntityStatus::VillagerHappy,
                );
                self.job_site_pending.store(false, Ordering::Relaxed);
    ...
    pub fn set_unhappy(&self) {
        ...
        entity
            .world
            .load()
            .send_entity_status(entity, papokin_data::entity::EntityStatus::VillagerAngry);
        entity.play_sound(papokin_data::sound::Sound::EntityVillagerNo);
    }

    pub fn open_trading_screen(&self, player: &Arc<Player>) {
        // 打开商人界面，然后发送当前交易选项的数据包
    ...
    fn send_trade_offers(
        ...
        player: &Player,
        sync_id: u8,
        offers: &[papokin_protocol::java::client::play::MerchantOffer],
        ...
        player.client.try_send_packet(&java);
    }
}
...
impl ScreenHandlerFactory for VillagerEntity {
    fn create_screen_handler(
        &self,
        sync_id: u8,
        player_inventory: &Arc<papokin_inventory::player::player_inventory::PlayerInventory>,
        ...
        handler.on_trade_updated = Some(Box::new(move |has_result| {
                ...
                .is_ok()
            {
                villager.get_entity().play_sound(if has_result {
                    papokin_data::sound::Sound::EntityVillagerYes
                } else {
                    papokin_data::sound::Sound::EntityVillagerNo
                });
            }
        }));
...
impl VillagerEntity {
    ...
    pub fn villager_mob_tick(&self) {
        ...
        if let Some(current_home) = self.get_home_pos() {
            ...
            let valid = if block.has_tag(&papokin_data::tag::Block::MINECRAFT_BEDS) {
            ...
            if !valid {
                *self
                ...
                if is_sleeping {
                    ...
                    self.get_entity().set_synced_data(
                        papokin_data::tracked_data::villager::SLEEPING_POS_ID,
                        None::<BlockPos>,
                    );
                }
            }
        ...
        if self.get_home_pos().is_none() {
            ...
            for entity in nearby_entities {
                if entity.get_entity().entity_id != self.get_entity().entity_id
                    && entity.get_entity().entity_type
                        == &papokin_data::entity::EntityType::VILLAGER
            ...
            for p in BlockPos::iterate(start, end) {
                ...
                if block.has_tag(&papokin_data::tag::Block::MINECRAFT_BEDS) {
                    let bed_props = BedProperties::from_state_id(state.id);
        ...
        if profession != VillagerProfession::Nitwit && age >= 0 {
            // 每 20 刻检查一次，每约 100 刻进行一次傀儡生成检查
            if age % 100 == 0 && self.get_home().is_some() {
                // 检查是否处于恐慌状态或近期有过交谈，以生成铁傀儡
                ...
                if has_bed && has_worked {
                    ...
                    if nearby_villagers >= 3 {
                        ...
                        let nearby_golems = world
                            ...
                            .filter(|e| {
                                e.get_entity().entity_type
                                    == &papokin_data::entity::EntityType::IRON_GOLEM
                            })
                            .count();
...
impl Mob for VillagerEntity {
            ...
            for tag in recipes {
                if let Some(recipe) = tag.extract_compound() {
                        ...
                        && buy_b.as_ref().is_none_or(|stack| !stack.is_empty())
                    {
                        ...
                        offers.push(papokin_protocol::java::client::play::MerchantOffer {
    ...
    fn on_damage(
        &self,
        _damage_type: papokin_data::damage::DamageType,
        source: Option<&dyn EntityBase>,
    ) {
        let Some(source) = source.filter(|source| {
            source.get_entity().entity_type == &papokin_data::entity::EntityType::PLAYER
        }) else {
            return;
        };
    ...
    fn mob_interact(&self, player: &Arc<Player>, _item_stack: &mut ItemStack) -> bool {
        ...
        player.increment_stat(
            ...
            papokin_data::statistic::CustomStatistic::TalkedToVillager as i32,
            1,
        );
...
mod tests {
    use papokin_data::data_component_impl::{EnchantmentsImpl, StoredEnchantmentsImpl};
    use papokin_data::villager::VillagerTradeModifier;
    use papokin_util::version::JavaMinecraftVersion;
    ...
    fn enchanted_book_offer_has_vanilla_items_and_a_nonzero_price() {
        let (emeralds, enchanted_book, book) =
            enchanted_book_offer_items(&mut rand::rng()).expect("生成的可交易附魔标签应已填充");
