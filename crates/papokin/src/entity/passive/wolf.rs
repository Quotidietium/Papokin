...
impl WolfEntity {
    pub fn new(entity: Entity) -> Arc<Self> {
        ...
        let wolf = Self {
            mob_entity,
            variant: AtomicU8::new(3),       // 默认为 pale
...

        {
            ...
            // 8：FollowParentGoal 与 WanderAroundGoal
            ...
            // 10：LookAtPlayer 与 RandomLookAround
            goal_selector.add_goal(
                10,
...

        {
            ...
            // 4：NearestAttackableTargetGoal（玩家）— 已省略；原版以特定条件限制其触发
            // NeutralMob 仇恨系统，且报复经由目标 3（RevengeGoal）执行。
            // 5：NonTameRandomTarget（羊、兔子、狐狸）
...
impl Mob for WolfEntity {
    ...
    fn mob_init_data_tracker(&self) {
        ...
        if is_baby {
            entity.set_synced_data(papokin_data::tracked_data::wolf::BABY_ID, true);
        }
        entity.set_synced_data(
            papokin_data::tracked_data::wolf::TAMEABLE_FLAGS,
            self.get_tame_flags(),
        );
        entity.set_synced_data(
            papokin_data::tracked_data::wolf::COLLAR_COLOR,
            VarInt(self.collar_color.load(Ordering::Relaxed) as i32),
        );
        entity.set_synced_data(
            papokin_data::tracked_data::wolf::WOLF_VARIANT_ID,
            VarInt(self.variant.load(Ordering::Relaxed) as i32),
        );
        entity.set_synced_data(
            papokin_data::tracked_data::wolf::OWNER_UUID,
            self.get_owner(),
        );
    }
    ...
    fn mob_interact(&self, player: &Arc<Player>, item_stack: &mut ItemStack) -> bool {
        ...
        if self.is_tame() {
                    ...
                    < self.mob_entity.living_entity.get_max_health()
            {
                ...
                self.play_eating_sound(papokin_data::sound::Sound::EntityWolfAmbient);
                return true;
            }
            ...
            if self.is_owned_by(&player.gameprofile.id) {
                ...
                let parent_interaction = self.animal_interact(
                    player,
                    item_stack,
                    papokin_data::sound::Sound::EntityWolfAmbient,
                );
