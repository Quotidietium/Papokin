...
impl OcelotEntity {
    pub fn new(entity: Entity) -> Arc<Self> {
...

        {
            ...
            // 目标 1：FloatGoal（SwimGoal）
            goal_selector.add_goal(1, Box::new(SwimGoal::default()));
            // 目标 1：PanicGoal（EscapeDangerGoal）
            goal_selector.add_goal(1, EscapeDangerGoal::new(1.5));
            // 目标 3：OcelotTemptGoal
            goal_selector.add_goal(3, Box::new(TemptGoal::new(0.6, TEMPT_ITEMS, true)));
            // 目标 4：OcelotAvoidEntityGoal（尚未信任时）
...

        {
            ...
            // 目标 Goal 1：鸡和海龟的 NearestAttackableTargetGoal
    ...
    pub fn set_trusting(&self, trusting: bool) {
        ...
        entity.set_synced_data(papokin_data::tracked_data::ocelot::TRUSTING, trusting);
    }
}
...
impl Mob for OcelotEntity {
    ...
    fn mob_init_data_tracker(&self) {
        ...
        if is_baby {
            entity.set_synced_data(papokin_data::tracked_data::ocelot::BABY_ID, true);
        }
        entity.set_synced_data(
            papokin_data::tracked_data::ocelot::TRUSTING,
            self.is_trusting.load(Ordering::Relaxed),
        );
    }
    ...
    fn mob_interact(&self, player: &Arc<Player>, item_stack: &mut ItemStack) -> bool {
        ...
        if !self.is_trusting() && is_food && dist_sqr < 9.0 {
            ...
            if rng.random_range(0..3) == 0 {
                ...
                self.get_entity()
                    .world
                    .load()
                    .send_entity_status(self.get_entity(), EntityStatus::TrustingSucceeded);
            } else {
                self.get_entity()
                    .world
                    .load()
                    .send_entity_status(self.get_entity(), EntityStatus::TrustingFailed);
            }
        ...
        self.animal_interact(
            player,
            item_stack,
            papokin_data::sound::Sound::EntityOcelotAmbient,
        )
    }
}
