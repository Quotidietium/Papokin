...
impl TurtleEntity {
    ...
    pub fn set_has_egg(&self, has_egg: bool) {
        ...
        let entity = self.get_entity();
        entity.set_synced_data(papokin_data::tracked_data::turtle::HAS_EGG, has_egg);
    }
    ...
    pub fn set_laying_egg(&self, laying_egg: bool) {
        ...
        let entity = self.get_entity();
        entity.set_synced_data(papokin_data::tracked_data::turtle::LAYING_EGG, laying_egg);
    }
}
...
impl Mob for TurtleEntity {
    ...
    fn mob_read_nbt(&self, nbt: &NbtCompound) {
        ...
        if let Some(has_egg) = nbt.get_bool("HasEgg") {
            // NBT 还原不是施肥；直接设置状态
            // 而不触发 `EntityFertilizeEggEvent`。
            self.has_egg.store(has_egg, Ordering::Relaxed);
            self.get_entity()
                .set_synced_data(papokin_data::tracked_data::turtle::HAS_EGG, has_egg);
        }
    }
    ...
    fn mob_init_data_tracker(&self) {
        ...
        if is_baby {
            entity.set_synced_data(papokin_data::tracked_data::turtle::DATA_BABY_ID, true);
        }
        entity.set_synced_data(papokin_data::tracked_data::turtle::HAS_EGG, self.has_egg());
        entity.set_synced_data(
            papokin_data::tracked_data::turtle::LAYING_EGG,
            self.is_laying_egg(),
        );
    }
