    ...
    fn breed(mob: &dyn Mob, mate: &dyn EntityBase) {
        ...
        if let Some(player) = player_opt {
            ...
            player.increment_stat(
                ...
                papokin_data::statistic::CustomStatistic::AnimalsBred as i32,
                1,
            );

            player.trigger_advancement_criterion(
                papokin_data::advancement::Advancement::HUSBANDRY_BREED_AN_ANIMAL,
                "bred",
            );
            player.trigger_advancement_criterion(
                papokin_data::advancement::Advancement::HUSBANDRY_BRED_ALL_ANIMALS,
        ...
        world_full.send_entity_status(entity, EntityStatus::InLoveHearts);
        // TODO: 待 `animalBreedingDropsXp` 游戏规则存在后，以它作为开关。
