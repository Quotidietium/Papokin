...
use papokin_data::environment_attribute::Activity;
...
/// 正在参与刻计算的生物的大脑，仅能通过此上下文访问。
///
/// 行为与传感器绝不能自行锁定 `MobEntity::brain`；tick 已经
/// 持有该锁。刻运行期间，行为与感知器列表会脱离
/// 大脑，因此 `stop_all`、`debug_dump` 和 `is_brain_dead` 只有在大脑之外才能给出正确结果。
...
/// 每次传感器扫描时读取一次；原版此时会重新调整其共享的 `TargetingConditions`。
...
/// 原版 `DebugBrainDump` 中大脑部分的内容：当前活动、正在运行的行为以及
/// 每个已注册记忆一行，格式与调试订阅发送给客户端的格式一致。
