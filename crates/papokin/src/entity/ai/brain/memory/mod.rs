...
impl MemorySlot {
    // 原版的 dump 打印的是 ttl 所属的游戏时间；读者真正想要的是实际 ttl。
...
// `StringUtil.truncateStringIfNecessary(text, 255, true)`，原版转储会逐行应用。
...
impl MemoryStore {
    ...
    pub fn load_packed(&mut self, packed: &PackedMemories) {
        for (id, tag, ttl) in &packed.0 {
            ...
            let Some(value) = (codec.decode)(tag) else {
                tracing::debug!("丢弃无法解码的脑子记忆 {}", id.name());
                continue;
            };
    ...
    /// 单个槽位的只读、无类型视图。
    ///
    ///当内存类型未在此存储中注册时，返回 `None`。
    /// 描述使用原版的调试转储格式（截断至 255
    /// 字符）；仅当值存在且会过期时 ttl 才为 `Some`。
    #[must_use]
...
pub struct DescribedMemory {
    ...
    pub has_value: bool,
    /// 以原版调试导出格式字符串化的值（已截断
    /// 至 255 个字符）；当槽位无值时为 `None`。
    pub description: Option<String>,
    /// 距离该值过期剩余的刻数；当该值永不过期或……时为 `None`
    /// 槽位中没有任何值。
    pub ttl: Option<i64>,
}
    ...
    pub fn from_nbt(brain: &NbtCompound) -> Self {
        ...
        for (name, tag) in &memories.child_tags {
            let Some(id) = types::from_name(name) else {
                tracing::debug!("跳过未知的脑子记忆 {name}");
                continue;
            };
            if types::codec_of(id).is_none() {
                tracing::debug!("跳过非持久化的脑子记忆 {name}");
                continue;
            }
