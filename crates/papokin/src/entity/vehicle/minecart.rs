...
const fn get_exits(
    shape: papokin_data::block_properties::RailShape,
) -> (Vector3<f64>, Vector3<f64>) {
    use papokin_data::block_properties::RailShape;
    match shape {
...
impl EntityBase for MinecartEntity {
    ...
    fn tick(&self, caller: &dyn EntityBase, _server: &Server) {
        ...
        if is_on_rails {
            use papokin_data::block_properties::RailLikeProperties;
            use papokin_data::block_properties::{RailShape, RailShapeStraight};
        ...
        if velocity.length() > 0.001 {
            ...
            let mut friction = 0.95; // 原版矿车空气阻力
