...
impl VehicleEntity {
    ...
    pub async fn move_vehicle(
        &self,
        from: papokin_util::math::vector3::Vector3<f64>,
        to: papokin_util::math::vector3::Vector3<f64>,
    ) {
        let mut move_event =
    ...
    pub async fn collide_block(&self, block_pos: papokin_util::math::position::BlockPos) {
        let mut base_event =
            crate::plugin::api::events::vehicle::vehicle_collision::VehicleCollisionEvent::new(
                self.entity.entity_id,
            );
    ...
    pub fn send_wobble_metadata(&self) {
        self.entity.send_meta_data(&[
            Metadata::new(
                papokin_data::tracked_data::boat::ID_HURT,
                VarInt(self.get_hurt_time()),
            ),
            Metadata::new(
                papokin_data::tracked_data::boat::ID_HURTDIR,
                VarInt(self.get_hurt_dir()),
            ),
        ]);
        self.entity.send_meta_data(&[Metadata::new(
            papokin_data::tracked_data::boat::ID_DAMAGE,
            self.get_damage(),
        )]);
    }
    ...
    /// 应用标准的矿车受击晃动效果，而不摧毁载具。
    /// TNT 矿车在判定伤害会点燃还是摧毁自身之前使用此项。
...
mod tests {
    ...
    fn wobble_metadata(version: JavaMinecraftVersion) -> Vec<u8> {
        ...
        Metadata::new(papokin_data::tracked_data::boat::ID_HURT, VarInt(10))
            .write(&mut bytes, &version)
            .unwrap();
        Metadata::new(papokin_data::tracked_data::boat::ID_HURTDIR, VarInt(-1))
            .write(&mut bytes, &version)
            .unwrap();
        Metadata::new(papokin_data::tracked_data::boat::ID_DAMAGE, 10.0f32)
            .write(&mut bytes, &version)
            .unwrap();
        bytes
    }
