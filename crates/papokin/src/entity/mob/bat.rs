...
/// 原版：大多数生物的 `getMinAmbientSoundDelay()` 返回 80
...
impl BatEntity {
    ...
    fn tick_flying(&self, world: &World, above_pos: &BlockPos, pos: &Vector3<f64>) {
        ...
        if let Some(target) = *hanging_pos {
            ...
            let yaw_diff = papokin_util::math::wrap_degrees(yaw - entity.yaw.load());
...
impl Mob for BatEntity {
    ...
    fn mob_read_nbt(&self, nbt: &NbtCompound) {
        ...
        // 直接恢复保存的状态：加载 NBT 不是睡眠开关，
        // 因此不会为它触发 BatToggleSleepEvent。
        self.roosting.store(roosting, Relaxed);
        self.mob_entity
            .living_entity
            .entity
