...
impl AttackType {
    pub fn new(player: &Player, attack_cooldown_progress: f32) -> Self {
        ...
        let is_mace = held_item.item.id == papokin_data::item::Item::MACE.id;
...
/// 依据生物的击退抗性缩放击退 `strength`，
/// 镜像原版 `LivingEntity.knockback`：`strength *= 1.0 - resistance`。
/// 抗性为 1.0（铁傀儡、监守者等）时会完全抵消击退。
...
/// 战斗计算规则，与原版 `net.minecraft.world.damagesource.CombatRules` 一致。
...
impl CombatRules {
    ...
    /// 计算盔甲减免后的伤害，对应原版 `CombatRules.getDamageAfterAbsorb`。
    #[must_use]
    ...
    /// 计算魔法/附魔盔甲减免后的伤害，对应原版 `CombatRules.getDamageAfterMagicAbsorb`。
    #[must_use]
...
impl FallLocation {
    ...
    pub fn get_current_fall_location(
        ...
        if block == &papokin_data::Block::LADDER
            || block.has_tag(&papokin_data::tag::Block::MINECRAFT_TRAPDOORS)
        {
            Self::Ladder
        } else if block == &papokin_data::Block::VINE {
            Self::Vines
        } else if block == &papokin_data::Block::WEEPING_VINES
            || block == &papokin_data::Block::WEEPING_VINES_PLANT
        {
            Self::WeepingVines
        } else if block == &papokin_data::Block::TWISTING_VINES
            || block == &papokin_data::Block::TWISTING_VINES_PLANT
        {
            Self::TwistingVines
        } else if block == &papokin_data::Block::SCAFFOLDING {
            Self::Scaffolding
        } else if living.entity.is_in_water() {
            Self::Water
        } else if block.has_tag(&papokin_data::tag::Block::MINECRAFT_CLIMBABLE) {
            Self::OtherClimbable
        } else {
            Self::Generic
        }
...
pub struct CombatEntry {
    pub damage_type: papokin_data::damage_ext::ResolvedDamageType,
    pub damage: f32,
    ...
    pub attacker_name: Option<papokin_util::text::TextComponent>,
    pub attacker_item_name: Option<papokin_util::text::TextComponent>,
...
impl CombatTracker {
    ...
    pub fn record_damage(
        ...
        let attacker_is_player = attacker.is_some_and(|e| {
            e.get_entity().entity_type == &papokin_data::entity::EntityType::PLAYER
        });
            ...
            attacker.and_then(|e| {
                if let Some(player) = e.get_player() {
                    ...
                    && let Some(custom_name) = hand_stack
                        .get_data_component::<papokin_data::data_component_impl::CustomNameImpl>()
                {
                    ...
                    .equipment
                    .get(&papokin_data::data_component_impl::EquipmentSlot::MAIN_HAND)
                    && !stack.is_empty()
    ...
    /// 当前战斗期间的伤害记录，最早的在前。
    ///
    /// 用于报告（死亡消息、插件查询）的只读视图；该
    /// 条目会在回合超时后被 [`Self::recheck_status`] 清除
    /// 输出。
    #[must_use]
    ...
    pub fn get_death_message(
        &self,
        victim_name: papokin_util::text::TextComponent,
        kill_credit_name: Option<papokin_util::text::TextComponent>,
    ) -> papokin_util::text::TextComponent {
        if self.entries.is_empty() {
            return papokin_util::text::TextComponent::translate(
        ...
        match killing_damage_type.death_message_type() {
            papokin_data::damage::DeathMessageType::FallVariants => {
                if let Some(knock_off) = knock_off_entry {
                    ...
                    papokin_util::text::TextComponent::translate(loc.language_key(), [victim_name])
                }
            }
            papokin_data::damage::DeathMessageType::IntentionalGameDesign => {
                let death_msg = format!("death.attack.{}", killing_damage_type.message_id());
                let link = papokin_util::text::TextComponent::text("[")
                    .add_child(papokin_util::text::TextComponent::translate(
                        format!("{death_msg}.link"),
                        [],
                    ))
                    .add_child(papokin_util::text::TextComponent::text("]"))
                    .click_event(papokin_util::text::click::ClickEvent::OpenUrl {
                        url: "https://bugs.mojang.com/browse/MCPE-28723".into(),
                    })
                    .hover_event(papokin_util::text::hover::HoverEvent::show_text(
                        papokin_util::text::TextComponent::text("MCPE-28723"),
                    ));

                papokin_util::text::TextComponent::translate(
                    format!("{death_msg}.message"),
                    [victim_name, link],
                )
            }
            papokin_data::damage::DeathMessageType::Default => {
    ...
    fn get_most_significant_fall(&self) -> Option<&CombatEntry> {
        ...
        for i in 0..self.entries.len() {
            ...
            let is_fake_fall = damage_type
                .has_tag(&papokin_data::tag::DamageType::MINECRAFT_ALWAYS_MOST_SIGNIFICANT_FALL);
            ...
            if (damage_type.has_tag(&papokin_data::tag::DamageType::MINECRAFT_IS_FALL)
                || is_fake_fall)
                && fall_distance > 0.0
    ...
    fn get_fall_message(
        ...
        if !knock_off_type.has_tag(&papokin_data::tag::DamageType::MINECRAFT_IS_FALL)
            && !knock_off_type
                .has_tag(&papokin_data::tag::DamageType::MINECRAFT_ALWAYS_MOST_SIGNIFICANT_FALL)
        {
                ...
                && (killer_name.is_none() || killer_name != Some(attacker_name))
            {
                ...
                papokin_util::text::TextComponent::translate("death.fell.killer", [victim_name])
            }
        } else {
    ...
    fn get_message_for_assisted_fall(
        victim_name: papokin_util::text::TextComponent,
        attacker_name: papokin_util::text::TextComponent,
        attacker_item: Option<papokin_util::text::TextComponent>,
