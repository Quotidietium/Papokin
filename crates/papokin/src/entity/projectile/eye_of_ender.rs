...
impl EyeOfEnder {
    ...
    /// 将末影之眼对准 `target`，必要时钳制在 [`TOO_FAR_DISTANCE`] 内，
    /// 并随机决定它在到期时是否掉落物品。
...
impl EntityBase for EyeOfEnder {
    ...
    fn init_data_tracker(&self) {
        self.entity.set_synced_data(
            papokin_data::tracked_data::eye_of_ender::ITEM_STACK,
    ...
    fn send_java_spawn_packet(&self, client: &crate::net::java::JavaClient) {
        ...
        if client.version.load() >= papokin_util::version::JavaMinecraftVersion::V_1_21 {
            ...
            if metadata.write(&mut data, &client.version.load()).is_ok() {
                data.push(255);
                let meta_packet = papokin_protocol::java::client::play::CSetEntityMetadata::new(
