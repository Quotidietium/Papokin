...
use papokin_data::placed_feature::PlacedFeature as PlacedFeatureKey;
use papokin_data::structures::{Structure, StructureKeys, StructureType};
use papokin_data::translation;
use papokin_data::{Block, BlockStateId, Mirror, Rotation};
use papokin_util::identifier::Identifier;
use papokin_util::math::block_box::BlockBox;
use papokin_util::math::position::BlockPos;
use papokin_util::math::vector3::Vector3;
use papokin_util::random::hash_block_pos;
use papokin_util::text::TextComponent;
use papokin_world::generation::proto_chunk::ProtoChunk;
use papokin_world::world::{BlockAccessor, WorldPortalExt};
...
use papokin_world::generation::feature::configured_features::CONFIGURED_FEATURES;
use papokin_world::generation::feature::placed_features::{Feature, PLACED_FEATURES};
use papokin_world::generation::structure::structures::StructureGeneratorContext;
use papokin_world::generation::structure::structures::jigsaw::{
    PoolElementStructurePiece, place_pool_element_templates,
};
use papokin_world::generation::structure::structures::jigsaw_placement::{
    DimensionPadding, JigsawPlacement, LiquidSettings, MaxDistance, PoolAliasLookup,
};
use papokin_world::generation::structure::template::BlockPlacer;
...
const DESCRIPTION: &str = "在世界中放置一个结构模板。";
const PERMISSION: &str = "minecraft:command.place";

static TEMPLATE_NOT_FOUND: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_PLACE_TEMPLATE_INVALID);
static JIGSAW_FAILED: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_PLACE_JIGSAW_FAILED);
static STRUCTURE_INVALID: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_PLACE_STRUCTURE_INVALID);
static FEATURE_INVALID: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_PLACE_FEATURE_INVALID);

const CHUNK_DIM: i32 = 16;

/// 将世界 Y 坐标限制在有效区块范围内，使表面位于目标位置下方一格。
fn ground_y(block_y: i32, chunk_min_y: i32, chunk_height: i32) -> i32 {
    (block_y - 1).clamp(chunk_min_y, chunk_min_y + chunk_height - 1)
}

/// 结构部件与特征生成器使用的原版风格区块种子派生方式。
...
impl WorldPortalExt for CommandBlockRegistry {
    ...
    fn spawn_mobs_for_chunk_generation(
        ...
        _biome: &'static papokin_data::biome::Biome,
        _chunk_x: i32,
        _chunk_z: i32,
    ) {
    }
...
impl CommandExecutor for PlaceTemplateExecutor {
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        let template_name = template_name
            ...
            .to_string();
        let Some(template) =
            papokin_world::generation::structure::template::get_template(&template_name)
        else {
            return Err(
        ...
        papokin_world::generation::structure::template::place_template(
            &mut placer,
            &template,
            block_pos.0,
            (0, 0),
    ...
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        let (_piece_count, placer) = {
            ...
            if structure.structure_type == StructureType::Jigsaw {
                ...
                let position = papokin_world::generation::structure::generate_structure_position(
                    &key,
                    structure,
...
fn apply_delta(
    ...
    // 合成区块以地形作为种子填充，以便结构部件能够雕刻
    // 与之相抵。因此应用差值时会有意传播空气
    // 也会变化，可能在雕刻体雕刻之处替换真实地形
    // 合成石头。
    let mut idx = 0usize;
    for x in 0..CHUNK_DIM {
        for y in min_y..min_y + height {
