...
use papokin_util::math::experience;
...
const DESCRIPTION: &str = "添加、设置或查询玩家经验。";
const PERMISSION: &str = "minecraft:command.experience";

const ERROR_SET_POINTS_INVALID: CommandErrorType<0> =
    CommandErrorType::new("commands.experience.set.points.invalid");
...
impl ExperienceExecutor {
    ...
    fn get_success_message(
        ...
        let display_amount = if amount >= 0 { amount } else { amount.abs() };

        if targets.len() > 1 {
            TextComponent::translate(
                format!("commands.experience.{mode_str}.{type_str}.success.multiple"),
                [
                    TextComponent::text(display_amount.to_string()),
