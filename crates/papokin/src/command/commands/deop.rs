...
const DESCRIPTION: &str = "撤销一名玩家的管理员权限。";
const PERMISSION: &str = "minecraft:command.deop";

const ERROR_DEOP_FAILED: CommandErrorType<0> =
    CommandErrorType::new(papokin_data::translation::java::COMMANDS_DEOP_FAILED);
        ...
        for profile in &targets {
            if let Some(op_index) = config.ops.iter().position(|o| o.uuid == profile.id) {
                ...
                let msg = TextComponent::translate(
                    papokin_data::translation::java::COMMANDS_DEOP_SUCCESS,
