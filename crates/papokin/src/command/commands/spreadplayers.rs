...
const DESCRIPTION: &str = "将实体传送到一个区域内的随机地表位置。";
...
const FAILED_ENTITIES_ERROR_TYPE: CommandErrorType<4> =
    CommandErrorType::new(translation::java::COMMANDS_SPREADPLAYERS_FAILED_ENTITIES);

const FAILED_TEAMS_ERROR_TYPE: CommandErrorType<4> =
    CommandErrorType::new(translation::java::COMMANDS_SPREADPLAYERS_FAILED_TEAMS);

const INVALID_MAX_HEIGHT_ERROR_TYPE: CommandErrorType<2> =
    CommandErrorType::new(translation::java::COMMANDS_SPREADPLAYERS_FAILED_INVALID_HEIGHT);
...
impl Pile {
    ...
    /// 返回实体在此堆叠上应站立的 Y 坐标，或
    /// 若位置不安全（位于液体上方或超出高度界限）则返回 `None`。
    fn surface_y(&self, world: &World, max_height: i32) -> Option<i32> {
        ...
        let ground = papokin_util::math::position::BlockPos(Vector3::new(block_x, top, block_z));
...
/// 将 `piles` 相互散开，直到每对之间至少相距 `spread_distance`
/// 且所有堆都位于扩散区域内，与原版算法一致。
/// 返回各堆叠是否在 [`MAX_ITERATIONS`] 内收敛。
fn spread_piles(
    piles: &mut [Pile],
    spread_distance: f64,
    min_x: f64,
...
/// 在 `area` 内对 `pile_count` 个堆进行随机化、松弛与落地处理。
/// 返回各堆叠及其可站立 Y 坐标；若无安全的
/// 排布方式。
