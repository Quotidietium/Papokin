...
const DESCRIPTION: &str = "获取一名玩家的档案。";
...
pub const NO_PROFILE_ERROR_TYPE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_FETCHPROFILE_NO_PROFILE);
...
fn report_resolved_profile(
    ...
    let clickable = format_clickable_list(vec![
        TextComponent::translate(translation::java::COMMANDS_FETCHPROFILE_COPY_COMPONENT, [])
            .click_event(ClickEvent::CopyToClipboard {
                value: Cow::from(encoded_profile_as_string.clone()),
            }),
        TextComponent::translate(translation::java::COMMANDS_FETCHPROFILE_GIVE_ITEM, [])
    ...
    let msg = TextComponent::translate(message_id, [argument, clickable]);
