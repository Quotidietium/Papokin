...
const DESCRIPTION: &str = "用指定方块填充一个区域的全部或部分。";
const PERMISSION: &str = "minecraft:command.fill";

const ERROR_AREA_TOO_LARGE: CommandErrorType<2> =
    CommandErrorType::new(translation::java::COMMANDS_FILL_TOOBIG);

const ERROR_FAILED: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_FILL_FAILED);
            ...
            let result = world.level.read_chunk_sync(&chunk_pos, |chunk| {
...

                {
                    ...
                    for y in min_y..=max_y {
                        ...
                        let section_index = rel_y / papokin_world::chunk::CHUNK_WIDTH;
                        let sub_y = rel_y % papokin_world::chunk::CHUNK_WIDTH;
