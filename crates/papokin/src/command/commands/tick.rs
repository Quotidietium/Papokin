...
const DESCRIPTION: &str = "控制或查询游戏的刻运行状态。";
const PERMISSION: &str = "minecraft:command.tick";

// 辅助函数：将纳秒格式化为保留 2 位小数的毫秒
    ...
    fn handle_query(
        ...
        if manager.is_sprinting() {
            source.send_feedback(
                TextComponent::translate(translation::java::COMMANDS_TICK_STATUS_SPRINTING, []),
                false,
            );
            source.send_feedback(
    ...
    fn handle_non_sprinting_status(
        ...
        if manager.is_frozen() {
            sender.send_feedback(
                TextComponent::translate(translation::java::COMMANDS_TICK_STATUS_FROZEN, []),
                false,
            );
        } else if avg_tick_nanos > manager.nanoseconds_per_tick() {
            sender.send_feedback(
                TextComponent::translate(translation::java::COMMANDS_TICK_STATUS_LAGGING, []),
                false,
            );
        } else {
            sender.send_feedback(
                TextComponent::translate(translation::java::COMMANDS_TICK_STATUS_RUNNING, []),
                false,
            );
        }
    }
        ...
        match self.0 {
            ...
            SubCommand::StepDefault => {
                if manager.step_game_if_paused(server, 1) {
                    ...
                    source.send_error(
                        TextComponent::translate(translation::java::COMMANDS_TICK_STEP_FAIL, [])
