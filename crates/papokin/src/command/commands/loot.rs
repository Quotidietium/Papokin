...
const DESCRIPTION: &str = "将指定战利品表的掉落物放入指定物品栏或世界中。";
const PERMISSION: &str = "minecraft:command.loot";

static ERROR_INVALID_LOOT_TABLE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::ARGUMENT_RESOURCE_NOT_FOUND);

static ERROR_NO_HELD_ITEMS: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DROP_NO_HELD_ITEMS);

static ERROR_NO_ENTITY_LOOT_TABLE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DROP_NO_LOOT_TABLE);

static ERROR_NO_BLOCK_LOOT_TABLE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DROP_NO_LOOT_TABLE_BLOCK);

static ERROR_NOT_CONTAINER: CommandErrorType<3> =
    CommandErrorType::new(translation::java::COMMANDS_ITEM_TARGET_NOT_A_CONTAINER);

static ERROR_NO_SUCH_SLOT: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_ITEM_TARGET_NO_SUCH_SLOT);
...
fn get_hand_item(
    ...
    context.source.as_player().map_or_else(
        || {
            let display_name = TextComponent::text("服务器");
...
fn send_callback(context: &CommandContext, drops: &[ItemStack], table_id: Option<&str>) {
    let msg = match (drops.len(), table_id) {
        ...
        (count, Some(table)) => TextComponent::translate(
        ...
        (count, None) => TextComponent::translate(
    ...
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        match self.source {
            Source::Fish { tool } => {
                ...
                let loot_table =
                    papokin_data::loot_table::get_loot_table(&key).ok_or_else(|| {
