...
const DESCRIPTION: &str = "安排一个动作或函数在指定时长后执行。";
const PERMISSION: &str = "minecraft:command.schedule";

pub const ERROR_SAME_TICK: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_SCHEDULE_SAME_TICK);

pub const ERROR_CANT_REMOVE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_SCHEDULE_CLEARED_FAILURE);

#[expect(dead_code)]
pub const ERROR_MACRO: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_SCHEDULE_MACRO);
