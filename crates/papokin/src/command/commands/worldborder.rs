...
const DESCRIPTION: &str = "管理世界边界。";
const PERMISSION: &str = "minecraft:command.worldborder";

const ERROR_SAME_CENTER: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_WORLDBORDER_CENTER_FAILED);
const ERROR_SAME_SIZE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_WORLDBORDER_SET_FAILED_NOCHANGE);
const ERROR_TOO_SMALL: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_WORLDBORDER_SET_FAILED_SMALL);
const ERROR_TOO_BIG: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_WORLDBORDER_SET_FAILED_BIG);
const ERROR_TOO_FAR_OUT: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_WORLDBORDER_SET_FAILED_FAR);
const ERROR_SAME_WARNING_TIME: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_WORLDBORDER_WARNING_TIME_FAILED);
const ERROR_SAME_WARNING_DISTANCE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_WORLDBORDER_WARNING_DISTANCE_FAILED);
const ERROR_SAME_DAMAGE_BUFFER: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_WORLDBORDER_DAMAGE_BUFFER_FAILED);
const ERROR_SAME_DAMAGE_AMOUNT: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_WORLDBORDER_DAMAGE_AMOUNT_FAILED);
...
fn set_size(
    ...
    let (current, diff) = {
        ...
        let speed = (time_in_ticks > 0).then(|| time_in_ticks * 50); // 将刻转换为毫秒
