...
const DESCRIPTION: &str = "创建和修改 Boss 血条。";
const PERMISSION: &str = "minecraft:command.bossbar";

const ERROR_INVALID_COLOR: CommandErrorType<1> =
    CommandErrorType::new(translation::java::ARGUMENT_ENUM_INVALID);

const ERROR_INVALID_STYLE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::ARGUMENT_ENUM_INVALID);

const ERROR_UNKNOWN_BOSSBAR: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_BOSSBAR_UNKNOWN);

const ERROR_CREATE_FAILED: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_BOSSBAR_CREATE_FAILED);
...
impl ArgumentType<CommandSource> for BossbarIdArgumentType {
    ...
    fn parse(&self, reader: &mut StringReader) -> Result<Self::Item, CommandSyntaxError> {
        let ident = papokin_util::identifier::Identifier::from_reader(reader)?;
        Ok(ident.to_string())
    }
...
impl ArgumentType<CommandSource> for BossbarColorArgumentType {
    ...
    fn client_side_parser(&'_ self) -> JavaClientArgumentType {
        JavaClientArgumentType::String(
            papokin_protocol::java::client::play::StringProtoArgBehavior::SingleWord,
        )
    }
    ...
    match error {
        ...
        BossbarUpdateError::NoChanges(value, variation) => {
            ...
            crate::command::errors::error_types::DISPATCHER_PARSE_EXCEPTION
                .create_without_context(TextComponent::translate(key, []))
        }
    }
}
...
impl CommandExecutor for ListExecutor {
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        if bossbars.is_empty() {
            context.source.send_feedback(
                TextComponent::translate(translation::java::COMMANDS_BOSSBAR_LIST_BARS_NONE, []),
                false,
            );
            return Ok(0);
        }
