...
const DESCRIPTION: &str = "管理实体的记分板标签。";
...
const ADD_FAILED_ERROR_TYPE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_TAG_ADD_FAILED);

const REMOVE_FAILED_ERROR_TYPE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_TAG_REMOVE_FAILED);
    ...
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        let msg = if targets.len() == 1 {
            ...
            if all_tags.is_empty() {
                TextComponent::translate(translation::java::COMMANDS_TAG_LIST_SINGLE_EMPTY, [name])
            } else {
                TextComponent::translate(
