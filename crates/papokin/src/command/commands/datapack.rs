...
const DESCRIPTION: &str = "控制已加载的数据包。";
const PERMISSION: &str = "minecraft:command.datapack";

static ERROR_UNKNOWN_DATAPACK: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DATAPACK_UNKNOWN);

static ERROR_ENABLE_FAILED: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DATAPACK_ENABLE_FAILED);

static ERROR_DISABLE_FAILED: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DATAPACK_DISABLE_FAILED);

static ERROR_CREATE_ALREADY_EXISTS: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DATAPACK_CREATE_ALREADY_EXISTS);

static ERROR_CREATE_INVALID_NAME: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DATAPACK_CREATE_INVALID_NAME);

static ERROR_CREATE_IO_FAILURE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DATAPACK_CREATE_IO_FAILURE);
...
fn send_enabled_list(context: &CommandContext<'_>, enabled: &[String]) {
    if enabled.is_empty() {
        context.source.send_feedback(
            TextComponent::translate(translation::java::COMMANDS_DATAPACK_LIST_ENABLED_NONE, []),
            false,
        );
    } else {
        let packs_component =
...
fn send_available_list(context: &CommandContext<'_>, available: &[String]) {
    if available.is_empty() {
        context.source.send_feedback(
            TextComponent::translate(translation::java::COMMANDS_DATAPACK_LIST_AVAILABLE_NONE, []),
            false,
        );
    } else {
        let packs_component =
...
impl CommandExecutor for DatapackDisableExecutor {
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        if target_pack == "vanilla" {
            return Err(ERROR_DISABLE_FAILED
                .create_without_context(TextComponent::text("原版".to_string())));
        }
