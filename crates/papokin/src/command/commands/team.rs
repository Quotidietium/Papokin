...
const DUPLICATE_TEAM_ERROR: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_TEAM_ADD_DUPLICATE);

const TEAM_NOT_FOUND_ERROR: CommandErrorType<1> =
    CommandErrorType::new(translation::java::TEAM_NOTFOUND);

const EMPTY_UNCHANGED_ERROR: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_TEAM_EMPTY_UNCHANGED);

const COLOR_UNCHANGED_ERROR: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_TEAM_OPTION_COLOR_UNCHANGED);

const NAME_UNCHANGED_ERROR: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_TEAM_OPTION_NAME_UNCHANGED);

const NAMETAG_VISIBILITY_UNCHANGED_ERROR: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_TEAM_OPTION_NAMETAGVISIBILITY_UNCHANGED);

const COLLISION_RULE_UNCHANGED_ERROR: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_TEAM_OPTION_COLLISIONRULE_UNCHANGED);

const FRIENDLY_FIRE_ALREADY_ENABLED_ERROR: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_TEAM_OPTION_FRIENDLYFIRE_ALREADYENABLED);

const FRIENDLY_FIRE_ALREADY_DISABLED_ERROR: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_TEAM_OPTION_FRIENDLYFIRE_ALREADYDISABLED);
...
impl CommandExecutor for TeamListExecutor {
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        if let Some(team_name) = team_name {
            ...
            if teams.is_empty() {
                context.source.send_feedback(
                    TextComponent::translate(translation::java::COMMANDS_TEAM_LIST_TEAMS_EMPTY, []),
                    false,
                );
            } else {
                let mut list_comp = TextComponent::empty();
...
impl CommandExecutor for TeamModifyFriendlyFireExecutor {
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        context
            .source
            .send_feedback(TextComponent::translate(key, [team_display_name]), true);
