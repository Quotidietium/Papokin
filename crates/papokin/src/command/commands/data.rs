...
const DESCRIPTION: &str = "查询或修改实体、方块和存储的 NBT 数据。";
const PERMISSION: &str = "minecraft:command.data";

pub const ERROR_MERGE_UNCHANGED: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_DATA_MERGE_FAILED);

pub const ERROR_GET_NOT_NUMBER: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DATA_GET_INVALID);

pub const ERROR_GET_NON_EXISTENT: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DATA_GET_UNKNOWN);

pub const ERROR_MULTIPLE_TAGS: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_DATA_GET_MULTIPLE);

pub const ERROR_EXPECTED_OBJECT: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DATA_MODIFY_EXPECTED_OBJECT);

pub const ERROR_EXPECTED_VALUE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::COMMANDS_DATA_MODIFY_EXPECTED_VALUE);

pub const ERROR_INVALID_SUBSTRING: CommandErrorType<2> =
    CommandErrorType::new(translation::java::COMMANDS_DATA_MODIFY_INVALID_SUBSTRING);

pub const ERROR_BLOCK_INVALID: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_DATA_BLOCK_INVALID);

pub const ERROR_ENTITY_INVALID: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_DATA_ENTITY_INVALID);
...
impl TargetKind {
    pub fn access(
        ...
        match self {
            ...
            Self::Storage => {
                let id = context.get_argument::<papokin_util::identifier::Identifier>(&format!(
                    "{prefix}_target"
                ))?;
