...
const DESCRIPTION: &str = "传送实体，包括玩家。";
const PERMISSION: &str = "minecraft:command.teleport";

const ERROR_INVALID_POSITION: CommandErrorType<0> =
    CommandErrorType::new(translation::java::COMMANDS_TELEPORT_INVALIDPOSITION);
...
impl CommandExecutor for EntitiesToEntityExecutor {
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        context.source.send_feedback(
            TextComponent::translate(key, [target_arg, destination_entity.get_display_name()]),
            true,
        );
