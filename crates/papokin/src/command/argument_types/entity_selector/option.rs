...
use papokin_util::identifier::{Identifier, VANILLA_NAMESPACE};
...
pub const UNKNOWN_OPTION_ERROR_TYPE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_OPTIONS_UNKNOWN);
pub const INAPPLICABLE_OPTION_ERROR_TYPE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_OPTIONS_INAPPLICABLE);
pub const DISTANCE_NEGATIVE_ERROR_TYPE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_OPTIONS_DISTANCE_NEGATIVE);
pub const LEVEL_NEGATIVE_ERROR_TYPE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_OPTIONS_LEVEL_NEGATIVE);
pub const LIMIT_TOO_SMALL_ERROR_TYPE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_OPTIONS_LIMIT_TOOSMALL);
pub const SORT_UNKNOWN_ERROR_TYPE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_OPTIONS_SORT_IRREVERSIBLE);
pub const GAMEMODE_INVALID_ERROR_TYPE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_OPTIONS_MODE_INVALID);
pub const TYPE_INVALID_ERROR_TYPE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_OPTIONS_TYPE_INVALID);

/// 用于自定义 [`EntitySelectorParser`] 的选项。
///
/// 这些可在命令中指定实体选择器时使用。
...
impl EntitySelectorOption {
    ...
    /// 修改提供的 [`EntitySelectorParser`]。
    ///
    /// 所有必填字段都将由此方法使用 [`StringReader`] 解析
    /// 方法，并且任何所需的谓词都会被添加。
    /// 找到的所有错误都会被返回。
    ...
    /// 返回提供的 [`EntitySelectorParser`] 是否可以使用此选项。
        ...
        for option in Self::VALUES {
            ...
            if option.name().starts_with(lower_prefix) && option.can_use(parser) {
                ...
                builder = builder.suggest_with_tooltip(
                    format!("{}=", option.name()),
                    TextComponent::translate(key.clone(), []),
                );
            }
        }
        builder
