...
pub const INVALID_NAME_OR_UUID_ERROR_TYPE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_INVALID);

pub const UNKNOWN_SELECTOR_TYPE_ERROR_TYPE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_SELECTOR_UNKNOWN);

pub const SELECTORS_NOT_ALLOWED_ERROR_TYPE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_SELECTOR_NOT_ALLOWED);

pub const MISSING_SELECTOR_TYPE_ERROR_TYPE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_SELECTOR_MISSING);

pub const EXPECTED_END_OF_OPTIONS_ERROR_TYPE: CommandErrorType<0> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_OPTIONS_UNTERMINATED);

pub const EXPECTED_OPTION_VALUE_ERROR_TYPE: CommandErrorType<1> =
    CommandErrorType::new(translation::java::ARGUMENT_ENTITY_OPTIONS_VALUELESS);

bitflags! {
    /// 一个位标志列表，用于设置实体选择器解析器的属性。
    /// 这些应当由实体选择器选项来设置。
    ...
    pub struct Flags: u32 {
        ...
        /// 是否已设置 `advancements` 选项。
        const ADVANCEMENTS_SET = 1 << 11;
    }
}

/// 一个用于解析 [`EntitySelector`] 的结构体。
///
/// * `'b` 是 [`StringReader`] 可变引用的生命周期。
/// * `'a` 是 [`StringReader`] 的生命周期。
#[derive(Debug)]
...
impl<'b, 'a> EntitySelectorParser<'b, 'a> {
    /// 构造一个新的 [`EntitySelectorParser`]。
    ///
    /// # Arguments
    ///
    /// * `reader`: 解析实体选择器时使用的 [`StringReader`]。
    /// * `allow_selectors`: 是否允许选择器变量（如 `@s` 或 `@p`）。
    ...
    /// 尝试从所提供的 [`StringReader`] 解析选择器，并消费
    /// 并在此过程中处理其自身。
...
impl EntitySelectorParserSuggestions {
    ...
    fn fill_selector_suggestions(mut builder: SuggestionsBuilder) -> SuggestionsBuilder {
        ...
        builder = builder.suggest_with_tooltip(
            "@a",
            TextComponent::translate(translation::java::ARGUMENT_ENTITY_SELECTOR_ALLPLAYERS, []),
        );
        builder = builder.suggest_with_tooltip(
            "@r",
            TextComponent::translate(translation::java::ARGUMENT_ENTITY_SELECTOR_RANDOMPLAYER, []),
        );
        builder = builder.suggest_with_tooltip(
            "@s",
            TextComponent::translate(translation::java::ARGUMENT_ENTITY_SELECTOR_SELF, []),
        );
        builder = builder.suggest_with_tooltip(
            "@e",
            TextComponent::translate(translation::java::ARGUMENT_ENTITY_SELECTOR_ALLENTITIES, []),
        );
        builder = builder.suggest_with_tooltip(
            "@n",
            TextComponent::translate(
    ...
    pub fn list_suggestions(
        ...
        parser.fill_suggestions(suggestions_builder, |mut suggestions| {
            ...
            // 仅适用于 EntityArgumentType：这是服务器端的，因此不会显示其他实体。
            suggestions
        })
    }
}
...
mod tests {
    ...
    fn parse_entity_type_with_namespace() {
        ...
        assert!(parse("@e[type=!minecraft:iron_golem]"));
        assert!(!parse("@e[type=papokin:iron_golem]"));
