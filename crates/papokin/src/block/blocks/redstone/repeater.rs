...
type RepeaterProperties = papokin_data::block_properties::RepeaterLikeProperties;
...
impl BlockBehaviour for RepeaterBlock {
    ...
    fn on_scheduled_tick(&self, args: OnScheduledTickArgs<'_>) {
        ...
        // 原版的 `DiodeBlock.tick` 只是执行 `setBlock(..., 2)`，这仍会调用相邻方块的
        // 前方。Pumpkin 的 `placed` 只设置方块类型，因此由下方的 `update_target` 负责。
    ...
    fn emits_redstone_power(&self, args: EmitsRedstonePowerArgs<'_>) -> bool {
        // 原版的 `isSignalSource` 恒为 true；`shouldConnectTo` 检查朝向
        // 及其反向（向内与向外）。能量仍仅限朝向。
