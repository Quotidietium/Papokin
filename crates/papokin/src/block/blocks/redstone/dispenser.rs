...
use papokin_data::data_component_impl::{EquippableImpl, IDSet, PotionContentsImpl};
use papokin_data::entity::{EntityType, entity_from_egg};
...
use papokin_data::{Block, BlockStateId, FacingExt};
use papokin_inventory::Inventory;
use papokin_inventory::generic_container_screen_handler::create_generic_3x3;
...
impl ScreenHandlerFactory for DispenserScreenFactory {
    ...
    fn get_display_name(&self) -> TextComponent {
        papokin_macros::translate!(translation::java::CONTAINER_DISPENSER)
    }
}
...
type DispenserLikeProperties = papokin_data::block_properties::DispenserLikeProperties;
    ...
    fn normal_use(&self, args: NormalUseArgs<'_>) -> BlockActionResult {
        if let Some(factory) = self.get_screen_handler_factory(GetScreenHandlerFactoryArgs {
            ...
            args.player.increment_stat(
                ...
                papokin_data::statistic::CustomStatistic::InspectDispenser as i32,
                1,
            );
            args.player
...
impl DispenserBlock {
    ...
    fn dispense_firework_rocket(ctx: &DispenseContext<'_>, item: &mut ItemStack) {
        ...
        // `FireworkRocketEntity` 不暴露其内部弹射物，因此需要复现
        // 此处对应 `ThrownItemEntity::set_velocity`。
    ...
    /// 将 `stack` 放入第一个空槽位，如果所有槽位都被占用则将其原样返回。
    /// 当前正在发射的槽位仍持有发射前的物品堆，因此它是
    /// 绝不会被视作空闲。
    ...
    fn dispense_filled_bucket(ctx: &DispenseContext<'_>, item: &mut ItemStack) {
        ...
        // TODO: 像玩家路径那样，为美西螈/鱼/蝌蚪桶生成其中存放的实体。
    ...
    fn dispense_flint_and_steel(ctx: &DispenseContext<'_>, item: &mut ItemStack) {
        ...
        if ignited {
            // `damage_item` 在工具损坏时已经会把它从物品堆中消耗掉。
