...
pub trait RedstoneGateBlock<T: Send + Sync + BlockProperties + RedstoneGateBlockProperties> {
    ...
    where
        Self: Send + Sync,
    {
        // 原版 `DiodeBlock.neighborChanged`：先执行 `getBlockState(pos).is(this)`。
        // 形状更新可能已弹出此单元格（`replaceWithStateForNeighborUpdate`）。
        ...
        // 原版先 `dropResources` 再 `removeBlock`，然后对每个
        // 邻居，来源为 `this`（红石二极管本身，而非触发它的方块）。
        args.world
