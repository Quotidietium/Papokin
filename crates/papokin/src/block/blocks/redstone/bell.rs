...
use papokin_data::block_properties::{AttachFace, BellAttachment, BellLikeProperties};
...
use papokin_data::{HorizontalFacingExt, tag};
...
fn ring_bell(
    ...
    //TODO 发出游戏事件：BLOCK_CHANGE -> 发送方块更新数据包
    true
}
...
impl BlockBehaviour for BellBlock {
    ...
    fn normal_use(&self, args: NormalUseArgs<'_>) -> BlockActionResult {
        ...
        if !is_point_on_bell(args.hit, props.attachment, props.facing) {
            return BlockActionResult::Pass; // 如果准星未正确定位则通过
        }
        if !ring_bell(
            *args.position,
            args.world,
        ...
        args.player.increment_stat(
            ...
            papokin_data::statistic::CustomStatistic::BellRing as i32,
            1,
        );
