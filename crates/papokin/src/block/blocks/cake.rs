...
use papokin_util::{GameMode, math::position::BlockPos};
use papokin_world::{
    tick::TickPriority,
    world::{BlockAccessor, BlockFlags},
};

/// 原版 `CakeBlock.getOutputSignal`。饱和处理 -> 超出范围的啃食数读作 0。
#[must_use]
pub const fn cake_output_signal(bites: u8) -> u8 {
    7u8.saturating_sub(bites) * 2
}

/// 原版 `CakeBlock.FULL_CAKE_SIGNAL`。
        ...
        match properties.bites {
            0..=5 => {
                player.increment_stat(
                    ...
                    papokin_data::statistic::CustomStatistic::EatCakeSlice as i32,
                    1,
                );
                properties.bites += 1;
