...
use papokin_data::{BlockDirection, BlockState, BlockStateId, tag};
...
impl BlockBehaviour for LanternBlock {
    fn on_place(&self, args: OnPlaceArgs<'_>) -> BlockStateId {
        let mut props = papokin_data::block_properties::LanternLikeProperties::default(args.block);
...
fn can_place_at(world: &World, position: &BlockPos) -> bool {
    //不知道为什么这里没有随 .is_center_solid 更新，因此这是一个“临时补丁”
    if world
        .get_block(&position.down())
        .has_tag(&tag::Block::C_FENCE_GATES)
    {
        let fence_gate_props =
            papokin_data::block_properties::OakFenceGateLikeProperties::from_state_id(
