...
impl BedBlock {
    /// 在进床失败时触发 `PlayerBedFailEnterEvent`。若事件被取消，
    /// 事件会抑制失败消息，并让玩家继续
    /// 进入床的路径（Paper 语义）。
    ///
    ///当失败仍然成立（未被取消）时，返回 `true`。
    fn fire_bed_fail(
        world: &Arc<World>,
        player: &Arc<Player>,
        bed_pos: BlockPos,
    ...
    fn use_bed(
        ...
        // 若床规则会爆炸则爆炸（EnvironmentAttributes.BED_RULE）
            ...
            )
        {
            player.send_system_message_raw(
                &papokin_macros::translate!(translation::java::BLOCK_MINECRAFT_BED_NO_SLEEP),
                true,
            );
            return BlockActionResult::SuccessServer;
        }
        ...
        {
            player.send_system_message_raw(
                &papokin_macros::translate!(translation::java::BLOCK_MINECRAFT_BED_OBSTRUCTED),
                true,
            );
            return BlockActionResult::SuccessServer;
        }
        ...
        if bed_props.occupied {
            ...
            if Self::fire_bed_fail(world, player, bed_head_pos, bed_fail_reasons::OCCUPIED) {
                player.send_system_message_raw(
                    &papokin_macros::translate!(translation::java::BLOCK_MINECRAFT_BED_OCCUPIED),
                    true,
                );
                return BlockActionResult::SuccessServer;
            }
        ...
        {
            player.send_system_message_raw(
                &papokin_macros::translate!(translation::java::BLOCK_MINECRAFT_BED_TOO_FAR_AWAY),
                true,
            );
            return BlockActionResult::SuccessServer;
        }
        ...
        {
            player.send_system_message(&papokin_macros::translate!(
        ...
        player.increment_stat(
            ...
            papokin_data::statistic::CustomStatistic::SleepInBed as i32,
            1,
        );
