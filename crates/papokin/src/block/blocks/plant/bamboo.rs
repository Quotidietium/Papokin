...
use papokin_data::block_properties::{BambooLeaves, BambooLikeProperties};
use papokin_data::tag::Block::MINECRAFT_SUPPORTS_BAMBOO;
use papokin_data::tag::Taggable;
use papokin_data::tag::{self};
...
fn bone_meal(world: &Arc<World>, position: &BlockPos) {
    ...
    for (bamboo_above, _) in (count_bamboo_above(world, position)..).zip(0..growth_amount) {
        ...
        // `next_pos` 是这根竹子最顶端的一根，因此我们要长入的空闲空间是
        // 其上方的方块。原版从最顶端的茎读取 `STAGE` 并从那里继续生长。
