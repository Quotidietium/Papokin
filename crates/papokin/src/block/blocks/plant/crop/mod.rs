...
trait CropBlockBase: PlantBlockBase {
    // 故意不命名为 `can_plant_on_top`：那会与
    // `PlantBlockBase` 的同名方法而不重写它，并且
    // `PlantBlockBase` 的默认实现会继续悄悄使用通用
    // `supports_vegetation` 检查。农作物必须重写
    // `PlantBlockBase::can_plant_on_top`，并委托到此处。
