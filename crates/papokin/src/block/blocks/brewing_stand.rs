...
impl ScreenHandlerFactory for BrewingScreenFactory {
    fn create_screen_handler(
        ...
        papokin_inventory::brewing::create_brewing(sync_id, player_inventory, inventory, &self.1)
            .map(|handler| Arc::new(Mutex::new(handler)) as SharedScreenHandler)
    }

    fn get_display_name(&self) -> TextComponent {
        papokin_macros::translate!(translation::java::CONTAINER_BREWING)
    }
}
    ...
    fn normal_use(&self, args: NormalUseArgs<'_>) -> BlockActionResult {
        if let Some(factory) = self.get_screen_handler_factory(GetScreenHandlerFactoryArgs {
            ...
            args.player.increment_stat(
                ...
                papokin_data::statistic::CustomStatistic::InteractWithBrewingstand as i32,
                1,
            );
            args.player
