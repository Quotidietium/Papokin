...
use papokin_data::block_properties::{CommandBlockLikeProperties, Facing};
use papokin_data::{Block, BlockId, BlockState, BlockStateId, FacingExt, Rotation};

use papokin_util::{GameMode, PermissionLvl, math::position::BlockPos};
...
impl CommandBlock {
    ...
    /// 等价于原版的 `CommandBlockEntity.markConditionMet()`。条件式
    /// 命令方块会检查其自身朝向背后的命令方块。
    ...
    /// 对应原版 CommandBlock.setPoweredAndUpdate：只有电力边沿才会改变状态
    /// 通电状态，红石模式的上升沿会记录该条件并
    /// 安排到一刻之后执行。连锁/自动方块不会安排
    /// 使自己远离边缘。
    fn update(
        world: &World,
        block: &Block,
    ...
    fn execute(
        ...
        let Ok(command_entity) = Arc::downcast::<CommandBlockEntity>(block_entity) else {
            warn!("无法将方块实体向下转型为 CommandBlockEntity");
            return;
        };
    ...
    fn chain_execute(server: &Arc<Server>, world: &Arc<World>, start: BlockPos) {
        ...
        while i > 0 {
            ...
            // 原版遵循每个锁链方块自身的 FACING，因此锁链可以转向。
            pos = pos.offset(props.facing.to_block_direction().to_offset());

            i -= 1;
            if i == 0 {
                warn!("命令方块链执行了 {} 次（已达上限）！", u16::MAX);
            }
        }
    }
}
    ...
    fn on_neighbor_update(&self, args: OnNeighborUpdateArgs<'_>) {
        {
            ...
            if let Some(block_entity) = args.world.get_block_entity(args.position) {
                ...
                let Some(command_entity) =
                    ...
                    warn!("位于 {} 的方块实体不是命令方块", args.position);
                    return;
                };
