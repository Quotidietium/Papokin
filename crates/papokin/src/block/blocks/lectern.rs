...
use papokin_data::block_properties::LecternLikeProperties;
...
use papokin_data::{Block, BlockDirection, BlockState, BlockStateId, tag, translation};
use papokin_inventory::Inventory;
use papokin_inventory::lectern_screen_handler::{LecternController, LecternScreenHandler};
...
impl LecternController for LecternPageController {
    ...
    fn on_book_take_click(&self, player: &dyn InventoryPlayer) -> bool {
        let Some(book) = self
            .entity()
            .map(|entity| papokin_inventory::Inventory::get_stack(entity, 0))
        else {
            return true;
        };
    ...
    fn on_page_change_click(&self, player: &dyn InventoryPlayer, new_page: i32) -> Option<i32> {
        ...
        let book = papokin_inventory::Inventory::get_stack(entity, 0);
...
impl ScreenHandlerFactory for LecternScreenFactory {
    ...
    fn get_display_name(&self) -> TextComponent {
        papokin_macros::translate!(translation::java::CONTAINER_LECTERN)
    }
}
...
impl LecternBlock {
    ...
    /// 设置 `has_book`，并像原版 `setHasBook` 那样丢弃任何待处理的脉冲。
...
impl BlockBehaviour for LecternBlock {
    ...
    fn normal_use(&self, args: NormalUseArgs<'_>) -> BlockActionResult {
        ...
        .map_or(BlockActionResult::Pass, |factory| {
            args.player.increment_stat(
                ...
                papokin_data::statistic::CustomStatistic::InteractWithLectern as i32,
                1,
            );
    ...
    fn use_with_item(&self, args: UseWithItemArgs<'_>) -> BlockActionResult {
        ...
        let props =
            LecternLikeProperties::from_state_id(args.world.get_block_state(args.position).id);
        if props.has_book {
            // 继续 fall-through，以便 `normal_use` 打开阅读界面
            return BlockActionResult::PassToDefaultBlockAction;
        }
