...
use papokin_protocol::java::client::play::{CMapItemData, MapPatch};
...
/// 辅助函数：将 RGB 值转换为最接近的 Minecraft 地图颜色索引。
#[must_use]
pub fn rgb_to_map_color(r: u8, g: u8, b: u8) -> u8 {
    use papokin_data::map_color::MapColor;

    // 亮度乘数：180（低）、220（普通）、255（高）、135（最低）
