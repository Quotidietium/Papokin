use papokin_data::data_component_impl::{WritableBookContentImpl, WrittenBookContentImpl};
...
impl LecternBlockEntity {
    ...
    /// 原版 `LecternBlockEntity.getRedstoneSignal`：`floor(progress * 14) + 1`，
    /// 或没有书时为 `0`。单页书视为已完整阅读并发出 15。
