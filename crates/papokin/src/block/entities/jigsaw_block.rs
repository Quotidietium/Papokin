...
use papokin_data::block_properties::{JigsawLikeProperties, Orientation};
use papokin_nbt::compound::NbtCompound;
use papokin_util::{
    math::position::BlockPos,
    random::{RandomGenerator, xoroshiro128::Xoroshiro},
};
use papokin_world::generation::structure::structures::{
    ...
    fn place_structure(world: &Arc<World>, structure: &StructurePosition, keep_jigsaws: bool) {
        ...
        for piece in &mut pieces {
            if let Some(pool_piece) = piece.as_any().downcast_ref::<PoolElementStructurePiece>() {
                papokin_world::generation::structure::structures::jigsaw::place_pool_element_templates(
                    pool_piece,
                    &mut placer,
                    None,
                    keep_jigsaws,
