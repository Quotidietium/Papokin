...
impl BlockEntity for EndGatewayBlockEntity {
    ...
    fn write_nbt(&self, nbt: &mut NbtCompound) {
            ...
            && let Some(exit) = exit_portal.as_ref()
        {
            nbt.put(
                "exit_portal",
                papokin_nbt::tag::NbtTag::IntArray(vec![exit.0.x, exit.0.y, exit.0.z]),
            );
        }
    }
...
impl EndGatewayBlockEntity {
    ...
    pub fn get_portal_position(
        ...
        if exit_portal_opt.is_none()
            && current_level.dimension == papokin_data::dimension::Dimension::THE_END
        {
            let exit_portal_pos =
            ...
            tracing::debug!("正在 {:?} 创建传送门", exit_portal_pos);
        ...
        level
            ...
            .read_chunk_sync(&chunk_pos, |chunk| {
                ...
                sections.iter().all(|s| match s {
                    papokin_world::chunk::palette::BlockPalette::Homogeneous(id) => {
                        papokin_data::block_properties::is_air(*id)
                    }
                    papokin_world::chunk::palette::BlockPalette::Heterogeneous(_) => false,
                })
            })
            .unwrap_or(true)
    }
            ...
            for zd in -dist..=dist {
                if xd != 0 || zd != 0 || exit_portal {
                    ...
                    while y > target_min_y {
                        ...
                        if !state.is_air()
                            && (exit_portal || block != &Block::BEDROCK)
                            && state.is_full_cube()
                        {
