...
use papokin_data::block_properties::{TrialSpawnerLikeProperties, TrialSpawnerState};
...
use papokin_data::trial_spawner::TrialSpawnerKey;
            ...
            Self::NoCreativePlayers => {
                ...
                players
                    ...
                    .filter_map(|player| {
                        ...
                        if mode == papokin_util::GameMode::Creative
                            || mode == papokin_util::GameMode::Spectator
                        {
                            return None;
                        }
...
impl From<&papokin_data::trial_spawner::TrialSpawnerConfigData> for TrialSpawnerConfig {
    fn from(data: &papokin_data::trial_spawner::TrialSpawnerConfigData) -> Self {
...
impl TrialSpawnerConfig {
    ...
    pub fn from_key(key: &str) -> Option<Self> {
        if let Some(cfg) = papokin_data::trial_spawner::get_trial_spawner_config(key) {
        ...
        if let Some(cfg) = papokin_data::trial_spawner::get_trial_spawner_config(&with_normal) {
            return Some(Self::from(cfg));
        }
        None
    }
    ...
    pub fn from_ominous_key(key: &str) -> Option<Self> {
        ...
        if let Some(cfg) = papokin_data::trial_spawner::get_trial_spawner_config(&with_ominous) {
            return Some(Self::from(cfg));
        }
        None
    }
...
impl TrialSpawnerFullConfig {
    #[must_use]
    pub fn from_key(key: &papokin_data::trial_spawner::TrialSpawnerKey) -> Self {
        Self {
...
impl TrialSpawner {
    ...
    pub fn get_state(&self, world: &World, spawner_pos: BlockPos) -> TrialSpawnerState {
        ...
        if block.id == papokin_data::BlockId::TRIAL_SPAWNER {
    ...
    pub fn spawn_mob(&mut self, world: &Arc<World>, spawner_pos: BlockPos) -> Option<Uuid> {
        ...
        if is_baby {
            ...
            ent.set_synced_data(papokin_data::tracked_data::ageable_mob::DATA_BABY_ID, true);
        }

        if let Some(size) = slime_size {
            entity
                .get_entity()
                .set_synced_data(papokin_data::tracked_data::slime::ID_SIZE, i32::from(size));
        }

        if let Some(equip) = equipment_loot_table
            && let Some(loot_table) = papokin_data::loot_table::get_loot_table(&equip)
    ...
    pub fn eject_reward(
        ...
        if let Some(key) = ejecting_loot_table
            && let Some(loot_table) = papokin_data::loot_table::get_loot_table(key)
        {
            let seed = rand::random::<i64>();
            let items = crate::world::loot::generate_loot(loot_table, seed);
            for stack in items {
...
impl BlockEntity for TrialSpawnerBlockEntity {
    ...
    fn tick(&self, world: &Arc<World>) {
        ...
        let is_ominous = if block.id == papokin_data::BlockId::TRIAL_SPAWNER {
