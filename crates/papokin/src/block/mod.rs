...
use papokin_util::random::{RandomGenerator, get_seed, xoroshiro128::Xoroshiro};
...
pub trait BlockBehaviour: Send + Sync {
    ...
    /// 处理方块事件，这是一种特定于方块的事件，带有整数 ID 和数据。
    ///
    /// 返回该事件是否被成功处理
    ...
    /// 与主方块一同放置的附加方块，出现在多方块
    /// 放置位置（如门的上半部分、床的床头、高植物的顶部）。
    ///
    /// 主方块已由调用方包含；实现只需返回
    /// [`Self::placed`] 即将放置的额外 `(position, state)` 对。
...
pub fn drop_loot(
    ...
    let has_silk_touch = params.tool.as_ref().is_some_and(|tool| {
        papokin_data::Enchantment::from_name("silk_touch")
...
/// 原版 `AbstractContainerMenu.getRedstoneSignalFromBlockEntity`：读取方块
/// 实体自身的容器，就像比较器那样。
#[must_use]
...
pub fn calculate_comparator_output(inventory: &dyn papokin_inventory::Inventory) -> u8 {
