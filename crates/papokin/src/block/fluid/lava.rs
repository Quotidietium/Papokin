...
use std::sync::Arc;
type FlowingFluidProperties = papokin_data::fluid::FlowingWaterLikeFluidProperties;
...
impl FlowingLava {
    ...
    fn receive_neighbor_fluids(world: &Arc<World>, block_pos: &BlockPos) -> bool {
        // 判断是否应将流体替换为（圆石、黑曜石、石头等）其中之一的逻辑
        ...
        for dir in BlockDirection::all() {
            // 原版在这里测试除向下以外的所有方向。熔岩落入水中
            // 下方的方块由 `spread_to` 变成石头，而非本次扫描，因此 Down
            // 会被跳过而不是终止——下方有水的岩浆方块
            // 且其旁边的水仍会生成圆石。
