...
use papokin_data::{Block, BlockDirection, BlockId, BlockState};
use papokin_inventory::screen_handler::ScreenHandlerFactory;
use papokin_protocol::java::server::play::SUseItemOn;
...
pub enum BlockActionResult {
    /// 操作成功 | 同原版中的 SUCCESS
    Success,
    /// 动作成功，我们应向服务器挥动手臂 | 与原版的 `SUCCESS_SERVER` 相同
    SuccessServer,
    /// 阻止其他动作被执行 | 与原版的 CONSUME 相同
    Consume,
    /// 允许执行其他操作，但指示操作失败 | 同原版中的 FAIL
    Fail,
    /// 允许执行其他操作 | 同原版中的 PASS
    Pass,
    /// 使用方块的默认动作：`normal_use` | 等同于原版中的 `PASS_TO_DEFAULT_BLOCK_ACTION`
...
/// 标记未注册任何行为的方块。绝不会作为真实索引分发。
const NO_BEHAVIOUR: u16 = u16::MAX;

pub struct BlockRegistry {
    block_indices: [u16; papokin_data::BlockId::COUNT as usize],
...
impl Default for BlockRegistry {
    fn default() -> Self {
        Self {
            block_indices: [NO_BEHAVIOUR; papokin_data::BlockId::COUNT as usize],
            behaviours: Vec::new(),
...
impl BlockRegistry {
    ...
    fn entity_blocks_block_placement(entity: &dyn EntityBase) -> bool {
        ...
        entity_type == &papokin_data::entity::EntityType::END_CRYSTAL
            || entity_type == &papokin_data::entity::EntityType::FALLING_BLOCK
            || entity_type == &papokin_data::entity::EntityType::TNT
    ...
    pub fn place_block(
        ...
        match player.gamemode.load() {
            papokin_util::GameMode::Spectator | papokin_util::GameMode::Adventure => {
        ...
        if location.0.y + face.to_offset().y > world.get_top_y() {
            player.send_system_message_raw(
                &papokin_util::text::TextComponent::translate(
                    papokin_data::translation::java::BUILD_TOOHIGH,
                    vec![papokin_util::text::TextComponent::text(
        ...
        if world.is_in_spawn_protection(player, &final_block_pos) {
            player.send_system_message(&papokin_util::text::TextComponent::translate(
        ...
        // 镜像原版遮挡检查：只有阻挡建造的实体才能阻止
        // 放置。（例如箭/经验球/展示实体/标记不应如此）
        ...
        if let Ok(hand) = papokin_util::Hand::from_packet_id(use_item_on.hand.0)
...
mod replacement_tests {
    ...
    use papokin_data::{Block, BlockState, block_properties::GlowLichenLikeProperties};
