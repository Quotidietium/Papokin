//! 末影龙战斗管理器。处理末影龙的生命周期、末影水晶
//! 生成、Boss 血条、折跃门生成、重生序列以及出口传送门。
//!
//! 与原版 `net.minecraft.world.level.dimension.end.EnderDragonFight` 行为一致
//! 以及 `DragonRespawnStage`。

use std::sync::{Arc, Mutex};
use tracing::{debug, info};
use uuid::Uuid;

use papokin_data::{Block, entity::EntityType, world::WorldEvent};
use papokin_util::{
...
pub use papokin_world::generation::feature::features::end_spike::Spike as EndSpike;

// ── 重生阶段 ─────────────────────────────────────────────────────────────

/// 对应原版的 `net.minecraft.world.level.dimension.end.DragonRespawnStage`。
...
impl DragonFight {
    ...
    pub fn get_spikes() -> [EndSpike; 10] {
        papokin_world::generation::feature::features::end_spike::EndSpikeFeature::get_spikes_for_seed(0)
    }
    ...
    /// 检查 8 区块半径内是否存在处于激活状态的出口传送门（`EndPortalBlockEntity`）
    /// 原点的距离，与原版 `hasActiveExitPortal` 一致。
    ...
    pub fn update_crystal_count(&mut self, world: &Arc<World>) {
        ...
        debug!("发现 {} 个末影水晶仍然存活。", self.alive_crystals);
    }
    ...
    pub fn try_respawn(&mut self, world: &Arc<World>) {
        ...
        if location.is_none() {
            debug!("尝试重生，但需要先找到传送门。");
            if let Some(found) = self.find_exit_portal(world) {
                debug!("已找到返回传送门并保存其位置供下次使用。");
                location = Some(found);
            } else {
                debug!("未找到传送门，已创建一个。");
                self.spawn_exit_portal(world, true);
    ...
    pub fn set_respawn_stage(&mut self, world: &Arc<World>, stage: DragonRespawnStage) {
        if self.respawn_stage.is_none() {
            tracing::warn!("末影龙重生未在进行中，无法跳过动画阶段。");
            return;
        }
            ...
            for dy in 0i32..=3 {
                for dx in -2i32..=2 {
                    for dz in -2i32..=2 {
                        ...
                        let mut props =
                            papokin_data::block_properties::OakFenceLikeProperties::default(
                                &Block::IRON_BARS,
                            );
