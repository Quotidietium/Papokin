use papokin_data::dimension::BedRule;
use papokin_data::environment_attribute::{
...
/// 世界的环境属性访问器，与原版的 `EnvironmentAttributeSystem` 一致。
...
impl<'a> EnvironmentAttributes<'a> {
    ...
    /// 在维度级别计算浮点型环境属性（例如 `EnvironmentAttributes.SKY_LIGHT_LEVEL`）。
    #[must_use]
    pub fn get_dimension_value_f32(&self, attribute: EnvironmentAttribute) -> f32 {
        match attribute {
            EnvironmentAttribute::GameplaySkyLightLevel => {
                ...
                // 天气修正（WeatherAttributes.RAIN 和 THUNDER）
