...
impl NetherPortal {
    ...
    pub fn find_safe_location(
        ...
        let direction = if axis == HorizontalAxis::X {
            ...
            BlockDirection::South // 已修复：正 Z 方向
        };
        ...
        // 原版：钳制在 max(bottomY, 70) 与 topYLimit - 9 之间
    ...
    fn is_valid_portal_pos(
        ...
        let perpendicular = if axis == HorizontalAxis::X {
            BlockDirection::South // 已修复：East.rotateYClockwise()
        } else {
            BlockDirection::West // 已修复：South.rotateYClockwise()
        };
...
mod tests {
    use super::*;
    use papokin_util::math::boundingbox::EntityDimensions;
    ...
    fn portal_teleport_position_x_axis() {
        ...
        // 宽度沿 X 方向为 2，所以中心 X 为 10 + 1.0 = 11.0，Y 为 64.0，Z 为 5.5
    ...
    fn portal_teleport_position_z_axis() {
        ...
        let pos = portal.get_teleport_position();
        // 宽度沿 Z 方向为 2，所以中心 X 为 5.5，Y 为 64.0，Z 为 10 + 1.0 = 11.0
    ...
    fn portal_cross_axis_x_to_z() {
        ...
        // 在 Z 传送门中：X 为垂直方向（12 + 0.5 = 12.5），Z 为沿传送门方向（25 + 1.0 = 26.0）
