...
use papokin_data::{Block, BlockDirection, BlockId, world::WorldEvent};
...
impl EndPortal {
    ...
    fn get_mid_pos(world: &World, pos: BlockPos) -> Option<BlockPos> {
        ...
        let offset = match (left_block.id, right_block.id) {
            (Self::FRAME_BLOCK_ID, Self::FRAME_BLOCK_ID) => 0, // 中间方块
            (Self::FRAME_BLOCK_ID, _) => -1,                   // 右侧方块（向左偏移）
            (_, Self::FRAME_BLOCK_ID) => 1,                    // 左侧方块（向右偏移）
            _ => return None,
        };
