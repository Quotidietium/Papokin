...
use papokin_util::loot_table::{LootBonusFormula, LootCondition, LootEntry, LootTable};
use papokin_util::random::{RandomImpl, xoroshiro128::Xoroshiro};
...
pub struct LootContextParameters {
    ...
    pub position: Option<papokin_util::math::vector3::Vector3<f64>>,
    pub world_time: u64,
...
pub fn generate_loot_with_context(
    ...
    let fortune_level = params.tool.as_ref().map_or(0, |tool| {
        let fortune = papokin_data::Enchantment::from_name("fortune")
            .map_or(0, |e| tool.get_enchantment_level(e));
        let looting = papokin_data::Enchantment::from_name("looting")
            .map_or(0, |e| tool.get_enchantment_level(e));
        fortune.max(looting)
    });
...
pub fn fill_chest_inventory(
    inventory: &std::sync::Arc<dyn papokin_inventory::Inventory>,
    table: &LootTable,
    seed: i64,
) {
