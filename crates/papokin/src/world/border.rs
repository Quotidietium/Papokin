...
impl Worldborder {
    ...
    pub fn clamp_block(&self, x: i32, z: i32) -> (i32, i32) {
        let half = self.new_diameter / 2.0;
        // 窄于一个方块的边界跨越不了方块边界，使 `max`
        // 低于 `min`。`Ord::clamp` 在倒置区间上会 panic，因此折叠
        // 范围改为落在持有中心的那个方块上。
...
mod tests {
    ...
    /// `find_safe_location` 仅会落到 `clamp_block` 回退逻辑
    /// 因为零宽边界会拒绝它扫描的每一个候选：`contains`
    /// 化简为 `x >= center && x < center`，任何坐标都无法满足。
    #[test]
    ...
    /// `/worldborder set 0` 会被接受，因为该命令的消费者
    /// `BoundedNumArgumentConsumer::new().min(0.0)`，且该边界包含在内。
    #[test]
    ...
    /// 中心偏离方块网格会使两条边落在同一个方块内，因此
    /// 边界即使直径不为零，也不跨越任何方块边界。
    #[test]
